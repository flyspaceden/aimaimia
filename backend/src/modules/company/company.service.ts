...
export class CompanyService {
  ...
  private mapInspectionReports(documents: any[] = [], companyIsPublic = true) {
    return documents
      .filter((doc) => doc.type === 'INSPECTION' && doc.verifyStatus === 'VERIFIED')
      .map((doc) => {
        const previewAvailable = companyIsPublic && this.uploadService.canPreviewCompanyDocument(doc.fileUrl);
