...
export class AfterSaleController {
  ...
  createReturnShippingPayment(
    ...
    return this.afterSaleShippingPaymentService.createOrGetPaymentForBuyer(
      userId,
      id,
    );
  }

  /** 发起小程序买家退货运费支付（服务端固定微信 JSAPI） */
  @Post(":id/return-shipping-payment/mini-program")
  createMiniProgramReturnShippingPayment(
    @CurrentUser("sub") userId: string,
    ...
    return this.afterSaleShippingPaymentService.createOrGetMiniProgramPaymentForBuyer(
      userId,
      id,
  ...
  /** 安全取消退货运费支付：服务端查单/关单成功后才转 CLOSED。 */
  @Post(":id/return-shipping-payment/cancel")
  cancelReturnShippingPayment(
    @CurrentUser("sub") userId: string,
    @Param("id") id: string,
  ) {
    return this.afterSaleShippingPaymentService.cancelPaymentForBuyer(userId, id);
  }

  /** 确认收到换货商品 */
  @Post(":id/confirm")
  confirmReceive(@CurrentUser("sub") userId: string, @Param("id") id: string) {
    return this.afterSaleService.confirmReceive(userId, id);
  }

  /** 申请平台仲裁 */
  @Post(":id/escalate")
  escalate(@CurrentUser("sub") userId: string, @Param("id") id: string) {
    return this.afterSaleService.escalate(userId, id);
  }

  /** 接受关闭（放弃售后） */
  @Post(":id/accept-close")
  acceptClose(@CurrentUser("sub") userId: string, @Param("id") id: string) {
