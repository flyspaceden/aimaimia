...
function createHarness() {
  const tx = {
    ...
    afterSaleShippingPayment: {
      ...
      findUnique: jest.fn((args: any) => {
        if (args?.where?.afterSaleId) return Promise.resolve(null);
        return Promise.resolve(args?.include?.afterSale
          ? { ...baseShippingPayment, afterSale: { userId: "user_001" } }
          : baseShippingPayment);
      }),
  ...
  const prisma = {
    ...
    afterSaleShippingPayment: {
      findFirst: jest.fn().mockResolvedValue({
        ...baseShippingPayment,
        afterSale: { userId: "user_001" },
      }),
      findUnique: jest.fn().mockResolvedValue(baseShippingPayment),
    },
    session: {
      findFirst: jest.fn().mockResolvedValue({
        authIdentity: {
          userId: "user_001",
          provider: "WECHAT",
          identifier: "openid_exact_session",
          appId: "wx-mini",
          verified: true,
        },
      }),
  ...
  const alipayService = {
    ...
    closeOrder: jest.fn().mockResolvedValue({ success: true, terminal: true }),
  ...
  const wechatPayService = {
    ...
    getMiniProgramAppId: jest.fn().mockReturnValue("wx-mini"),
    ...
    createMiniProgramOrder: jest.fn().mockResolvedValue({
      appId: "wx-mini",
      timeStamp: "1716537601",
      nonceStr: "mini-nonce",
      package: "prepay_id=mini-prepay",
      signType: "RSA",
      paySign: "mini-sign",
      prepayId: "mini-prepay",
    }),
  ...
  const service = new AfterSaleShippingPaymentService(
    prisma as any,
...
describe("AfterSaleShippingPaymentService provider dispatch", () => {
  it("creates WECHAT_PAY shipping payment params when original checkout used WeChat Pay", async () => {
    ...
    const result = await service.createOrGetPaymentForBuyer(
      "user_001",
      "as_001",
    );
  ...
  it("creates mini-program return-shipping payment from the exact current WeChat identity", async () => {
    const { service, tx, prisma, alipayService, wechatPayService } =
      createHarness();
    let miniPayment: any = {
      ...baseShippingPayment,
      paymentScene: "MINI_PROGRAM",
    };
    tx.afterSaleShippingPayment.upsert.mockResolvedValue(miniPayment);
    prisma.afterSaleShippingPayment.findUnique.mockImplementation(() => Promise.resolve(miniPayment));
    tx.afterSaleShippingPayment.findUnique.mockImplementation((args: any) => {
      if (args?.where?.afterSaleId) return Promise.resolve(null);
      return Promise.resolve(args?.include?.afterSale
        ? { ...miniPayment, afterSale: { userId: "user_001" } }
        : miniPayment);
    });
    tx.afterSaleShippingPayment.updateMany.mockImplementation(({ data }: any) => {
      miniPayment = { ...miniPayment, ...data };
      return Promise.resolve({ count: 1 });
    });
    service.setWechatPayService(wechatPayService as any);

    const result = await service.createOrGetMiniProgramPaymentForBuyer(
      "user_001",
      "as_001",
      { sessionId: "auth_session_1", authIdentityId: "identity_1" },
    );
    ...
    expect(wechatPayService.createMiniProgramOrder).toHaveBeenCalledWith({
      outTradeNo: "AS_SHIP_PAY_as_001",
      amount: 18.13,
      description: "爱买买退货运费-as_001",
      openId: "openid_exact_session",
    });
    ...
    const replay = await service.createOrGetMiniProgramPaymentForBuyer(
      "user_001",
      "as_001",
      { sessionId: "auth_session_1", authIdentityId: "identity_1" },
    );
    expect(replay.paymentParams).toEqual(result.paymentParams);
    expect(wechatPayService.createMiniProgramOrder).toHaveBeenCalledTimes(1);
    expect((miniPayment.paymentParamState as any).state).toBe("READY");
  });

  it("queries the original merchant number before retrying an UNCERTAIN JSAPI preorder", async () => {
    const { service, tx, prisma, wechatPayService } = createHarness();
    let miniPayment: any = {
      ...baseShippingPayment,
      paymentScene: "MINI_PROGRAM",
    };
    ...
    wechatPayService.createMiniProgramOrder
    ...
    wechatPayService.queryOrder.mockResolvedValue({ outcome: "DEFINITIVE_NOT_FOUND" });
    service.setWechatPayService(wechatPayService as any);

    await expect(service.createOrGetMiniProgramPaymentForBuyer(
      "user_001",
      "as_001",
      { sessionId: "auth_session_1", authIdentityId: "identity_1" },
    )).rejects.toBeInstanceOf(Error);
    expect((miniPayment.paymentParamState as any).state).toBe("UNCERTAIN");
    ...
    await expect(service.createOrGetMiniProgramPaymentForBuyer(
      ...
      paymentParams: expect.objectContaining({ prepayId: "mini-prepay" }),
    }));
    expect(wechatPayService.queryOrder).toHaveBeenCalledWith(
      miniPayment.merchantPaymentNo,
    );
    expect(wechatPayService.queryOrder.mock.invocationCallOrder[0])
      .toBeLessThan(wechatPayService.createMiniProgramOrder.mock.invocationCallOrder[1]);
    expect((miniPayment.paymentParamState as any).state).toBe("READY");
  });

  it("rejects reusing an App shipping payment in the mini-program scene", async () => {
    const { service, tx, wechatPayService } = createHarness();
    tx.afterSaleShippingPayment.findUnique.mockResolvedValueOnce({
      ...baseShippingPayment,
      paymentScene: "APP",
    });
    service.setWechatPayService(wechatPayService as any);
    ...
    await expect(
      service.createOrGetMiniProgramPaymentForBuyer("user_001", "as_001", {
        sessionId: "auth_session_1",
        authIdentityId: "identity_1",
      }),
    ).rejects.toMatchObject({
      response: expect.objectContaining({ code: "PAYMENT_SCENE_MISMATCH" }),
    });
    expect(wechatPayService.createMiniProgramOrder).not.toHaveBeenCalled();
  });

  it("keeps pending WeChat shipping refund in REFUNDING instead of marking REFUNDED", async () => {
    const { service, tx, alipayService, wechatPayService } = createHarness();
    service.setWechatPayService(wechatPayService as any);
    const merchantRefundNo = (service as any).getWechatMerchantRefundNo(
      "as_001",
    );
    ...
    expect(tx.afterSaleShippingPayment.updateMany).toHaveBeenLastCalledWith(
      expect.objectContaining({
        where: {
          afterSaleId: "as_001",
  ...
  it("keeps synchronous WeChat SUCCESS refund in REFUNDING when verification fields are missing", async () => {
    ...
    expect(tx.afterSaleShippingPayment.updateMany).not.toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it("queries stale WeChat shipping refunds and reuses notify state handling", async () => {
    ...
    expect(tx.afterSaleShippingPayment.findMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          provider: "WECHAT_PAY",
          status: "REFUNDING",
  ...
  it("marks very stale WeChat shipping refund as FAILED when queryRefund keeps returning no result", async () => {
    ...
    try {
      ...
      expect(wechatPayService.queryRefund).toHaveBeenCalledWith(
        merchantRefundNo,
      );
      expect(wechatPayService.refund).not.toHaveBeenCalled();
      expect(tx.afterSaleShippingPayment.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          data: expect.objectContaining({
            status: "FAILED",
            failureReason:
              expect.stringContaining("微信退货运费退款查单无结果"),
          }),
        }),
      );
    } finally {
  ...
  it.each(["PROCESSING", "ABNORMAL", "FAILED"])(
    "ignores %s WeChat shipping refund notify when amount mismatches",
    async (tradeState) => {
      const { service, tx, wechatPayService } = createHarness();
      service.setWechatPayService(wechatPayService as any);
      tx.afterSaleShippingPayment.findUnique.mockResolvedValue({
  ...
  it("shortens long afterSale IDs for WeChat merchant payment and refund numbers", async () => {
    ...
    const merchantPaymentNo = (service as any).getMerchantPaymentNo(
      longAfterSaleId,
      "WECHAT_PAY",
    );
    const merchantRefundNo = (service as any).getWechatMerchantRefundNo(
  ...
  it("safely closes a stale mini-program shipping payment before releasing the deletion blocker", async () => {
    const { service, prisma, tx, wechatPayService } = createHarness();
    service.setWechatPayService(wechatPayService as any);
    const pending = {
      ...baseShippingPayment,
      paymentScene: "MINI_PROGRAM",
    ...
    prisma.afterSaleShippingPayment.findFirst.mockResolvedValue(pending);
    wechatPayService.queryOrder.mockResolvedValue({
      outcome: "DEFINITIVE_NOT_FOUND",
    });

    await expect(service.cancelPaymentForBuyer("user_001", "as_001"))
      .resolves.toEqual({ ok: true, status: "CLOSED" });

    expect(wechatPayService.closeOrder).not.toHaveBeenCalled();
    expect(tx.afterSaleShippingPayment.updateMany).toHaveBeenCalledWith({
      where: {
        id: pending.id,
  ...
  it("keeps the payment blocking deletion when provider query is uncertain", async () => {
    const { service, prisma, tx, wechatPayService } = createHarness();
    service.setWechatPayService(wechatPayService as any);
    prisma.afterSaleShippingPayment.findFirst.mockResolvedValue({
      ...baseShippingPayment,
      paymentScene: "MINI_PROGRAM",
      status: "PENDING",
    });
...
describe("AfterSaleModule provider wiring", () => {
  it("sets optional WeChat Pay service through ModuleRef without changing constructor injection", () => {
    const paymentService = {
      setAfterSaleRefundService: jest.fn(),
      setAfterSaleShippingPaymentService: jest.fn(),
    };
    ...
    expect(
      afterSaleShippingPaymentService.setWechatPayService,
