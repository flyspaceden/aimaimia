...
describe("AfterSaleShippingPaymentService", () => {
  ...
  beforeEach(() => {
    ...
    tx.afterSaleShippingPayment.upsert.mockResolvedValue(
      shippingPaymentFixture(),
    );
    // 创建支付单时服务会先按 afterSaleId 检查跨端已有支付；默认场景应
    // 表示“尚无支付单”。回调/查单路径按 merchantPaymentNo 查询时则
    // 返回默认支付单。各退款/终态用例仍可在用例内覆盖此实现。
    tx.afterSaleShippingPayment.findUnique.mockImplementation((args: any) => {
      if (args?.where?.afterSaleId) return Promise.resolve(null);
      const payment = shippingPaymentFixture();
      return Promise.resolve(args?.include?.afterSale
        ? { ...payment, afterSale: { userId: "user_001" } }
        : payment);
    });
    ...
    service = new AfterSaleShippingPaymentService(
      prisma as any,
  ...
  it("estimates buyer return shipping fee from RuleConfig rounded to cents", async () => {
    await expect(service.estimateReturnShippingFee("as_001")).resolves.toBe(
      18.13,
    );
  ...
  it("closes a verified unpaid WeChat shipping payment after provider 204 success", async () => {
    const payment = shippingPaymentFixture({
      provider: "WECHAT_PAY",
      paymentScene: "MINI_PROGRAM",
    });
    prisma.afterSaleShippingPayment.findFirst.mockResolvedValue(payment);
    ...
    expect(wechatPayService.closeOrder).toHaveBeenCalledWith(payment.merchantPaymentNo);
  ...
  it("locks payment amount to request returnShippingFee even when RuleConfig changes", async () => {
    ...
    await expect(
      service.createOrGetPaymentForBuyer("user_001", "as_001"),
  ...
  it("fillReturnShipping blocks buyer-paid return shipping until payment is recorded", async () => {
    ...
    await expect(
      afterSaleService.fillReturnShipping("user_001", "as_001", {
  ...
  it("rejects buyer wrapper when the after-sale request belongs to another user", async () => {
    ...
    await expect(
      service.createOrGetPaymentForBuyer("user_other", "as_001"),
  ...
  it("handlePaymentSuccess refunds late CLOSED success through original Alipay trade", async () => {
    ...
    expect(tx.afterSaleRequest.update).not.toHaveBeenCalledWith(
      expect.objectContaining({
  ...
  it.each(["UNPAID", "FAILED"])(
    ...
    async (status) => {
      ...
      expect(tx.afterSaleShippingPayment.update).toHaveBeenCalledWith(
        expect.objectContaining({
...
describe("AfterSaleController return shipping payment route", () => {
  it("starts buyer return shipping payment through the shipping payment service", async () => {
    ...
    await expect(
      controller.createReturnShippingPayment("user_001", "as_001"),
    ).resolves.toEqual(
      expect.objectContaining({
        merchantPaymentNo: "AS_SHIP_PAY_as_001",
        paymentParams: { channel: "alipay", orderStr: "alipay-order-str" },
    ...
    expect(
      shippingPaymentService.createOrGetPaymentForBuyer,
