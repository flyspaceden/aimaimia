...
import { SELLER_AUDIT_KEY } from '../seller/common/decorators/seller-audit.decorator';
...
describe('ProductVisualCommerceController', () => {
  it('keeps quote reads product-bound and delegates rate-card filtering to the adapter', async () => {
    ...
    expect(Reflect.getMetadata(SELLER_AUDIT_KEY, controller.listRateCards)).toEqual({
      action: 'RESOLVE_PRODUCT_VISUAL_RATE_CARDS',
