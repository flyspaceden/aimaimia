...
describe('AdminProductPaidVisualCandidatesController', () => {
  it('delegates fact review to the candidate service and never performs publication itself', async () => {
    const candidates = {
      listPendingForAdmin: jest.fn().mockResolvedValue([{ id: 'optimization-1', status: 'PENDING_REVIEW' }]),
    ...
    await expect(controller.list()).resolves.toEqual([{ id: 'optimization-1', status: 'PENDING_REVIEW' }]);
