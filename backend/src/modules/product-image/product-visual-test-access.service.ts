...
import { AIMAI_VISUAL_CLIENT_ID, AIMAI_VISUAL_TENANT_ID } from './aimai-product-visual-adapter.service';
...
export class ProductVisualTestAccessService {
  ...
  async grant(input: {
    ...
    if (this.config.get('AI_VISUAL_AGENT_TEST_ACCESS_ENABLED', 'false') !== 'true'
    ...
    const rateCard = await this.credits.upsertRateCard({
      ...
      code: this.rateCode(input.visualMode),
    ...
    const existingObjectPolicy = (await this.invocations.listBudgetPolicies({ provider: PROVIDER, model: MODEL, take: 500 }))
      .find((policy) => policy.scope === VisualAgentBudgetScope.EXTERNAL_OBJECT
        ...
        && policy.enabled);
    const objectDailyCapCents = Math.max(existingObjectPolicy?.dailyCapCents ?? 0, input.dailyCallLimit * RESERVE_CENTS);
    const objectWeeklyCapCents = Math.max(existingObjectPolicy?.weeklyCapCents ?? 0, input.weeklyCallLimit * RESERVE_CENTS);
    ...
    for (const scope of [
      ...
      await this.invocations.upsertBudgetPolicy({
        ...
        dailyCapCents: objectScope ? objectDailyCapCents : actorScope ? 500 : 2_000,
        weeklyCapCents: objectScope ? objectWeeklyCapCents : actorScope ? 2_500 : 10_000,
  ...
  private rateCode(mode: ProductVisualMode) {
    return mode === ProductVisualMode.MARKETING_SCENE
      ? 'STAGING_WAN_PRO_MARKETING_V1'
      : `STAGING_WAN_PRO_${mode}_V1`;
  }
