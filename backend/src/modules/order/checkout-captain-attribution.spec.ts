...
function makeService(tx: any) {
  ...
  const vipDirectService = {
    createFrozenForPaidOrder: jest.fn().mockResolvedValue('credited'),
  };
  const captainAttributionService = {
    createFrozenForPaidOrder: jest.fn().mockResolvedValue('credited'),
  };
  service.setVipDirectReferralCommissionService(vipDirectService as any);
  service.setCaptainAttributionService(captainAttributionService as any);
  return { service, prisma, vipDirectService, captainAttributionService };
}

describe('CheckoutService captain attribution hook', () => {
  it('calls captain attribution service once per created normal goods order inside the payment transaction', async () => {
    const { tx } = makeTx(makeNormalSession());
    const { service, captainAttributionService } = makeService(tx);
    ...
    expect(captainAttributionService.createFrozenForPaidOrder).toHaveBeenCalledTimes(2);
    expect(captainAttributionService.createFrozenForPaidOrder).toHaveBeenNthCalledWith(1, tx, 'order-1');
    expect(captainAttributionService.createFrozenForPaidOrder).toHaveBeenNthCalledWith(2, tx, 'order-2');
  });

  it('does not call captain attribution service for VIP package payment', async () => {
    ...
    expect(captainAttributionService.createFrozenForPaidOrder).not.toHaveBeenCalled();
  });

  it('does not call captain attribution service for group-buy payment', async () => {
    const session = makeNormalSession({
      id: 'sess-gb',
      bizType: 'GROUP_BUY',
  ...
  it('does not call captain attribution service again for duplicate payment callback', async () => {
