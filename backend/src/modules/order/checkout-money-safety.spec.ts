...
describe('CheckoutService group-buy rebate deduction on ordinary checkout', () => {
  ...
  it('团购被推荐人支付成功但返还服务缺失时拒绝静默跳过 pending 返还流水', async () => {
    const session = {
      id: 'sess-gb-ref-missing-service',
      userId: 'buyer_2',
      status: 'PAYING',
      merchantOrderNo: 'M-GB-REF-NO-SVC',
      paymentChannel: 'ALIPAY',
      expectedTotal: 100,
      goodsAmount: 100,
      shippingFee: 0,
    ...
    await expect(service.handlePaymentSuccess('M-GB-REF-NO-SVC', 'trade-1'))
