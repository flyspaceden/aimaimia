...
describe("PaymentController.handleWechatNotify", () => {
  ...
  const buildController = (overrides?: {
    ...
    getAfterSaleShippingPaymentScene?: jest.Mock;
  }) => {
    const paymentService = {
      getByOrderId: jest.fn(),
      handlePaymentCallback:
        overrides?.handlePaymentCallback ??
        jest.fn().mockResolvedValue({ code: "SUCCESS" }),
      assertWechatAmountMatchesSession:
        overrides?.assertWechatAmountMatchesSession ?? jest.fn(),
      assertWechatPaymentAmountMatches:
        overrides?.assertWechatPaymentAmountMatches ??
        jest.fn().mockResolvedValue(undefined),
      assertWechatAfterSaleShippingPaymentAmountMatches:
        overrides?.assertWechatAfterSaleShippingPaymentAmountMatches ??
        jest.fn().mockResolvedValue(undefined),
      getAfterSaleShippingPaymentScene:
        overrides?.getAfterSaleShippingPaymentScene ??
        jest.fn().mockResolvedValue("APP"),
      handleWechatRefundNotify:
        overrides?.handleWechatRefundNotify ??
        jest.fn().mockResolvedValue(undefined),
    };
    const alipayService = { verifyNotify: jest.fn() };
    const checkoutService = {
      findByMerchantOrderNo:
        overrides?.findByMerchantOrderNo ??
    ...
    const getAppId =
      overrides?.getAppId ?? jest.fn().mockReturnValue("wx_app_1");
    const wechatPayService = {
      parseNotify:
        overrides?.parseNotify ??
        jest.fn().mockResolvedValue(overrides?.parsedNotify ?? paymentNotify),
      getAppId,
      getMchId: overrides?.getMchId ?? jest.fn().mockReturnValue("mch_1"),
      matchesPaymentScene:
        overrides?.matchesPaymentScene ??
        jest
          .fn()
          .mockImplementation(
            (identity: any, scene: string) =>
              scene === "APP" &&
              identity?.appId === getAppId() &&
              identity?.tradeType === "APP",
          ),
    };
    const deliveryPaymentsService = {
      assertWechatAmountMatchesCheckout:
        overrides?.assertWechatDeliveryAmountMatches ??
        jest.fn().mockResolvedValue(undefined),
    };
    const res = {
  ...
  it("returns 401 when WeChat signature parsing fails", async () => {
    ...
    await controller.handleWechatNotify(
      {},
      { rawBody } as any,
      headers as any,
      res as any,
  ...
  it("accepts MINI_PROGRAM notify only when checkout scene, mini AppID and JSAPI all match", async () => {
    const miniNotify = {
      ...paymentNotify,
      appId: "wx_mini_1",
      tradeType: "JSAPI",
    };
    const matchesPaymentScene = jest
      .fn()
      .mockImplementation(
        (identity: any, scene: string) =>
          scene === "MINI_PROGRAM" &&
          identity.appId === "wx_mini_1" &&
          identity.tradeType === "JSAPI",
      );
    const { controller, paymentService, res } = buildController({
      parsedNotify: miniNotify,
    ...
    expect(matchesPaymentScene).toHaveBeenCalledWith(
      { appId: "wx_mini_1", tradeType: "JSAPI" },
      "MINI_PROGRAM",
    );
  ...
  it.each([
    ["wrong mini AppID", { appId: "wx_other", tradeType: "JSAPI" }],
    ["wrong mini trade_type", { appId: "wx_mini_1", tradeType: "APP" }],
  ])("rejects MINI_PROGRAM notify with %s", async (_case, identity) => {
    const { controller, paymentService, res } = buildController({
      parsedNotify: { ...paymentNotify, ...identity },
      findByMerchantOrderNo: jest.fn().mockResolvedValue({
        expectedTotal: 100,
        merchantOrderNo: paymentNotify.outTradeNo,
        paymentScene: "MINI_PROGRAM",
      }),
      matchesPaymentScene: jest
        .fn()
        .mockImplementation(
          (candidate: any, scene: string) =>
            scene === "MINI_PROGRAM" &&
            candidate.appId === "wx_mini_1" &&
            candidate.tradeType === "JSAPI",
        ),
    });
  ...
  it("acks 200 without callback when payment amount validation fails", async () => {
    ...
    expect(
      paymentService.assertWechatAmountMatchesSession,
  ...
  it("validates legacy Payment amount when CheckoutSession is not found", async () => {
    ...
    expect(
      paymentService.assertWechatPaymentAmountMatches,
    ).toHaveBeenCalledWith(paymentNotify.outTradeNo, paymentNotify.amountFen);
    expect(paymentService.handlePaymentCallback).toHaveBeenCalled();
  });
  ...
  it("acks 200 without callback when CheckoutSession and legacy Payment are missing", async () => {
    const { controller, paymentService, res } = buildController({
      findByMerchantOrderNo: jest.fn().mockResolvedValue(null),
      assertWechatPaymentAmountMatches: jest
        .fn()
        .mockRejectedValue(new BadRequestException("支付记录不存在")),
    });
  ...
  it("validates after-sale shipping payment amount without CheckoutSession lookup", async () => {
    ...
    const { controller, paymentService, checkoutService, res } =
      buildController({
    ...
    expect(
      paymentService.assertWechatAfterSaleShippingPaymentAmountMatches,
    ).toHaveBeenCalledWith("AS_SHIP_PAY_after_sale_1", 1813);
    expect(paymentService.handlePaymentCallback).toHaveBeenCalledWith(
  ...
  it("accepts mini-program after-sale shipping notify only with its stored scene", async () => {
    const shippingNotify = {
      ...paymentNotify,
      appId: "wx_mini_1",
      tradeType: "JSAPI",
    ...
    const { controller, paymentService, res } = buildController({
      parsedNotify: shippingNotify,
      getAfterSaleShippingPaymentScene: jest
        .fn()
        .mockResolvedValue("MINI_PROGRAM"),
      matchesPaymentScene,
    });
  ...
  it("validates delivery payment amount without normal CheckoutSession lookup", async () => {
    ...
    expect(
      deliveryPaymentsService.assertWechatAmountMatchesCheckout,
    ).toHaveBeenCalledWith("PSZF0000000000001", 4900);
    expect(paymentService.handlePaymentCallback).toHaveBeenCalledWith({
  ...
  it.each(["SUCCESS", "CLOSED"])(
    "delegates refund %s notify to PaymentService",
    async (tradeState) => {
      const parsedNotify = {
        ...refundNotify,
        tradeState,
