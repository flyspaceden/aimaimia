...
function build(overrides: { quote?: Record<string, unknown> } = {}) {
  ...
  const runner = {
    preflightProvider: jest.fn().mockResolvedValue(undefined),
    submitProvider: jest.fn().mockResolvedValue({ kind: 'ACCEPTED', providerTaskId: 'wan-task-1', state: 'QUEUED' }),
    queryProvider: jest.fn(),
    fetchProviderOutput: jest.fn(),
  };
  return { service: new VisualPaidExecutionService(credits as any, invocations as any, runner as any), credits, invocations, runner };
}
...
describe('VisualPaidExecutionService', () => {
  ...
  it('submits only a reserved quote whose source reference and fixed plan still match', async () => {
    ...
    expect(runner.preflightProvider).toHaveBeenCalledWith(expect.objectContaining({ provider: 'BAILIAN_WAN', model: 'wan2.7-image' }));
    ...
    expect(runner.submitProvider).toHaveBeenCalledWith(expect.objectContaining({ invocationId: 'invocation-1', provider: 'BAILIAN_WAN', model: 'wan2.7-image' }));
  });

  it('releases a reserved quote before any invocation when local provider preflight is unavailable', async () => {
    const { service, credits, invocations, runner } = build();
    runner.preflightProvider.mockRejectedValue(new ServiceUnavailableException('disabled'));
  ...
  it('releases a known Provider rejection but preserves a billing-unknown submission for reconciliation', async () => {
    ...
    declined.runner.submitProvider.mockResolvedValue({ kind: 'DECLINED', code: 'INVALID_REQUEST' });
    ...
    const unknown = build();
    unknown.runner.submitProvider.mockResolvedValue({ kind: 'UNKNOWN', code: 'TRANSPORT_TIMEOUT', requiresReconciliation: true });
  ...
  it('does not submit a reconciliation or mismatched-evidence quote again', async () => {
    ...
    expect(reconciling.runner.submitProvider).not.toHaveBeenCalled();
  ...
  it('routes a Qwen rate card through the same quote, budget, and Provider-runner guardrails', async () => {
    ...
    expect(runner.preflightProvider).toHaveBeenCalledWith(expect.objectContaining({ provider: 'BAILIAN_QWEN_IMAGE', model: 'qwen-image-3.0' }));
    expect(invocations.reserve).toHaveBeenCalledWith(expect.objectContaining({ provider: 'BAILIAN_QWEN_IMAGE', model: 'qwen-image-3.0' }));
  ...
  it('polls a bound task and downloads output only once the Core has recorded success for verification', async () => {
    ...
    runner.queryProvider.mockResolvedValue({ kind: 'KNOWN', providerTaskId: 'wan-task-1', state: 'SUCCEEDED', outputUrl: 'https://wanx-v1.oss-cn-beijing.aliyuncs.com/result.jpg' });
    runner.fetchProviderOutput.mockResolvedValue({ buffer: Buffer.from('candidate'), mimeType: 'image/jpeg' });
