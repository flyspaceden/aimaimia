...
describe('CaptainMonthlySettlementService V3 monthly profit settlement', () => {
  ...
  it('approves a draft after its own unused-reserve release while retaining refund invalidation', async () => {
    const attribution = makeAttribution({ reserve: 50 });
    const harness = createHarness({ attributions: [attribution] });
    const [draft] = await harness.service.createDraftSettlements('2026-06');

    // Simulate the funding-ledger relation returned by a fresh database query.
    // This release is generated by the draft and must not make the draft stale.
    attribution.profitSnapshot.fundingLedgers.unshift({
      id: 'release-attr-1',
    ...
    const updatedContext = {
      ...
      facts: (draft.meta as any).monthFacts,
    };
    const fingerprintBeforeRefund = (harness.service as any).sourceFingerprint(updatedContext);
    attribution.profitSnapshot.fundingLedgers.push({
      id: 'refund-hold-attr-1',
      type: 'REFUND_ADJUSTMENT',
      amount: 10,
      sourceLedgerId: 'hold-attr-1',
    });
    expect((harness.service as any).sourceFingerprint(updatedContext))
      .not.toBe(fingerprintBeforeRefund);
  });
