...
test('seller pickup station pins the Node 20 compatible QR reader pair', () => {
...
test('verification is bound to the credential that produced the visible preview', () => {
  assert.match(page, /resolvedCredentialRef = useRef<VerifyPickupPayload \| null>\(null\)/);
  assert.match(page, /resolvedCredentialRef\.current = credential/);
  assert.match(page, /const credential = resolvedCredentialRef\.current/);
  assert.match(page, /verifyPickupCredential\(credential\)/);
  assert.match(page, /resolveSessionRef\.current !== resolveSession/);
  assert.doesNotMatch(page, /const credential = toCredential\(mode, credentialInput\)/);
});
