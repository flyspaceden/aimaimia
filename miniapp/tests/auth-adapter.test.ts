...
describe('miniapp auth adapter', () => {
  ...
  it('does not let a late registration completion overwrite a newer WeChat login', async () => {
    ...
    postMock
      ...
      .mockResolvedValueOnce({
        ok: true,
        data: {
          accessToken: 'new-access', refreshToken: 'new-refresh', userId: 'new-user',
          expiresAt: '2026-08-03T00:00:00.000Z', loginMethod: 'wechat-miniapp',
        },
      });
  ...
  it('does not let a late phone merge recreate a session after logout generation changed', async () => {
    const staleMerge = deferred<any>();
    postMock.mockReturnValueOnce(staleMerge.promise);

    const merge = bindWechatMiniappPhone('merge-ticket', '13800138000', '123456');
    await vi.waitFor(() => expect(postMock).toHaveBeenCalledTimes(1));
