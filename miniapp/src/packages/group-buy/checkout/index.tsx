...
export default function GroupBuyCheckoutPage() {
  ...
  const previewQuery = useQuery({
    queryKey: ['group-buy', 'checkout-preview', activityId, addressId, shareCode],
    queryFn: () => MiniGroupBuyRepo.previewCheckout({ activityId, addressId, shareCode: shareCode || undefined, expectedTotal: activity!.price }),
    enabled: Boolean(loggedIn && activity && addressId && !occupiesSlot && (!shareCode || landing?.valid)),
    staleTime: 0,
  });
  ...
  const submitMutation = useMutation({
    mutationFn: async () => {
      if (!activity || !addressId || !preview) throw new Error('MISSING_CHECKOUT_INPUT');
    ...
    onSuccess: async (result) => {
      if (!result.ok) {
        if (result.error.code === 'PENDING_GROUP_BUY_CHECKOUT_EXISTS' || result.error.code === 'PENDING_CHECKOUT_EXISTS') {
    ...
    onError: (error) => {
      ...
      Taro.showToast({ title: missing ? '请选择地址并等待价格计算' : '网络结果不确定，请稍后查看待支付订单', icon: 'none' });
    },
  });
  ...
  if (!loggedIn) return <GroupBuyAuthGate returnUrl={returnUrl} description='登录后才能选择收货地址并完成微信支付' />;
  if (activityQuery.isLoading || addressesQuery.isLoading || currentQuery.isLoading || (shareCode && landingQuery.isLoading)) return <View className='group-buy-page'><CatalogFeedback kind='loading' /></View>;
  ...
  return <View className='group-buy-page group-buy-checkout'>
    ...
    <View className='group-buy-card aim-card'>
    ...
    <View className='group-buy-card aim-card'><View className='group-buy-card__heading'><Text>支付方式</Text><Text>服务端固定通道</Text></View><View className='group-buy-cash-only'><Text className='group-buy-cash-only__mark'>微</Text><View className='group-buy-cash-only__copy'><Text>微信支付</Text><Text>小程序只接收微信 JSAPI 支付参数，不提供渠道选择</Text></View></View></View>
    ...
    {preview ? <View className='group-buy-card aim-card'><View className='group-buy-card__heading'><Text>金额明细</Text><Text>服务端实时计算</Text></View><View className='group-buy-price-lines'><View className='group-buy-price-line'><Text>商品金额</Text><Text>¥{formatGroupBuyMoney(preview.goodsAmount)}</Text></View><View className='group-buy-price-line'><Text>运费</Text><Text>{preview.shippingFee ? `¥${formatGroupBuyMoney(preview.shippingFee)}` : '包邮'}</Text></View><View className='group-buy-price-line'><Text>红包 / 积分 / 返还抵扣</Text><Text>不支持</Text></View><View className='group-buy-price-line group-buy-price-line--total'><Text>应付金额</Text><Text>¥{formatGroupBuyMoney(preview.expectedTotal)}</Text></View></View><Text className='group-buy-notice'>团购为现金购买，不会提交优惠券、消费积分、团购返还余额或其他折扣字段。</Text></View> : null}

    <View className='group-buy-policy' onClick={() => setPolicyAccepted((value) => !value)}><View className={policyAccepted ? 'group-buy-policy__check group-buy-policy__check--active' : 'group-buy-policy__check'}>{policyAccepted ? '✓' : ''}</View><Text>我已核对商品、收货地址和金额，并知晓团购商品不支持取消、退款、退货或换货，仅收货后 24 小时质量问题补发。</Text></View>
    <View className='group-buy-checkout__bar'><View className='group-buy-checkout__bar-copy'><Text>微信现金支付</Text><Text>{preview ? `¥${formatGroupBuyMoney(preview.expectedTotal)}` : '金额计算中'}</Text></View><Button className='group-buy-checkout__submit' loading={submitMutation.isPending} disabled={!preview || !addressId || !policyAccepted || submitMutation.isPending} onClick={async () => { if (!await ensureWechatMiniProgramSession(returnUrl)) return; submitMutation.mutate(); }}>{submitMutation.isPending ? '支付确认中...' : '确认付款'}</Button></View>
  </View>;
}
