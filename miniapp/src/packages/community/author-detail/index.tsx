...
export default function AuthorDetailPage() {
  ...
  return <View className='author-page'>
    <View className='author-hero'><View className='author-hero__rings' />{author.avatar ? <Image className='author-avatar' src={author.avatar} mode='aspectFill' /> : <View className='author-avatar author-avatar--empty'>{author.type === 'company' ? '企' : '人'}</View>}<Text className='author-name'>{author.name}</Text><Text className='author-title'>{author.title || (author.type === 'company' ? '平台入驻企业' : '爱买买用户')}</Text><View className='author-metrics'><View><Text>{author.followerCount || 0}</Text><Text>粉丝</Text></View><View><Text>{author.city || '未填写'}</Text><Text>所在地</Text></View></View></View>
    ...
    <View className='author-actions'><Button loading={mutation.isPending} disabled={mutation.isPending} onClick={() => mutation.mutate()}>{author.isFollowed ? '取消关注' : '+ 关注'}</Button>{author.type === 'company' ? <Button onClick={() => Taro.navigateTo({ url: `/packages/commerce/catalog-company/index?id=${encodeURIComponent(author.companyId || author.id)}` })}>查看企业商品</Button> : null}</View>
  </View>;
}
