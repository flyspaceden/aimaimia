...
export default function ScannerPage() {
  ...
  const openTarget = async (raw: string) => {
    ...
    if (!target) { await Taro.showModal({ title: '无法识别', content: '仅支持 AI爱买买邀请码、团购码和团长码，外部网址不会自动打开。', showCancel: false, confirmText: '我知道了' }); return; }
  ...
  const scan = async () => {
    ...
    try {
      ...
      if (miniProgramPath) {
        ...
        await Taro.showModal({ title: '无法识别', content: '这不是有效的 AI爱买买小程序码。', showCancel: false, confirmText: '我知道了' });
  ...
  return <View className='scanner-page'>
    <View className='scanner-stage'>
      <View className='scanner-frame'><View className='scanner-corner scanner-corner--a' /><View className='scanner-corner scanner-corner--b' /><View className='scanner-corner scanner-corner--c' /><View className='scanner-corner scanner-corner--d' /><View className='scanner-line' /><Text>对准 AI爱买买二维码</Text></View>
    ...
    <View className='scanner-manual aim-card'><Text>手动输入</Text><Text>可输入 8 位邀请码，或粘贴完整的 AI爱买买分享链接。</Text><View><Input value={manualValue} maxlength={512} placeholder='邀请码 / 分享链接' onInput={(event) => setManualValue(event.detail.value)} /><Button disabled={!manualValue.trim()} onClick={() => openTarget(manualValue)}>识别</Button></View></View>
