...
export default function AccountDeletionPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => AccountDeletionRepo.execute(preview!.identityVerify === 'SMS'
      ...
      : { confirmationMethod: 'WECHAT_MODAL', modalConfirmText: wechatText.trim(), acknowledgedNotice: true }),
  ...
  const valid = agreed && preview.canDelete && (preview.identityVerify === 'SMS' ? /^\d{4,8}$/.test(code.trim()) : wechatText.trim() === CONFIRM_TEXT);
