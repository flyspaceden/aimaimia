...
export default function AccountSecurityPage() {
  ...
  return <View className='aim-page security-page'>
    <View className='security-heading'><Text>登录身份</Text><Text>AI爱买买账号通用</Text></View>
    <View className='security-card aim-card'><View className={profile.phone ? 'security-row' : 'security-row security-row--action'} onClick={() => { if (!profile.phone) void Taro.navigateTo({ url: '/packages/account/account-bind-phone/index' }); }}><Text>手机号</Text><Text>{profile.phone ? maskPhone(profile.phone) : '未绑定，可选绑定 ›'}</Text></View><View className='security-row'><Text>微信</Text><Text>{profile.wechatBound ? profile.wechatNickname || '已绑定' : '未绑定'}</Text></View><View className='security-row' onClick={() => { if (!hasWechatSession) void ensureWechatMiniProgramSession('/packages/account/account-security/index'); }}><Text>当前小程序会话</Text><Text>{hasWechatSession ? '微信身份已验证' : '微信身份待重新验证 ›'}</Text></View></View>
    <View className='security-heading'><Text>{profile.hasPassword ? '修改密码' : '设置密码'}</Text><Text>修改后其他设备会退出</Text></View>
    {profile.hasPassword ? <View className='security-card aim-card'><Input className='security-input' password value={oldPassword} maxlength={128} placeholder='旧密码' onInput={(event) => setOldPassword(event.detail.value)} /><Input className='security-input' password value={newPassword} maxlength={128} placeholder='新密码（含大小写字母和数字）' onInput={(event) => setNewPassword(event.detail.value)} /><Input className='security-input' password value={confirmPassword} maxlength={128} placeholder='再次输入新密码' onInput={(event) => setConfirmPassword(event.detail.value)} /><Button className='security-primary' disabled={mutation.isPending} loading={mutation.isPending} onClick={submit}>{mutation.isPending ? '提交中...' : '确认修改密码'}</Button></View> : <View className='security-card aim-card'><Text>{profile.phone ? '首次设置密码需验证已绑定手机号，完成后会退出当前设备。' : '请先绑定手机号，才能设置密码。'}</Text>{profile.phone ? <Button className='security-primary' onClick={() => Taro.navigateTo({ url: '/packages/account/account-forgot-password/index' })}>通过手机号验证设置密码</Button> : <Button className='security-primary' onClick={() => Taro.navigateTo({ url: '/packages/account/account-bind-phone/index' })}>去绑定手机号</Button>}</View>}
