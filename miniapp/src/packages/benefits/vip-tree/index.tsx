...
export default function VipTreePage() {
  ...
  if (!loggedIn) return <View className='aim-page benefits-page'><BenefitsFeedback kind='login' description='登录后查看你在 VIP 树中的服务端节点' actionLabel='去登录' onAction={() => Taro.redirectTo({ url: benefitsLoginUrl('/packages/benefits/vip-tree/index') })} /></View>;
  ...
  if (member.tier !== 'VIP' || !tree.node) return <View className='aim-page benefits-page'><BenefitsFeedback kind='empty' title='尚未进入 VIP 树' description='购买 VIP 礼包并完成服务端建单后，节点会在这里展示' actionLabel='查看 VIP 礼包' onAction={() => Taro.redirectTo({ url: '/packages/benefits/vip-gifts/index' })} /></View>;
  ...
  return <View className='aim-page benefits-page'>
    ...
    <View className='benefits-section-head'><Text>节点结构</Text><Text>服务端返回两层</Text></View>
    ...
    {tree.children.length ? <View className='tree-branches'>{tree.children.map((child) => <View className='tree-node aim-card' key={child.id}><Text className='tree-node__mark'>L{child.level}</Text><Text className='tree-node__title'>位置 {child.position + 1}</Text><Text className='tree-node__meta'>下级 {child.childrenCount}</Text>{child.children?.length ? <Text className='tree-node__meta'>次级 {child.children.length}</Text> : null}</View>)}</View> : <BenefitsFeedback kind='empty' title='暂无直属节点' description='新节点进入后会由服务端自动更新' />}
    <View className='benefits-card aim-card'><Text className='benefits-card__title'>奖励口径</Text><Text className='benefits-card__description'>树结构只用于展示当前节点关系；实际奖励、冻结和可用状态以服务端钱包流水为准。</Text></View>
  </View>;
}
