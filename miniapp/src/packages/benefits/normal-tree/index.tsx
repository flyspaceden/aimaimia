...
export default function NormalTreePage() {
  ...
  if (!tree.inTree || !tree.node) return <View className='aim-page benefits-page'><BenefitsFeedback kind='empty' title='尚未进入普通用户树' description='资格与节点分配由服务端根据实际业务规则处理' /></View>;
  return <View className='aim-page benefits-page'>
    ...
    <View className='benefits-card aim-card'><Text className='benefits-card__title'>奖励与进度</Text><Text className='benefits-card__description'>树页不估算奖励金额；奖励入账、解锁层级与过期时间以服务端奖励流水为准。</Text></View>
  </View>;
}
