...
import { benefitsLoginUrl, formatDate, formatPercent } from '../utils';
import './index.scss';

const LINKS = [
  { mark: '礼', title: 'VIP 礼包', meta: '选档位与多商品赠品', url: '/packages/benefits/vip-gifts/index', tone: 'gold' },
  { mark: '奖', title: '每日抽奖', meta: '当日次数以服务端为准', url: '/packages/benefits/lottery/index', tone: 'red' },
  { mark: '树', title: 'VIP 分润树', meta: '查看我的节点与下级', url: '/packages/benefits/vip-tree/index', tone: 'gold' },
  { mark: '队', title: '订单队列奖励', meta: '查看位置与服务端进度', url: '/packages/benefits/queue-reward/index', tone: '' },
];

const VIP_BENEFITS = [
  { mark: '折', title: '会员结算权益', highlight: '动态计算', description: '普通商品价格、运费门槛与消费积分抵扣比例，以结算页和平台配置为准。' },
  { mark: '奖', title: 'VIP 直推奖励', highlight: '售后期释放', description: '好友成为 VIP 后进入团队，其后续普通商品订单按付款时的平台比例计算。' },
  { mark: '树', title: '专属三叉树', highlight: '独立关系', description: 'VIP 树与普通用户奖励树相互隔离，节点和下级均以服务端数据为准。' },
  { mark: '享', title: '会员身份标识', highlight: '专属展示', description: '可使用 VIP 身份与专属头像框，并进入会员礼包浏览空间。' },
];

const REWARD_STEPS = [
  ['1', '普通商品确认收货', 'VIP 礼包订单不参与奖励计算'],
  ['2', '进入售后冻结期', '退款或退货成功时按服务端规则冲回'],
...
export default function VipCenterPage() {
  ...
  return <View className='aim-page benefits-page benefits-page--gold'>
    <View className='benefits-hero benefits-hero--gold'>
      ...
      <Text className='benefits-hero__description'>{isVip ? `${formatDate(member?.vipPurchasedAt)} 加入，会员状态与 App 实时共用` : '礼包价格、赠品库存与资格由服务端最终确认'}</Text>
    ...
    {!loggedIn ? <View className='benefits-card aim-card'><Text className='benefits-card__title'>登录后查看你的会员身份</Text><Text className='benefits-card__description'>手机号或微信登录都会进入与 App 相同的账户。</Text><Button className='benefits-primary benefits-primary--gold' onClick={() => Taro.redirectTo({ url: benefitsLoginUrl('/packages/benefits/vip-center/index') })}>去登录</Button></View> : null}

    {member ? <View className='vip-relation-card aim-card'>
      <View className='vip-relation-card__mark'>{member.inviterUserId ? '友' : '?'}</View>
      <View className='vip-relation-card__copy'><Text>推荐关系</Text><Text>{member.inviterUserId ? (isVip ? `已加入 ${member.inviter?.nickname || member.inviter?.maskedPhone || '推荐人'} 的 VIP 团队` : `已绑定 ${member.inviter?.nickname || member.inviter?.maskedPhone || '推荐人'}，升级时由服务端确认团队关系`) : '尚未绑定推荐人，成为 VIP 时按系统节点分配'}</Text></View>
      {!isVip ? <Text className='vip-relation-card__action' onClick={() => Taro.navigateTo({ url: '/packages/community/scanner/index' })}>去绑定 ›</Text> : null}
    </View> : null}
    ...
    <View className='benefits-section-head'><Text>功能入口</Text><Text>与 App 共用账户和规则</Text></View>
