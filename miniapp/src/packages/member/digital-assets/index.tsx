...
export default function DigitalAssetsPage() {
  ...
  return <View className='aim-page member-page digital-page'>
    ...
    <View className='member-section-head'><Text>最近资产流水</Text><Text>最近 5 条</Text></View>
    {summary.recentRecords.length ? <View className='asset-ledger-list'>{summary.recentRecords.map((item) => <View className={`asset-ledger-card asset-ledger-card--${item.subjectType.toLowerCase()}`} key={item.id}><View className='asset-ledger-card__accent' /><View className='asset-ledger-card__main'><Text className='asset-ledger-card__title'>{digitalLedgerTitle(item)}</Text><Text className='asset-ledger-card__description'>{item.description || item.releaseHint || '资产变动记录'}</Text><Text className='asset-ledger-card__time'>{formatDateTime(item.createdAt)}</Text></View><View className='asset-ledger-card__value'><Text>{digitalLedgerAmount(item)}</Text><Text>{digitalLedgerBalance(item)}</Text></View></View>)}</View> : <MemberFeedback kind='empty' title='暂无数字资产流水' description='符合条件的消费记录会自动记入此处' />}
