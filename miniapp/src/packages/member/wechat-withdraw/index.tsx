...
export default function WechatWithdrawPage() {
  ...
  return <View className='aim-page member-page withdraw-page'>
    ...
    <View className='withdraw-form aim-card'>
      ...
      <View className='withdraw-rule'><Text>资金说明</Text><Text>{`提现将扣除 ${(taxRate * 100).toFixed(0)}% 税费；每位用户每日实际到账最多 ¥${policy.netUserDailyMax.toLocaleString('zh-CN', { maximumFractionDigits: 0 })}。`}</Text></View>
