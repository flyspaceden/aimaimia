...
export default function ConsumptionRecordsPage() {
  ...
  return <View className='aim-page member-page records-page'>
    <View className='records-intro'><Text>资产流水</Text><Text>每条记录、余额与状态均来自服务端账本</Text></View>
    ...
    {query.isLoading ? <MemberFeedback kind='loading' /> : query.isError ? <MemberFeedback kind='error' title='资产流水加载失败' onAction={() => query.refetch()} /> : records.length ? <View className='asset-ledger-list'>{records.map((item) => <View className={`asset-ledger-card asset-ledger-card--${item.subjectType.toLowerCase()}`} key={item.id}><View className='asset-ledger-card__accent' /><View className='asset-ledger-card__main'><Text className='asset-ledger-card__title'>{digitalLedgerTitle(item)}</Text><Text className='asset-ledger-card__description'>{item.description || item.releaseHint || '服务端资产记录'}</Text><Text className='asset-ledger-card__time'>{formatDateTime(item.createdAt)}</Text></View><View className='asset-ledger-card__value'><Text>{digitalLedgerAmount(item)}</Text><Text>{digitalLedgerBalance(item)}</Text></View></View>)}</View> : <MemberFeedback kind='empty' title='暂无资产流水' description='当前服务端筛选条件下没有记录' />}
    {query.hasNextPage ? <Button className='member-load-more' loading={query.isFetchingNextPage} onClick={() => query.fetchNextPage()}>{query.isFetchingNextPage ? '加载中...' : '加载更多'}</Button> : records.length ? <Text className='member-list-end'>已显示全部服务端记录</Text> : null}
  </View>;
}
