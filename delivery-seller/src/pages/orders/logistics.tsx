      ...
      <Table<Order>
        ...
        columns={[
          ...
          },
          {
            title: '物流',
            render: (_, row) => {
              const item = row.shipment ? shipmentStatusMap[row.shipment.status] : null;
              return row.shipment ? (
                <Space direction="vertical" size={0}>
                  <Tag color={item?.color}>{item?.text || row.shipment.status}</Tag>
        ...
        <Table<Shipment>
          ...
          columns={[
            ...
            { title: '运单号', dataIndex: 'trackingNo', render: (value) => value || '-' },
            {
              ...
              dataIndex: 'status',
              render: (value) => {
                const item = shipmentStatusMap[value];
                return <Tag color={item?.color}>{item?.text || value}</Tag>;
              },
            },
            {
