...
client.interceptors.request.use((config) => {
  const token = localStorage.getItem('delivery_seller_token');
  if (token) {
...
const safeNavigateToLogin = () => {
  ...
  localStorage.removeItem('nongmai-delivery-seller-auth');
...
client.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      !originalRequest.url?.includes('/delivery-seller/auth/refresh') &&
      !originalRequest.url?.includes('/delivery-seller/auth/login')
    ) {
      originalRequest._retry = true;
      const refreshTokenStr = localStorage.getItem('delivery_seller_refresh_token');
      ...
      try {
        ...
        const refreshPromise = axios.post(
          `${import.meta.env.VITE_API_BASE_URL || '/api/v1'}/delivery-seller/auth/refresh`,
        ...
        localStorage.setItem('delivery_seller_token', accessToken);
        if (newRefreshToken) {
