...
export const refreshToken = (refreshToken: string): Promise<{ accessToken: string; refreshToken: string }> =>
  client.post('/delivery-seller/auth/refresh', { refreshToken });

/** 登出 */
export const logout = (): Promise<void> =>
  client.post('/delivery-seller/auth/logout');

/** 获取当前卖家信息 */
export const getMe = (): Promise<SellerProfile> =>
  client.get('/delivery-seller/auth/me');
...
export const changePassword = (data: {
  ...
  client.post('/delivery-seller/auth/change-password', data);
