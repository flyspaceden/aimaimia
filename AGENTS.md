...
- `docs/operations/双客户端版本控制.md` — App 与微信小程序双客户端版本控制协议（干净开发 worktree、固定 staging 测试目录、源码 SHA、构建/预览/真机证据、共享后端联调与回退，**所有 App/miniapp 代码变更的强制流程**）
...
11. **App 与微信小程序双客户端版本控制（强制）**：
    ...
    - **只允许从干净、包含最新 `origin/staging` 的临时 worktree 开发**：先 `git fetch origin staging`，检查 `git status --short` 为空且 HEAD 不落后。脏根目录禁止开发、推送或导入微信开发者工具。固定目录 `/Users/jamesheden/Desktop/农脉 - AI赋能农业电商平台-staging` 是唯一例外，但仅可作为 staging 测试锚点，禁止直接编辑或推送
    - **小程序发布/真机前必须在开发 worktree 的 `miniapp/` 执行 `npm run verify:release-context`、`npm run verify`、最后一次 `npm run build:staging`**；staging CI/deploy 成功后，执行 `node scripts/sync-staging-test-checkout.mjs`，微信开发者工具固定导入 `/Users/jamesheden/Desktop/农脉 - AI赋能农业电商平台-staging/miniapp`，并记录 `SOURCE_SHA + SOURCE_PATH + 环境 + 构建时间`
