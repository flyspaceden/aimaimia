...
test('商品图片智能美化管理受权限保护并提供费率、额度和历史候选链路', () => {
  ...
  assert.match(page, /有图片积分即可使用/);
  assert.match(page, /无需单独授权/);
  assert.match(page, /无需单独授权，但模型服务尚未就绪/);
  assert.match(page, /图片积分不是唯一门槛/);
  assert.match(page, /图片积分使用状态读取失败/);
  assert.match(page, /!testAccessStatus\.isError && testAccessStatus\.data/);
  assert.doesNotMatch(page, /开通指定测试商家|只授权指定商家、人员和商品/);
  assert.match(api, /client\.get\('\/admin\/visual-agent\/test-authorizations\/status'\)/);
  assert.match(page, /营销展示图/);
  assert.doesNotMatch(api, /client\.post\('\/admin\/visual-agent\/test-authorizations'/);
