...
test('extracts upload key from OSS object URL and routes through backend download endpoint', () => {
  const request = buildUploadDownloadRequest(
    'https://bucket.oss-cn-hangzhou.aliyuncs.com/products/abc-123.webp?Expires=1&Signature=x',
    '商品图片-1',
    '/api/v1',
  );
  ...
  assert.equal(
    request.href,
    '/api/v1/upload/download?key=products%2Fabc-123.webp&filename=%E5%95%86%E5%93%81%E5%9B%BE%E7%89%87-1.webp',
  );
  assert.equal(request.filename, '商品图片-1.webp');
});
