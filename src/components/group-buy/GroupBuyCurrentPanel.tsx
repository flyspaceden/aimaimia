...
const statusCopy: Record<GroupBuyCurrentInstance['status'], {
  ...
  TERMINATED: {
    title: '本次分享已结束',
    description: '已结束后不会再接受新的推荐订单，已产生的有效订单仍按规则确认。',
    tone: 'muted',
  },
  ...
  EXPIRED: {
    title: '本次分享已过期',
    description: '活动有效期结束后，不再接受新的推荐订单。',
    tone: 'muted',
  },
};
...
export const GroupBuyCurrentPanel = ({
  ...
  const canShare = current.status === 'SHARING' && current.code?.status === 'ACTIVE' && Boolean(code);
  const canTerminate = current.status === 'SHARING';
  const canAbandon = current.status === 'QUALIFICATION_PENDING';
  ...
  const handleShare = async () => {
    if (!canShare) {
      show({ message: '推荐码生成后可分享', type: 'info' });
      return;
    }
    try {
      await Share.share({
