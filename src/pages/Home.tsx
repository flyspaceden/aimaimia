  ...
  return (
    <>
      ...
      <section className="relative min-h-screen flex items-center justify-center bg-gradient-to-b from-dark-bg via-dark-surface to-dark-elevated overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Button variant="ghost" size="lg" onClick={() => document.getElementById('values')?.scrollIntoView({ behavior: 'smooth' })}>
              了解更多 ↓
            </Button>
          </div>
        </div>
              ...
              <ScrollReveal key={item.title} delay={i * 0.15}>
                <div className="group rounded-card-lg overflow-hidden bg-dark-elevated border border-white/5 hover:border-ai-start/30 transition-all duration-300 hover:shadow-ai-glow">
                  ...
                  <div className="p-6">
                    ...
                    <h3 className="text-h3-mobile md:text-h3 text-text-on-dark mb-2">{item.title}</h3>
                    <p className="text-text-on-dark-secondary leading-relaxed">{item.desc}</p>
                  </div>
                </div>
              </ScrollReveal>
            ))}
      ...
      <section className="py-20 md:py-28 bg-gradient-to-b from-dark-surface via-[#0D1A0D] to-light-bg">
        <div className="max-w-page mx-auto px-6">
          <ScrollReveal>
            <SectionHeading title="一个平台，全链路覆盖" subtitle="买家、卖家、平台，每个角色都有专属体验" light={false} />
          </ScrollReveal>
              ...
              <ScrollReveal key={feat.title}>
                <div className={`flex flex-col ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} gap-8 md:gap-12 items-center`}>
                  <div className="flex-1">
                    <h3 className={`text-h2-mobile md:text-h2 mb-4 ${i < 2 ? 'text-text-on-dark' : 'text-text-primary'}`}>
                      {feat.title}
                    </h3>
                    <p className={`text-lg mb-6 leading-relaxed ${i < 2 ? 'text-text-on-dark-secondary' : 'text-text-secondary'}`}>
                      {feat.desc}
                    </p>
                    <div className="flex flex-wrap gap-3">
                      {feat.features.map(f => (
                        <span key={f} className="px-4 py-1.5 rounded-pill text-sm font-medium bg-brand/10 text-brand border border-brand/20">
                          {f}
                        </span>
                      ))}
                    </div>
        ...
        <div className="max-w-page mx-auto px-6 text-center">
          ...
          <div className="relative">
            <div className="flex animate-marquee gap-16 whitespace-nowrap">
              {[...Array(2)].map((_, loop) => (
                ['中国农业大学', '阿里云', '高德地图', '讯飞语音', '顺丰速运', '中国邮政', '京东物流'].map(name => (
                  <span key={`${loop}-${name}`} className="text-text-secondary/40 font-semibold text-lg md:text-xl flex-shrink-0">{name}</span>
                ))
              ))}
            </div>
          </div>
      ...
      <section className="py-20 md:py-28 bg-gradient-to-r from-brand via-ai-start to-ai-end relative overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <img src={IMAGES.agriculture.riceField} alt="" className="w-full h-full object-cover" />
        </div>
