...
- [x] **WMP33** 2026-08-17 小程序取货凭证 iPhone Canvas 根因修复：凭证 API、短码和有效期始终正常，失败只在前端 Canvas 节点查询。普通选择器取不到跨 Taro 组件的 node；`CustomWrapper + .in(scope)` 在真实微信基础库又报组件未注册并返回 `SCOPE_UNAVAILABLE`，已完整撤销。最终改用微信小程序支持的 `>>>` 跨组件选择器，并在 staging/production 构建前严格清理限定的 `miniapp/dist`，避免旧组件模板污染；分类日志不记录 QR payload 或短码，短码兜底继续保留。仍需 staging iPhone 重新读取 READY 订单后确认二维码实际显示。
