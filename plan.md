...
- [x] **WMP39** 2026-08-21 小程序购物车数量串线与跳动本地修复：根因是所有商品共用全局 mutation 状态，数量响应又以整车快照覆盖缓存并立即 refetch；现改为按 `cartItemId` 的独立串行队列、局部乐观更新、旧 GET 取消和 idle 后权威刷新，单商品快速连点可合并，不同商品乱序响应不会互相覆盖。接口失败会立即回到最后一次服务端确认数量，即使后续 GET 也失败也不会保留错误乐观值。后端新增精确行接口并保留 App/旧客户端 SKU 路由；事务内强制当前用户 cart + 普通行归属，历史普通行 partial unique index 已复核，无新增 migration。小程序 54 files/295 tests、Lint/TypeScript/双构建/72页产物，后端 319 suites/3450 tests、Nest build、Prisma validate，以及 PostgreSQL 16 双商品并发均通过。该勾选只代表本地代码完成，尚未 push/deploy；staging 开发者工具与真机快速连点复测仍是发布门槛。
