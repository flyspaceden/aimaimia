...
- [x] **WMP34** 2026-08-17 staging 后端部署卡死治理：`d42cd6fd` 部署确认停在服务器 `npm ci`，约 132 分钟无输出后 SSH Broken pipe，未进入 Prisma/构建/PM2；旧服务保持 200。相同 lockfile 本地干净安装 14 秒，新增 `qrcode` 无安装脚本，排除 lockfile/二维码包本身。部署 workflow 增加 SSH keepalive、npm 网络重试/超时、15 分钟安装上限、45 分钟 job 上限和 timing 证据，并让回滚复用同一受控安装函数；待重新部署验证。
