...
- [x] **WMP33** 2026-08-17 小程序取货凭证 iPhone Canvas 作用域修复：真机显示“二维码未能显示”但短码/有效期均正常，确认凭证 API 无误，问题在 Taro 全局 SelectorQuery 无法跨原生自定义组件取得 Canvas node。改由 `CustomWrapper` 的 `.in(scope)` 查询，失败日志仅保留分类代码、绝不记录 QR payload 或短码，原短码兜底和重试保留。小程序 lint、TypeScript、专项契约测试和 staging 构建通过；仍需 staging iPhone 重新读取 READY 订单后确认二维码实际显示。
