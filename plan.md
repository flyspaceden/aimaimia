...
- [ ] **MP-PROD05D** 小程序跨端微信账号防重复（2026-08-25 开始）：后端按 `appId + openid` / `unionId` 三态裁决；已匹配直接登录，未匹配且有 `unionId` 时允许用户明确微信建号，未匹配且无 `unionId` 时只允许手机号绑定。账号衔接页必须有清晰手机号/验证码输入框，删除无条件“作为新用户继续”。本地代码已通过后端 260 suites / 3136 tests、Auth 专项 56/56、小程序 55 files / 305 tests、lint/typecheck、staging+production 双构建与 72 页产物校验；尚未 push/deploy/上传体验版。生产误建账号 `AIMM00000000000217` 已在完整备份、强制 ROLLBACK 干跑和业务关系审计后安全标记 DELETED，小程序微信身份已归到原主账号 `AIMM00000000000001`，API health 正常。完成新体验版后仍需覆盖 App 微信既有账号、App 手机号既有账号、全新微信+新手机号、缺失 `unionId`、重复提交和修复账号重新登录真机测试。
- [ ] **MP-PROD05E** 体验版推荐码与语音搜索修复（2026-08-25）：推荐码生产失败已由日志确认微信 `41030`，正式版场景页未发布时仅对该错误从 `release + check_path=true` 回退一次 `trial + check_path=false`，同 AppID 探针已返回有效 JPEG；其他错误不降级。语音结果删除“我听到”，搜索接收页对 Taro 路由 query 安全解码一次，避免“水产”显示为 `%E6...`。后端专项 8/8、语音路由专项 11/11 已通过；仍需完整发布级验证、push/deploy和新体验版真机验收。
