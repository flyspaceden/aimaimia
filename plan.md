...
- [x] **WMP31** 2026-08-16 平台中心仓共享自提：保留商家自有自提点，新增仅平台公司可拥有的 `PLATFORM_HUB` 和“全部正常企业 / 指定企业”服务范围；小程序仅展示服务端授权的中心仓并显示明确徽记。企业员工继续只处理本企业订单；平台后台使用独立 `pickup_fulfillment:operate` 权限执行备货完成与先解析、后确认的不可逆核销。多企业选择同一中心仓仍保留逐订单凭证、退款和收货副作用。平台后台复用“自提点管理”弹窗的“平台中心仓”开关，打开后隐藏企业选择并由服务端自动绑定唯一平台公司。Prisma、Nest build、中心仓定向 30 项测试、管理端构建/契约测试、小程序 51 文件 281 项测试及双环境构建通过；后端全量 3,347 项通过，另有 4 项支付/自动收货既有失败已在未修改 `origin/staging` 复现。2026-08-17 首次 staging 部署在新增权限的 `AdminRolePermission` 漏写必填 `id` 时由 Prisma 阻止并自动回滚；修复迁移改为稳定 `rpf_ + md5(roleId:permissionId)` 主键，重新部署前仍须在 staging 受控标记该失败迁移为 rolled back。中心仓实际创建和真机验收仍未执行。
