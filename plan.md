...
- [x] **红包活动触发类型与手动发放规则整理**（2026-07-02 新增并完成）
  ...
  - **实际做了**: 将 `WIN_BACK` 管理端展示改为“久未下单唤醒”；隐藏当前未接通的签到/好评新建入口；后端校验触发类型与发放方式的合法组合，并要求累计消费填写消费门槛、久未下单唤醒填写未下单天数；`CouponCampaign.endAt` 改为可空，长期活动可设置“不限结束时间”；手动发放新建表单提示先创建草稿并上架，创建成功后自动切到草稿 Tab，上架后自动打开发放弹窗；发放弹窗支持“指定用户”和“全部用户”，后端按 ACTIVE 且有买家编号的用户发放；固定金额红包要求先填抵扣金额，最低消费门槛必须大于等于抵扣金额；节日活动/限时抢在表单内说明当前机制相同但运营场景不同；限定品类/限定店铺改为加载真实启用品类和已审核店铺的多选下拉，保存 ID，不选表示不限。
