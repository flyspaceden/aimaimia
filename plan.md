...
- [x] **MP-PROD03**（Draft PR #1 已推送，最新语义收口尚未 push/未合并 main/未部署/未迁移生产库）小程序后端兼容闭包与 19 条 migration（18 条 staging-derived 同哈希 + 1 条生产集成自动退款副作用 outbox）；原 main 路由零删除，小程序新增路由为并列接口；微信支付/退款/提现、自提、通知、购物车、六类收货副作用 outbox 与自动退款数字资产/旧团长佣金冲回 outbox 已接入。Prisma validate、Nest build、后端 260 suites / 3133 tests 通过；真实 PostgreSQL 120 migrations + 5 suites / 9 tests 通过。生产备份克隆库演练仍由 MP-PROD06 单独阻断。
...
- [x] **MP-PROD09C** staging 已验收业务语义收口：42 个核心商城/后台 Service 或 DTO 的 Git blob 精确固定到 `staging@acc0e08c`；Git 自动枚举的 20 个非 Delivery 运行时差异全部形成生产强化、Delivery 排除或格式/兼容说明，manifest blob 反向验证来源 commit，防止伪造。恢复 staging 的地址、购物车、预约/参团、客服、商品/推荐、个人资料、团购后台、组合商品、卖家发货、微信提现渠道查询、注销账号资金围栏、团购返还、分润/推荐、订单支付退款及任务奖励 fail-closed 测试。验证为后端 260/3133、真实 PG 5 suites/9、App 30/127+TS、Admin 12+build、Seller 12+build、miniapp 303+双构建、scripts 256；App/根 src/Delivery 改动 0。
