...
- [x] **三后台商品字段跨品类中性化**（2026-08-21）
  ...
  - **实际做了**: 卖家中心、配送卖家中心、平台管理后台统一使用“标签 / 月份（选填）/ 场景 / 属性 / 来源地与生产地”等中性名称；商品描述、自定义属性、企业简介和溯源模板示例同步移除种植、养殖、口感等单一行业指向。
...
- [x] **企业详情检测报告受控预览**（2026-08-11）
  ...
  - **实际做了**: 买家 App 和小程序不再使用公开接口的原始 `fileUrl`；报告动作改为“预览”，仅打开由报告 ID 构造的受控平台预览地址。后端只允许当前有效、已验证且属于活跃非平台企业的 `documents/` 存储 PDF/图片，以 `inline` 响应返回；不支持预览的历史文件明确禁用。原始存储 URL 不再下发给公开客户端；旧 App 的同名字段只在配置 `PUBLIC_API_BASE_URL` 时返回受控预览 URL，以避免 OTA 前回归；对象丢失、流错误和报告失效会得到安全的 404 或用户可读预览页。
  - **限制**: Expo 系统浏览器会优先内联预览，但 Android/iOS 最终是否提供保存功能仍由系统浏览器决定；“绝不允许下载”需要后续原生 PDF/图片阅读器和真机验收，不应由本次浏览器方案声称保证。
  - **验证**: App/小程序/后端聚焦回归测试、App TypeScript 编译、Prisma schema 校验；后端完整 build 如仍存在本改动无关的既有错误，须在发布日志单列。
...
- [x] **R-RS-LF04** 恢复 E2E tests TypeScript 覆盖（2026-08-21 收口）—— 将卖家权限 E2E 登录辅助函数的无效 `test.request` 条件类型替换为 Playwright 官方 `APIRequestContext`；`cd tests && npx tsc --noEmit -p tsconfig.json` 已通过。CI 继续由 Playwright 运行门禁执行，后续可再将 tests project reference 恢复到根 `tsc -b`。
...
- [ ] **WMP37** 2026-08-20 账号安全：未知微信身份可选择新建或合并既有手机号账号；首次设置密码改走短信验证；无手机号账号注销改为重新微信验证的一次性证明；已删除无实际管理能力的微信授权说明入口。影响：App=否、miniapp=是、backend=是；本地验证中，staging/真机未验收。
...
- [x] **WMP21** 2026-08-13 微信小程序微信提现额度收口：按已获批“佣金报酬”(1005) 场景，将实际转账单笔 ¥0.10–¥200、同一用户中国自然日实际到账 ¥2,000、商户中国自然日实际到账 ¥50,000 固化为后端资金规则；继续复用 App 的 20% 扣缴配置，提现页从只读 policy 取得预估。在 Serializable 事务、余额冻结前用用户锁和平台日 advisory lock 裁决；1005 报备字段、用户感知或税费配置不合法时 fail-closed。
- [x] **WMP26** 2026-08-13 微信小程序提现全链路复审与中断恢复：修复用户关闭确认页后重复冻结/建单、关闭新建开关同时切断存量查单/回调、微信流水误标支付宝三类问题。同一用户只允许一笔微信处理中提现；历史页只恢复同一微信原单；存量单据在关闭新建后仍可经回调或主动查单收口。后端聚焦回归 60/60、Nest build 与 Prisma validate，小程序 ESLint/TypeScript/48 文件 262/262/生产构建和 71 页产物断言、管理端生产构建均通过；真实零钱出款仍须 staging 真机小额验收。
- [x] **WMP27** 2026-08-14 微信小程序提现资金说明精简：仅显示当前 20% 税费和每位用户每日实际到账上限 ¥2,000，数值继续来自后端只读 policy，不在小程序硬编码资金规则。
- [x] **WMP28** 2026-08-15 App 与微信小程序双客户端版本控制：新增受控 worktree、源码 SHA、构建产物、开发者工具路径、预览/真机二维码、共享后端部署和回退的强制协议；`miniapp` 新增 `npm run verify:release-context`，拒绝脏工作区及落后/分叉的 `origin/staging`。开发者工具重复补充的共享 `project.config.json` 默认值已纳入版本控制，个人配置继续留在 private 文件。后续每次 App/小程序或共享契约改动必须在任务记录中声明受影响系统并保留验证证据，推送仍须用户明确授权。

- [x] **WMP29** 2026-08-15 自提凭证显示与卖家扫码核销台：小程序取货凭证 Canvas 绘制改为可判定成功/失败的 node API；失败时明确展示 8 位取货码与重试，不留下空白二维码。卖家新增 `/pickup-verify` 核销台，OWNER / MANAGER / OPERATOR 可通过 USB/蓝牙二维扫码枪、电脑摄像头或人工短码先解析订单、展示脱敏取货人和商品摘要，再显式确认核销；后端全局凭证解析仍会校验商户归属和 READY/PAID 状态，正式状态变更复用原有 Serializable/CAS 核销链。商品/SKU 条码仅作可选“拿对货”核对，不得替代买家一次性凭证。已补单元/合约与构建验证；待 staging 真实二维码、扫码枪/摄像头与短码设备验收后才能标记真机完成。
- [x] **WMP30** 2026-08-16 固定微信开发者工具测试目录：保留 `/Users/jamesheden/Desktop/农脉 - AI赋能农业电商平台-staging` 作为唯一 `staging` 测试锚点；开发仍在临时 worktree，推送和部署成功后才通过 `scripts/sync-staging-test-checkout.mjs` 对固定目录做快进同步、锁文件依赖重建和 staging 构建。首次整理已将该目录的 15 项本地改动保存在本地恢复分支 `codex/recover-staging-local-20260816` 的 `752652ee`，固定目录已同步到 `e60c41f9`。固定目录的 `dist` 只能是 staging 包，不得在其上运行生产产物断言；禁止在固定目录开发、推送或累积本地改动。
- [x] **WMP31** 2026-08-16 平台中心仓共享自提：保留商家自有自提点，新增仅平台公司可拥有的 `PLATFORM_HUB` 和“全部正常企业 / 指定企业”服务范围；小程序仅展示服务端授权的中心仓并显示明确徽记。企业员工继续只处理本企业订单；平台后台使用独立 `pickup_fulfillment:operate` 权限执行备货完成与先解析、后确认的不可逆核销。多企业选择同一中心仓仍保留逐订单凭证、退款和收货副作用。平台后台复用“自提点管理”弹窗的“平台中心仓”开关，打开后隐藏企业选择并由服务端自动绑定唯一平台公司。Prisma、Nest build、中心仓定向 30 项测试、管理端构建/契约测试、小程序 51 文件 281 项测试及双环境构建通过；后端全量 3,347 项通过，另有 4 项支付/自动收货既有失败已在未修改 `origin/staging` 复现。2026-08-17 首次 staging 部署在新增权限的 `AdminRolePermission` 漏写必填 `id` 时由 Prisma 阻止并自动回滚；已受控标记失败记录并重新部署，migration 成功应用，临时恢复逻辑已移除。中心仓实际创建和真机验收仍未执行。
- [x] **WMP32** 2026-08-17 卖家自提工作队列收口：自提订单标签固定查询 `PICKUP + PAID`，只保留备货中和待自提；成功核销后的 `PICKED_UP + RECEIVED` 订单从该队列移除，仅出现在已完成。补充静态回归测试，防止完成订单再次在两个标签重复展示。
- [x] **WMP33** 2026-08-17 小程序取货凭证二维码根因收口：凭证 API、短码和有效期始终正常，连续真机证据确认失败只在当前 Taro/微信组合的 Canvas node 查询；普通 SelectorQuery、`CustomWrapper + .in(scope)` 和 `>>>` 均未通过真实设备。最终移除小程序 Canvas 和客户端 `qrcode`，改由后端将同一短时签名 payload 生成标准 PNG/Base64，小程序按推荐码的安全模式写入私有临时文件并用 `Image` 展示；图片失败仍保留 8 位短码。release 构建前继续严格清理限定的 `miniapp/dist`。后端图片/审计测试、小程序文件校验和双环境构建完成后，仍需 staging iPhone 读取 READY 订单完成最终验收。
...
- [x] **WMP38** 2026-08-21 首次 staging 上线门禁返馈收口：首轮在连接服务器前正确拦截了测试库命名、Poppler 依赖和配送 Prisma 产物路径问题；修复后第二轮又暴露并修复真实退款 `P2034`、test-only 验证码隔离、PDF `ToUnicode`、旧运费 E2E 字段与种子数据缺少企业/退款快照。本机临时 PostgreSQL 16 复刻 GitHub 门禁：双库 migration/build、319 suites/3442 tests 全绿；完整 Web E2E 首跑 51 通过/23 按前置跳过/4 失败，四条修复后聚焦复验 7 通过/1 显式跳过。该项仍不代表 staging 服务器部署成功，须等待推送后 GitHub 全量门禁、迁移、PM2 与公网 readiness 回执。
- [x] **WMP39** 2026-08-21 小程序购物车数量串线与跳动修复：根因是所有商品共用全局 mutation 状态，数量响应又以整车快照覆盖缓存并立即 refetch；现改为按 `cartItemId` 的独立串行队列、局部乐观更新、旧 GET 取消和 idle 后权威刷新，单商品快速连点可合并，不同商品乱序响应不会互相覆盖。接口失败会立即回到最后一次服务端确认数量，即使后续 GET 也失败也不会保留错误乐观值。后端新增精确行接口并保留 App/旧客户端 SKU 路由；事务内强制当前用户 cart + 普通行归属，历史普通行 partial unique index 已复核，无新增 migration。首个 staging/真机预览确认快速五连点、跨商品交错和前后台切换数量正确，但又暴露整车刷新卡片换位与全选圆钮偏下；跟进增加 `createdAt + id` 固定排序，并以高优先级 header selector 清除通用顶部 margin。最终 `origin/staging=4ff68e6e`，小程序 workflow `32505097925`、部署 workflow `32505097909` 全绿（323 suites/3458 tests，E2E 55 passed/23 skipped），公网主库/配送库/Redis readiness 全部 up；第二次开发者工具与 iPhone 预览确认卡片顺序、全选对齐、快速点击和前后台返回均正常，测试购物车已清理。
...
- [x] **WMP24** 2026-08-13 推荐中心 VIP 身份竞态修复：会员身份确认成 VIP 后，不再读取或重试 React Query 缓存中的普通分享资料历史错误；普通用户的普通分享资料错误仍正常展示。新增回归测试覆盖 VIP 忽略旧错误与普通用户保留当前错误，ESLint、TypeScript、48 文件 259/259 测试通过；staging/production 构建通过，生产产物断言单独使用 production 包验证。此项只修查询状态与重试分支，不改变推荐关系、分享码或奖励业务规则。
...
- [x] **WMP00-D** 账号合并与小程序支付后端安全收口、独立复审和集成测试。
- [x] **WMP00-E** 微信商家转账 Provider、查单/回调/幂等/失败退回，以及提现确认参数返回。
- [ ] **WMP00-F** 外部平台与真机验收（尚未全部完成）：
...
- [x] **WMP04** Phase 4 代码与自动化收口：71 个小程序运行时页面全部注册，任务页和历史 AI 页按 App 当前隐藏状态不进入发布包；配送与支付宝静态排除；账号异步结果防串号、支付/提现/发货上报的幂等与并发路径完成复审；管理端可查看/重试微信发货上报；小程序 ESLint 0 警告、37 套件 210/210、staging/production 双构建及生产产物检查通过，production CI 同步执行产物断言；合并 `origin/staging` 后后端 302 套件 3234/3234（另 2 套件/5 个仓库既有用例跳过）、根 App 39 套件 152/152 与 208/208 源码合规测试、配送管理端 25/25 和配送卖家端 30/30 合同测试、Nest/两个配送 Web 生产构建及主库/配送库 Prisma 校验通过。可独立升级的 `nanoid` 已更新至 3.3.18，剩余 14 条 Taro/构建链上游公告按 WMPF08 跟踪：生产包无公告涉及的 Swiper JS、开发服务器不暴露公网，等待兼容升级且不使用破坏依赖的 `--force`。真实 AppID 已配置；微信开发者工具、真实资金、平台权限和真机回归继续由 WMP00-F 阻塞，不能视为已发布。
- [x] **WMP05** 逐页对标复核：以 `miniapp/src/features/page-parity.ts` 分类当前 App 全部路由并由测试阻止漏页；首页、商品、我的按 App 当前模块和海鲜视觉重做；补齐绑定手机、企业结构化搜索、近期搜索、AI 语义推荐、独立地址/红包选择；普通、团购、VIP 结算统一选址；VIP 已购用户礼包页改为只读浏览和分享；App 当前隐藏的任务、AI 历史页和考察团不在小程序恢复。代码级逐页核对完成，微信开发者工具截图、真机大字体/安全区/支付/授权验收归 WMP00-F。
- [x] **WMP06** 微信官方规则、全链路和资金安全全面复审：逐项核对微信/Taro 请求、分享、隐私、扫码、录音、地图、支付、商家转账、订阅、Socket、分包和最终产物；修复 `PATCH`、隐私按钮、真实 AbortController 打包、分享开关、扫码 path、订单/售后错误 Contract、退出/刷新竞态、注销后资产写入屏障、日志/钱包内部字段泄露及退款通知字段绑定/TOCTOU；生产产物 71 页、主包 1.235 MiB、总包 2.293 MiB。完整报告见 `docs/issues/wechat-miniapp-full-audit-2026-08-04.md`。
  - **2026-08-08 第三轮资金复核**：确认现用微信 SDK 默认传输层会丢弃 APIv3 应答签名头，已替换为原始 body 公钥验签 transport；关单仅签名 204/明确终态可释放；提现补偿改为到期公平调度并让人工/Cron CAS 抢占；交易发货发送前重建当前订单/包裹快照，拒绝退款/取消旧快照；买家无 `sessionId` 历史 JWT 不再借用其他设备会话。8 个聚焦套件 262/262 通过，最终全量验证和合并后回归仍按 WMP04 记录。
- [x] **WMP07** 2026-08-09 第四轮上线前审查：再次核验全部 App 路由映射、71 个运行时页面、静态后端 Contract、微信/Taro API 调用与资金状态机；修复企业真实资质显示、商品卡片快捷加购、结算中新建地址回填和首页录音早松手竞态。支付回调不再依赖 SDK 不完整的 GCM 解密，必须执行 auth tag 认证；支付凭据格式和 HTTPS 回调地址在启用前校验，生产错误配置 fail-closed，取消固定生产回调默认值。小程序 `npm run verify` 为 39 文件 218 测试、双环境构建和产物检查通过；后端支付聚焦 128 测试、全量 302 套件/3241 项（2/5 既有跳过）、Nest build 与双 Prisma schema 校验通过。真实小额支付/退款/提现/发货、商户密钥匹配和真机回归继续由 WMP00-F 阻塞。
- [x] **WMP08** 2026-08-10 微信开发者工具首轮显示修复：首页 WXSS 移除不兼容的 `:not()`；原生 TabBar 补齐与 App 一致的河豚/海星/牡蛎图标并加入生产产物断言。开发者工具本地合法域名校验可临时关闭供 staging 联调，微信后台 request/upload/download/socket 白名单仍归 WMP00-F 外部门槛。
- [x] **WMP09** 2026-08-10 App 实页面二次对标：以 App 源码和用户截图重新审核商品卡、首页 VIP 轮播、VIP 礼包、购物车、结算、订单/支付成功、售后、钱包和推荐关系；补齐企业/标签/销量/限购/低库存、连续无缝礼包轮播、App 风格 VIP 会员空间、奖品本地勾选、个性推荐、运费承担方/退款摘要等字段，并清理主要页面的工程化文案。自动化负责防止缺页、Contract 和构建回归；微信开发者工具/真机逐页截图验收仍属 WMP00-F，不用单测冒充视觉已验收。
...
- [x] **WMP14** 2026-08-10 微信登录页视觉精简：删除“一个微信身份”“无需填写手机号”两块说明卡、英文眉题和底部辅助文案，仅保留品牌光球、微信登录标题、协议确认与单一微信登录按钮；认证和自动建号逻辑不变。
- [x] **WMP15** 2026-08-10 商品详情购买栏视觉收口：购买数量移入规格卡，固定底栏只保留带真实小推车图标/数量角标的购物车入口、单行描边“加入购物车”和 App 金色主操作“立即购买”；移除“购”字占位和连体胶囊，窄屏不再断字换行。
- [x] **WMP16** 2026-08-10 全局显示合理性复审：从 71 路由、公共组件、固定操作栏、空/登录状态、工程化文案、字号触控和安全区七个维度复查；新增购物车/麦克风/搜索/钱包/VIP 共用纯 WXSS 图标，统一购物车/账户/会员/售后/发票海鲜插画状态，清理消费端“后端/服务端/JSAPI/轮询”等术语；第二轮继续覆盖商品卡、购物车、结算、订单/物流、售后、钱包提现、VIP、推荐、消息、客服、账户与设置，正文/说明统一提升到 11～12px、主要操作提升到 40～48px，小字号仅保留角标和装饰信息，并补普通滚动页底部安全区；新增 UI 回归守卫。小程序 ESLint 0 警告、TypeScript 通过、47 个测试文件 245/245、staging/production 双构建及 71 页生产产物检查通过（主包 1.255 MiB、总包 2.318 MiB）；同步 staging 后微信运行时 21 个公开页 PASS、50 个认证门禁、0 FAIL。登录后的订单/资金/售后真实数据截图仍归 WMP00-F 真机验收，不以未登录门禁替代。

## 小程序配送 / 到店自提履约（2026-08-14）

- [x] 按 `docs/superpowers/specs/2026-08-14-miniapp-pickup-fulfillment-design.md` 完成本地实现：普通商品、团购和 VIP 礼包配送 / 自提双履约；自提点、支付后备货、一次性凭证核销、小程序/App/卖家/管理三端展示、受控退款、售后与顺丰隔离。
- [x] 在最新 `origin/staging` 的干净 worktree 完成 Prisma 空库迁移、后端关键测试、小程序/App/Web 类型与构建、卖家/管理 Mock 浏览器检查和微信开发者工具目标路由巡检。
- [x] 提交 `9eb5d174` 并部署到 staging：主库迁移已应用，管理端/卖家端/后端 CI 成功，PM2 与三个 HTTPS 入口验活；配置独立自提凭证密钥并开启 staging 灰度开关。
- [x] 根据 2026-08-14 微信开发者工具截图修复普通结算首屏：无地址也会安全请求基础预结算并展示商品/金额，配送/自提预览缓存相互隔离，少量卡片不再拉伸；无自提点按钮点击后会明确说明原因；订单备注固定为 56px 紧凑高度。小程序 51 个测试文件 / 276 个用例、ESLint、TypeScript、staging/production 双构建和 72 页产物检查通过；当前测试账号所选商家的真实自提点数仍为 0，需在卖家端创建点位后继续自提选择实测。
- [x] 管理后台自提点跨企业完整管理前端已本地实现：新增 `pickup_points:read/create/update/delete` 四权限门控、企业选择新建、企业归属锁定、完整编辑与启停、正常/已删除筛选、删除原因加二次确认，以及仅恢复且恢复后保持停用的软删除流程；同步展示删除管理员、时间和原因。该项仅表示本地代码与静态验证完成，不代表 staging 权限种子、接口联调或发布完成。
- [ ] 在有效测试账号、自提点和订单夹具下完成真实 PostgreSQL 并发、真实微信支付/退款、卖家备货、订阅提醒、真机扫码核销和关联分润/数字资产/售后窗口验收；真实资金与 main 发布须另获授权。
...
- [x] 生产发布与数据修复完成：定价修复窄范围合入 `main` 至 `f52b82a4`，GitHub Actions run `32057943962` 的 admin / seller / backend 全部成功；服务器 SHA 复验一致。部署后 dry-run 仍为 `MARKUP_RATE=1.35`、17 个商品 / 27 个 SKU、原 preview token；在利润安全 lock + Serializable 事务内更新后，写后扫描剩余不一致为 0。生产公开 API 验证帝王蟹 ¥759.91、399 龙虾套装 ¥414.45，API health=200；线上管理端/卖家端价格预览与确认文案已验证。执行回执保存为 `pricing-artifacts/production-reprice-execute-20260817.jsonl`。
