...
- [x] **WMP22** 2026-08-12 VIP 礼包真实支付联调热修：微信实查确认遗留会话为签名有效的 `NOTPAY`，真实应答省略 `trade_type`；后端改为先校验商户号、AppID、商户订单号和金额，再允许非成功态缺少交易类型以安全关单/释放，`SUCCESS` 仍强制交易流水号和 `JSAPI/APP` 类型。VIP 结算统一解析有效显式地址、默认地址或第一条地址，展示、按钮和提交不再分叉；代码与聚焦回归已完成，staging 部署和 iOS/Android 真机复测记录完成后方可视为本轮外部验收结束。
