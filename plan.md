...
- [ ] **MP-PROD05B** 推荐 H5 双入口（2026-08-24 Website 已正式发布，App OTA/真实推荐验收待办）：复用旧 staging 已验证的 `InviteChoiceLanding`，基于最新 main 只移植 website 路由/页面、客户端交接语义、测试与文档；保留旧后端 Auth 路由作兼容但新页面不调用。App 旧 H5 登录漏斗源码已进入 main，但本次 Website-only run 未发布 App OTA。正式站 marker 已验证为 `main@98511bab`；待完成真实推荐码全链路和后续 App OTA 后才能勾选。
- [ ] **MP-PROD05C** App 自提只读兼容 OTA（2026-08-24 用户已批准，最新 main-based 候选实施中）：同账号可在 App 看到小程序创建的自提订单，按“备货中 / 待自提 / 已取货”展示自提点、营业时间、自提人和须知；自提单禁止出现物流、修改收货地址和买家确认收货。当前批次不在 App 展示二维码，明确引导到微信小程序查看凭证；完成最新 main 差异审查、App 全量回归、Android/iOS production export、PR checks、production OTA 和真机验收后才能勾选。
