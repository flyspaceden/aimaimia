...
- [ ] **H5-INV07**（2026-08-24 独立候选已完成，待生产发布验收）无登录双入口：`/invite/{code}` 不收集手机号/验证码、不执行 H5 绑定。手机端展示“打开爱买买小程序”和“下载爱买买 App”两张按钮；桌面端直接展示 App 推荐交接二维码，小程序二维码由用户点击后按需生成，禁止页面加载自动消耗 URL Link 配额。小程序 URL Link 绑定同一落地会话并限制为 `wxaurl.cn`；App 下载前保存普通/VIP 对应推荐交接口令；App 推荐中心移除旧 H5 登录漏斗。tree-identical 代码已进入 `staging-next` 并通过 CI/E2E/本地响应式验收；因 Website 测试/生产共用目录，用户决定不建立测试站，改由 main Website-only 正式发布后完成真实微信验收。
...
- [ ] **MP-PROD05B** 推荐 H5 双入口（2026-08-24 已单独批准，待生产发布验收）：复用旧 staging 已验证的 `InviteChoiceLanding`，基于最新 main 只移植 website 路由/页面、客户端交接语义、测试与文档；保留旧后端 Auth 路由作兼容但新页面不调用。App 旧 H5 登录漏斗源码同步下线，但不随 website 部署冒充已发布 OTA。tree-identical 候选已进入 `staging-next` 并通过 required checks；用户明确不建立测试 Website，后续以 main Website-only 发布正式页面。
