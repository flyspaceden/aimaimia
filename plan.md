...
- [x] **团购精选货架与详情页**（2026-06-22 新增并完成）
  ...
  - **实际做了**: 新增 `/group-buy` 货架页和 `/group-buy/[activityId]` 详情页；无当前团购时展示商品货架，有当前团购时默认进入“我的团购”并可切换到商品；当前团购面板展示推荐码、二维码、进度和结束入口；商品卡、详情和进度不展示返还基数或返还百分比；购买新团购但已有进行中分享时，用底部抽屉提示需先结束本次分享。
  - **验证**: `npx tsc -b` 通过。

- [x] **团购扫码落地与现金付款**（2026-06-22 新增并完成）
  - **来源**: 团购分享回馈功能 Chunk 6.3，需要把推荐码扫码购买和团购现金付款链路串起来。
  - **实际做了**: 新增 `/gb/[code]` 推荐码落地页和 `/group-buy/checkout` 团购付款页；落地页展示分享用户和同款商品，未登录先弹登录再回到付款；付款页复用地址选择和现有支付宝/微信支付确认链路，但不展示消费积分、平台红包、团购返还余额等抵扣入口。
  - **验证**: `npx tsc -b` 通过。
