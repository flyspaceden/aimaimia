...
- [x] **MP-PROD03**（Draft PR #1 已推送，最新语义收口尚未 push/未合并 main/未部署/未迁移生产库）小程序后端兼容闭包与 19 条 migration（18 条 staging-derived 同哈希 + 1 条生产集成自动退款副作用 outbox）；原 main 路由零删除，小程序新增路由为并列接口；微信支付/退款/提现、自提、通知、购物车、六类收货副作用 outbox 与自动退款数字资产/旧团长佣金冲回 outbox 已接入。Prisma validate、Nest build、后端 260 suites / 3125 tests 通过；真实 PostgreSQL 120 migrations + 4 suites / 8 tests 通过。生产备份克隆库演练仍由 MP-PROD06 单独阻断。
...
- [x] **MP-PROD06L** 本地 PostgreSQL 18 空库与并发门禁：120 条主库 migration 全量 deploy 通过；售后退款、购物车数量、普通树、利润安全真实数据库 4 suites / 8 tests 全通过；临时库已删除。该结果不替代 MP-PROD06 的生产历史数据副本演练。
...
- [x] **MP-PROD09C** staging 已验收业务语义收口：34 个核心商城 Service 的 Git blob 精确固定到 `staging@acc0e08c`；Payment/Company/SF/Auth/Schema 9 个允许差异均形成生产强化说明和 Delivery 排除断言。恢复 staging 的地址、购物车、预约/参团、客服、商品/推荐、个人资料、微信提现渠道查询、注销账号资金围栏、团购返还、分润/推荐及订单支付退款测试。验证为后端 260/3125、真实 PG 4 suites/8、App 30/127+TS、Admin 12+build、Seller 12+build、miniapp 303+双构建、scripts 256；App/根 src/Delivery 改动 0。
