...
- [x] **三后台商品字段跨品类中性化**（2026-08-21）
  - **来源**: 用户在卖家中心编辑智能手环时发现“口味标签 / 饮食属性”等名称仍限定食品和海鲜品类，并要求逐页检查同类问题。
  - **实际做了**: 卖家中心和平台管理后台统一使用“标签 / 月份（选填）/ 场景 / 属性 / 来源地与生产地”等中性名称；商品描述、自定义属性、企业简介和溯源模板示例同步移除种植、养殖、口感等单一行业指向。staging 另同步其独有的配送卖家中心；main 当前不包含该项目，未为文案改动引入整套目录。
  - **边界**: 仅修改页面展示文案；保留 `flavorTags / seasonalMonths / usageScenarios / dietaryTags / originText` 字段、API、数据库和搜索逻辑不变，不迁移既有数据。
  - **验证**: 三后台生产构建、跨页面文案静态回归和 `git diff --check` 通过后，按独立提交依次发布 staging 与 main。

- [x] **管理后台企业安全删除与生产标签修复**（2026-08-20）
  - **来源**: 用户截图反馈企业管理无删除能力，企业详情的“企业标签”下拉框无法填写；用户进一步确认 staging 标签管理有企业/商品标签，正式环境只有空占位类别。
  - **实际做了**: 新增 `DELETED` 软删除状态、删除前业务阻塞检查（结算会话只统计仍在有效期内的 `ACTIVE` 会话）、企业名称二次确认、独立 `companies:delete` 权限、回收站恢复与卖家会话失效；企业编辑/审核/创始人绑定/资质与员工资料写入统一在 Serializable 事务内复查未删除状态，避免并发形成半删除数据；企业审核 API 将 `APPROVED/REJECTED` 动作映射到真实 `ACTIVE/BANNED` 生命周期枚举。买家公开企业/商品/购物车/关注/预约/考察团/推荐/团购/结算统一拦截非 `ACTIVE` 企业。新迁移和 production bootstrap 幂等补齐 5 类企业标签 + 1 类商品标签及 `tags:read/manage` 权限，仅清理无标签的空占位类别，不运行生产禁用的 demo seed。
  - **安全边界**: 平台企业不可删除；待支付、待履约、售后、退款、物流、团购、预约/考察团、未结束活动、冻结产业基金存在时拒绝删除；删除与恢复使用 Serializable + CAS，不删交易历史。
  - **验证**: Prisma validate/generate、后端/管理端生产构建、App TypeScript、企业/商品/购物车/团购聚焦测试、Checkout 回归和静态合同测试通过。

- [x] **管理后台用户管理支持查看已注销账户**（2026-08-13）
  - **来源**: 用户反馈管理后台“用户管理”默认只能筛选正常/封禁，无法检索已注销账户。
  - **实际做了**: 在状态筛选中补齐“已注销”；已注销账户沿用原有用户表格和详情页，可按买家编号、昵称、注册时间及状态核查，且保持没有封禁/解封操作的安全边界。
  - **隐私边界**: 不恢复账号注销时已销毁的手机号、微信身份标识、头像及原昵称；页面只显示法定保留及当前仍存在的账户/订单审计资料。
  - **验证**: 管理后台静态回归、TypeScript 生产构建及差异检查通过。

- [x] **底部导航“产品”改为“商品”**（2026-07-28）
  - **来源**: 用户确认中间 Tab 相比“产品”更适合使用“商品”。
  - **实际做了**: 将 `museum` Tab 的展示标题和无障碍标签由“产品”统一调整为“商品”；保留路由、海星图标、商品与企业浏览功能，以及页内“发现”标题。
  - **响应式与无障碍**: 新旧文案均为两字，沿用现有 Tab 单行布局；辅助说明同步为“商品，浏览商品与企业”。
  - **安全边界**: 仅改买家 App 导航展示与无障碍文案，不涉及商品、搜索、购物车、权限或状态逻辑。
  - **验证**: 底部 Tab 静态回归、TypeScript 编译、Expo Web 导出与差异检查通过。

- [x] **底部导航“发现”改为“产品”**（2026-07-28）
  - **来源**: 用户真机截图要求将底部中间 Tab 的“发现”改为“产品”。
  - **实际做了**: 将 `museum` Tab 的展示标题和无障碍标签统一改为“产品”；保留路由、海星图标、商品与企业浏览功能，以及页内“发现”标题，避免改变既有导航行为。
  - **响应式与无障碍**: 新文案仍为两字，沿用现有 Tab 单行布局；辅助说明同步为“产品，浏览商品与企业”。
  - **安全边界**: 仅改买家 App 导航展示与无障碍文案，不涉及商品、搜索、购物车、权限或状态逻辑。
  - **验证**: 底部 Tab 静态回归、TypeScript 编译、Expo Web 导出与差异检查通过。

- [x] **首页购物车上移与搜索栏整行化**（2026-07-24）
  - **来源**: 用户真机截图希望将搜索框右侧的圆形购物车移到首页顶部两行品牌标语右侧。
...
- [x] **首页 VIP 邀请栏与搜索栏海洋化优化**（2026-07-24）
  ...
  - **实际做了**: VIP 邀请栏使用独立珍珠小贝角色、浅金圆形光晕、薄荷白背景和双层文案，保留原「去分享」动作与 `/me/referral` 跳转；搜索栏使用独立河豚角色替换放大镜，以细分隔线连接搜索文案。麦克风和购物车继续保留原语义图标，避免功能识别下降。
  - **响应式与无障碍**: 邀请标题和 CTA 使用单行自适应并将系统字体倍率封顶 1.1，搜索提示同样限制最大倍率；海鲜图标使用固定透明容器，360pt 窄屏不会通过偏移裁切或挤出相邻区域。Pressable 原有搜索、分享、购物车辅助说明保持不变。
  - **验证**: 首页顺序/VIP 文案/海鲜图标静态回归 11/11；`npm exec tsc -- --noEmit --pretty false`；`git diff --check`。

- [x] **首页品牌标语恢复与 AI 海鲜跨设备一致性**（2026-07-24）
  - **来源**: 用户对比小米与华为真机截图：小米上的龙虾/帝王蟹大小和位置正确，华为因系统显示大小或逻辑宽度进入旧紧凑分支后，两个角色缩小并被推到舞台两端；同时要求恢复「消费者就是生产力，是社会价值的创造者」。
  - **实际做了**: 首页顶部恢复固定两行「消费者就是生产力 / 是社会价值的创造者」，不重复购物车入口；新增 `getHomeAiStageLayout`，把龙虾、AI 光球、帝王蟹锁定为最大 360pt 的居中视觉组合，删除紧凑模式负横向边距和 78/80pt 角色尺寸。
  - **响应式修复**: 装饰性主视觉只读取窗口逻辑宽度，不再由 `fontScale` 触发缩小；360pt 华为与 393pt 小米都使用 large 光球并保持接近的角色重叠比例，480pt 大屏封顶 360pt，只有 <340pt 真窄屏切换 medium 光球和 96/104pt 角色。
  - **验证**: 首页/我的页目标静态回归 18/18；`homeAiStage` 与 `homeHero` Jest 2 suites / 5 tests；`npm exec tsc -- --noEmit --pretty false`；`git diff --check`。

- [x] **首页/我的页海鲜角色视觉重排**（2026-07-24）
  - **来源**: 用户基于首页和“我的”页真机截图确认新布局，并要求在全新的干净工作区实施：删除首页品牌使命文案；搜索移动到原精选团购位；VIP 礼包轮播回到首页；团购和抽奖迁入“我的”；首页 AI 入口只保留一个；首页与“我的”增加卡通龙虾、帝王蟹、小贝、小鱼等海鲜角色。
  - **实际做了**: 首页固定为“身份卡 → 亮色 VIP 推荐条 → 搜索/购物车 → 单一亮色 AI 买买 → VIP 礼包轮播”，仅保留最终确认的龙虾和帝王蟹并在紧凑屏使用 136pt 中号光球防重叠；“我的”页新增团购/抽奖活动中心，订单状态改为五个独立海鲜角色，财库和 VIP 改为上下全宽卡并加入两只独立小贝装饰，常用工具使用 12 个不重复的透明海鲜角色且取消可见外框；首页/发现/我的 Tab 改为河豚/海星/小贝。所有角色均从独立透明 PNG 加载，不再通过合集图偏移裁切。
  - **响应式修复（初版，已由上方跨设备锁定方案替代）**: 320px 窄屏下订单入口用固定三列 `flexBasis` 换行；首页 AI 舞台的全宽紧凑规则在华为真机暴露出角色过小、离中心过远问题，现已改为最大 360pt 的居中组合。
  - **审查修复**: VIP 礼包无数据时隐藏整个空区；抽奖状态未知时改为引导进入抽奖页，不再误报“今日已参与”；UTC+8 午夜刷新改为持续逐日重排定时器，页面跨多天停留仍能更新。
  - **验证**: `npm exec tsc -- --noEmit --pretty false`；`npm test -- --runInBand`（36 个 Jest 套件 / 139 个测试 + 201 个静态与合规测试全部通过）；`npx expo export --platform web` 成功收集 19 张海鲜 PNG；Expo Web 390×844 与 320×700 视觉验收；浏览器控制台无运行错误；`git diff --check`。

- [x] **管理后台 VIP 用户普通奖励树无效入口修复**（2026-07-13）
  - **来源**: 用户截图反馈：直接成为 VIP、未进入普通树的用户，在会员详情点击“普通奖励账户”的“查看完整结构”后，普通奖励可视化显示加载失败。
  - **实际做了**: 会员详情接口复用已有 `normalTreeNodeId` 返回只读 `normalTree.hasNode`；详情页仅在存在普通树节点时显示跳转入口，未入树用户展示“未进入普通树 / 尚未参与普通树奖励”的明确状态。已在普通树中后升级 VIP 的用户仍可查看其冻结的历史结构。
  - **验证**: 后端单测覆盖有/无普通树节点两种返回；管理后台 TypeScript 编译与静态回归测试通过。

- [x] **团长利润安全配置页可操作化**（2026-07-11 完成）
  - **问题修复**: 原页面把未保存的关闭状态、缺失的非财务系统配置和真实 SKU 利润缺口混为同一条红色告警，导致“封顶 0%、四行均安全、页面却提示突破底线”的矛盾展示。
  - **实际完成**: 后端将订单利润参数完整度与全量系统配置快照完整度分开：仅加价率、VIP 折扣及 VIP/普通奖励、产业基金、直推奖励缺失时阻断保存；团长配置未保存默认按 V3 关闭、0% 奖励模拟。后台按“团长关闭 / 参数待完善 / 真实利润缺口 / 安全”展示中文状态，给出精确的配置跳转与可改参数说明；只有真实缺口时展示风险组合和限制商品明细，商品名称和规格名称替代内部 ID。
  - **审计边界**: 全量配置快照不完整仍会把新版本标记为不可回滚，不会伪装成商品利润风险；保存失败响应保留结构化的团长状态与利润参数完整度，供后台准确引导。
  - **验证**: 利润安全后端单元测试、异常响应测试、管理后台利润引导测试、TypeScript 编译、管理后台生产构建和差异检查通过。
  - **2026-07-11 交互热修**: “开始配置团长激励”不再跳转当前 `/captain/settings` 路由；点击后平滑定位到“基础开关”、聚焦启用开关并显示中文定位提示，不自动修改开关。关闭状态下移除与利润摘要重复的蓝色状态卡。
  - **2026-07-12 关闭状态保存修复**: `schemaVersion` 并非可编辑表单字段，`validateFields()` 不会返回它；保存时现固定补入 V3 协议标识和项目编码，避免关闭状态下编辑目标净利率、风险预留或利润奖励后被前端误判为旧版配置并恢复默认值。
  - **2026-07-12 保存入口补齐**: 团长配置在风控参数下方新增与顶部同一保存动作；审查管理后台长表单后，为红包活动、团购活动和 VIP 赠品方案抽屉补齐固定底部保存/取消动作。既有短表单、`ProForm` 底部提交及普通商品编辑页的顶部 sticky 保存不重复添加按钮。
  - **2026-07-12 适用范围与说明优化**: 适用类目、适用商品、适用商户和不参与奖励的商品改为点击即加载、名称/ID 远程联想、滚动分页的真实数据多选；已保存但后续停用的对象仍可回显。所有可编辑参数的问号说明全部重写为“什么意思 / 举个例子 / 还要一起看”，每项都有金额、人数、日期或百分比的具体例子。
  - **2026-07-12 浮层裁剪修复**: 问号说明浮层统一由外层 body 控制宽度，内容占满浮层且允许中文自动换行，桌面端保持最大宽度，窄屏按视口宽度收缩，不再被右侧裁剪。
  - **2026-07-12 全部普通商品范围模式**: 团长配置新增明确的“全部普通商品 / 指定范围”模式，不再将空白白名单解释为全部。全部模式覆盖当前及未来普通商品，VIP 礼包、团购和奖品固定不参与，排除商品优先；保存配置、商品上架/审核和 SKU 改价/改成本均进入同一利润安全事务，订单结算只读取已保存快照。旧 V3 配置缺少模式字段时自动按“指定范围”兼容读取，V1/V2 历史配置读取时剥离该 V3 字段。

- [x] **VIP/普通系统配置利润安全实时预检**（2026-07-11 完成）
  - **实际完成**: VIP 系统配置页和普通用户系统配置页对已加载且已修改的有效七项比例做 500ms 候选预检，丢弃过期响应；仅 `config:update` 管理员可触发，不写入 `RuleConfig`、版本或审计记录。
  - **2026-07-11 交互收口**: 利润安全状态、团长配置保存失败和利润纠错页面均只显示中文原因；内部错误码、配置键、场景键和规格标识不会直接暴露给管理员。
  - **2026-07-11 全后台收口**: 管理后台通用 API 错误入口统一转换内部错误码和英文技术异常；登录、账号安全、订单发货、VIP/普通配置、团长月结及团长利润相关成功响应字段均复用中文化规则，避免后续接口再次将内部标识展示到页面。
  - **验证边界**: 浏览器预检仅作建议，保存时后端仍在 Serializable 事务和 advisory lock 内执行权威原子硬校验。

- [x] **团长优惠后利润分成 V3 与平台利润安全收口**（2026-07-10 完成）
  - **固定口径**: `D=max(0, 非奖品商品金额-成本-VIP折扣-平台红包-消费积分-团购返还抵扣-明示商品优惠)`，运费不进入；`C` 是命中团长范围的商品级 `D` 子集，`0<=C<=D`。VIP/普通会员路径与团长路径独立，但团长资金只能占用本单实际平台留存利润。
  - **实际完成**: 支付利润快照、VIP/普通与团长 V3 独立归因、按订单配置快照月结、全奖励桶累计退款冲回、已提现差额抵扣/追偿、不可变利润 revision、利润纠错/调整单后台、月结对账阻断、配置与 SKU 共享 Serializable advisory lock 硬校验、V3 参数问号说明、买家端分页/错误态和完整配置版本回滚隔离。连续 revision 只使用最新活动 canonical 来源，历史 `VOIDED` 流水不会重新入账。
  - **最终审查补强（2026-07-11）**: 月结草稿指纹排除其自身的预留释放流水并固定排序；买家/管理端月份统一为 Asia/Shanghai 且后台订单按支付时间筛选；“最低计佣商品实付”改为按团长范围内优惠后净商品 GMV 判断而非利润 C；团长归因与释放服务缺失时启动失败，禁止静默漏记佣金。
  - **验证**: 真实 PostgreSQL 并发测试 2/2、资金聚焦测试 129/129、完整后端 Jest 210 个套件/2482 个测试、后端/买家 App/管理后台/卖家后台编译构建、Prisma 96 个迁移、管理与 App 静态交付测试全部通过；两轮独立资金审查和一轮前端审查的问题已收口。

- [x] **预包装海鲜团长一层直推合规改造**（2026-07-10 完成）
  - **来源**: 用户明确要求删除二级团长，团长不得因下级团长或其客户成交获得收益。
  - **实际做了**: 团长绑定只保留直接团长；合格订单只创建 11% `DIRECT_ORDER` 冻结佣金；月结仅统计直接客户有效净 GMV，原团队池改为直接团长独享的 1% 经营绩效奖；后台配置删除二级费率、层级、团队人数和 40/60 权重，买家端和管理后台改为直接客户指标。历史二级字段重命名为 `legacyIndirect*`，历史账本类型重命名为 `LEGACY_INDIRECT_ORDER`，仅用于审计及已存在账本的释放、退款冲回和支付收尾。
  - **验证**: Prisma schema 校验与生成、后端团长/管理端 68 项相关测试、后端 TypeScript、买家端 TypeScript、买家端团长仓库测试和管理后台生产构建均通过。

- [x] **团长一层直推二次审查与配置说明**（2026-07-10 完成）
  - **实际做了**: 修复配置生效时间只保存不执行、无有效订单的新绑定客户被错误计数的问题；移除从未接入执行链路的同设备/同地址阈值，并兼容清理已有 V2 配置。团长配置页仅保留真正会改变运行结果的字段，所有可编辑参数右侧新增问号悬停说明，解释计算口径、关联参数、档位叠加和一层直推边界。
  - **验证**: 团长与管理端相关 Jest 72 项、后端 TypeScript、管理后台生产构建通过；源码审计确认新绑定、新订单和新月结均没有二级收益路径。
...
- [x] **预包装海鲜团长经营管理后台**（2026-07-08 新增并完成）
  ...
  - **实际做了**: 新增独立“团长经营”菜单和 `/captain/*` 路由；补齐团长列表、详情、订单归因、佣金流水、月度结算和团长配置页；月度结算支持按月份生成草稿、审核、标记支付和安全重算，批量生成不会改动已审核/已支付结算；`CAPTAIN_SEAFOOD_CONFIG` 完整暴露启用、范围、逐单佣金、月度门槛、阶梯奖励、封顶、税务和风控参数，并实时展示总激励率、预估净利和风险预留后净利；操作权限独立为 `captain:read/manage/settlement/settings`，不混入 VIP 树、普通奖励、消费积分或平台红包。
...
- [x] **首页/我的页换位后推荐中心与积分成长入口收口**（2026-07-05 新增并完成）
  ...
  - **实际做了**: 将共享 `MeIdentityCard` 和“我的”页常用工具统一为“推荐中心”入口，普通用户进入 `/me/referral` 查看普通分享码、绑定关系和推荐用户，VIP 用户进入同页查看 VIP 推荐码和直推用户；“我的”页成长入口统一为“积分成长”，只进入 `/me/growth` 查看积分、成长值、等级和兑换。
...
- [x] **普通成长体系与普通分享码推广链路**（2026-07-04 新增并完成）
  ...
  - **实际做了**: 新增独立 Growth / NormalShare 后端模块，覆盖普通积分账户、成长值账户、流水、行为规则、等级、兑换商品、普通分享码、普通分享绑定、延迟归因和幂等发放；积分不写入 `RewardAccount`、不提现、不直接抵扣订单现金，成长值只升等级不消耗；红包奖励通过现有 Coupon 发券能力发放。注册、签到、任务、普通商品确认收货、退款冲正、邀请好友首单奖励均已接入，手动确认和自动确认收货都会触发成长事件。管理后台新增“会员成长”运营台，支持配置行为规则、等级、兑换项、账户流水、普通分享码与绑定关系。买家 App 第一版在成长页承载普通分享码能力；2026-07-05 已收口为：`/me/referral` 统一承载普通分享码二维码/复制/系统分享、绑定好友分享码和推荐用户列表，`/me/growth` 只保留积分、成长值、等级和兑换。官网新增 `/s/{code}` 普通分享落地页，App 延迟深链区分普通 `/s`、VIP `/r`、团购 `/gb`。
...
- [x] **普通成长体系二次收口：用户可见规则、任务、升级说明与存量账户回填**（2026-07-04 新增并完成）
  - **来源**: 用户反馈普通用户不知道普通推荐有什么收益，也看不到积分 / 成长值具体通过哪些任务积累、如何升级；同时管理后台“普通成长 > 账户”显示 0 个账户，期望这里管理现有所有普通用户。
  - **实际做了**: 买家 App “我的”页面用户卡片和常用工具统一进入“推荐中心”，普通用户在 `/me/referral` 查看普通分享码、推荐收益、绑定推荐人和最近推荐用户，VIP 用户在同页查看 VIP 推荐码、直推比例、直邀 VIP 人数和最近推荐用户；`/me/referral-users` 展示全部直接推荐用户。`/me/growth` 保持为独立“积分成长”页，仅通过 `GET /growth/guide` 展示“赚积分和成长值”“升级规则”“积分兑换”，奖励、发放时机、限制次数和等级门槛全部来自管理后台配置；页面明确积分兑换会消耗，成长值只用于升级不因兑换减少。管理后台“普通成长 > 账户”和顶部统计改为按所有有效普通买家展示，缺失 `GrowthAccount` 的旧用户以 0 积分 / 0 成长值虚拟展示；统计合计、今日流水和兑换数同步按普通买家过滤；等级筛选同时兼容 `currentLevelCode` 和成长值区间，避免迁移后的 0 值账户筛不出来。新增 Prisma 迁移补齐旧普通买家的 0 初始成长账户和普通分享码，并新增幂等脚本 `npm run normal-growth:backfill -- --execute` 为旧普通买家做二次兜底；新注册、微信首次登录和验证码自动注册都会自动创建 `GrowthAccount` 与 `NormalShareProfile`。官网普通分享落地页在安卓浏览器中优先尝试 `aimaimai://normal-share` 打开已安装 App，失败后再进入下载兜底，避免已安装用户被迫重新下载。
  - **验证**: 新增后端回归测试覆盖“没有 GrowthAccount 的普通买家也能出现在管理后台账户页”“迁移后等级码为空但成长值为 0 的账户能按起始等级筛选”“新用户注册自动创建成长账户和普通分享码”“回填脚本 dry-run/execute 行为”；新增 App/管理端静态测试覆盖普通推荐码按钮、推荐收益、任务列表、升级规则和后台分类；已运行 `cd backend && npm test -- admin-growth.service.spec.ts growth.service.spec.ts backfill-normal-growth.spec.ts auth.service.spec.ts --runInBand`、`node --test scripts/__tests__/normal-growth-ui.test.mjs`、`cd backend && npm run build`、`cd backend && DATABASE_URL='postgresql://user:password@localhost:5432/nongmai_validate' npx prisma validate`、`cd admin && npm run build`、`npx tsc --noEmit`。

- [x] **管理后台“积分成长”统一账户视图**（2026-07-04 新增并完成）
  - **来源**: 用户确认积分和成长值应统一管理普通用户与 VIP 用户，但 VIP 推荐关系已经有 VIP 会员、VIP 奖励可视化等页面，不应在成长页重复做一套推荐关系管理。
  - **实际做了**: 管理后台侧边栏从“普通成长”改为“积分成长”；顶部统计改为成长账户总数、普通用户 / VIP 用户数量、平台积分余额和平台成长值；账户页新增用户身份筛选，支持查看全部、普通用户或 VIP 用户。普通用户行继续展示普通分享码并支持启停，VIP 用户行只读展示 VIP 推荐码，并提供跳转到 VIP 会员详情和 VIP 奖励树的入口；账户表统一使用“积分余额”列名，手动扣减错误也改为通用积分文案，避免 VIP 行出现“普通积分”误导；行为规则列表隐藏内部英文行为码，将“接入状态”和“状态”合并为“生效状态”开关，已接入规则可直接切换生效/停用，未接入规则禁用开关并提示未接入，适用用户改为中文展示；成长等级列表隐藏内部等级编码，只保留管理员实际要维护的等级名称、成长阈值、展示称号、月兑换上限和启用状态；红包管理支持将手动发放红包标记为“积分兑换专用”，该类红包池不走普通手动发放入口；积分兑换项的红包选择改为“积分兑换专用红包池”，只展示启用中、未发完、未过期、且已标记“积分兑换专用”的手动发放红包池，并由后端阻止自动奖励红包、用户主动领取红包和普通手动红包绑定兑换项，下拉选项分行展示红包名称、发放方式、剩余数量和使用门槛；“普通分享”Tab 保持只服务普通用户拉新，避免和 VIP 推荐体系混用。后端管理接口同步支持 `userType=ALL|NORMAL|VIP`，看板返回普通 / VIP 账户数，并在用户摘要中返回 `vipReferralCode`。
  - **验证**: 新增后端回归测试覆盖 VIP 成长账户可被查询但不暴露普通分享码、手动扣减使用通用积分余额错误、积分兑换只能绑定标记为“积分兑换专用”的手动红包池、普通手动红包不能被积分兑换或系统发券入口误用；新增管理端静态回归测试覆盖“积分成长”命名、用户身份筛选、VIP 推荐码只读展示、跳转 VIP 页面、不新增 VIP 推荐 Tab、行为规则不展示内部行为码、成长等级不展示等级编码、行为规则状态列不重复、行为规则生效状态使用开关且未接入规则不可切换、红包管理可标记积分兑换专用池、积分兑换只展示专用红包池。已运行 `node --test scripts/__tests__/coupon-campaign-rules.test.mjs scripts/__tests__/normal-growth-ui.test.mjs`、`cd backend && npm test -- admin-growth.service.spec.ts coupon.service.issue-system.spec.ts --runInBand`、`cd backend && npm test -- coupon-campaign-rules.spec.ts --runInBand`、`cd backend && DATABASE_URL='postgresql://user:password@localhost:5432/nongmai_validate' npx prisma validate`、`npx tsc --noEmit --pretty false`、`cd backend && npm run build`、`cd admin && npm run build`、`cd backend && npm test -- --runInBand`、`npm test`、`git diff --check`。

- [x] **管理后台运营工作台 V2 审查与优化**（2026-07-04 新增并完成）
  - **来源**: 用户要求全面审查管理后台工作台数据接口、真实可靠性、显示是否正常，并从运营经理视角补齐需要统计的内容。
  - **实际做了**: 新增 `GET /admin/stats/operations-overview` 聚合接口，工作台不再前端散调多个列表接口拼统计；今日订单、GMV、客单价和支付渠道统一按有效支付时间与 UTC+8 中国自然日统计，优先 `paidAt`，历史已付款订单缺失 `paidAt` 时回退到 `createdAt`；抽奖今日数据改按 `LotteryRecord.drawDate` 中国自然日统计；红包有效活动按状态和起止时间窗口统计；页面重做为待办中心、今日经营、销售趋势、最近订单、奖励资金、数字资产概览、活动增长、经营脉搏和处理优先级，最近订单改为紧凑列表避免表格裁切。
  ...
  - **2026-07-04 数字资产口径修正**: 数字资产是虚拟权益指标，不再放入资金结构饼图和奖励资金卡片；工作台单独新增“数字资产概览”，展示数字资产值、资产账户数、今日新增资产和对应累计消费。
...
- [x] **我的页“我的财库 / 我的福利”命名统一**（2026-07-03 新增并完成）
  - **来源**: 用户截图反馈买家 App 我的页中“钱包”应改为“我的财库”，“我的红包”应改为“我的福利”，并强调相关位置都要统一修改。
  - **实际做了**: 买家 App 我的页金色资产卡、常用工具入口、财库页标题/错误态、VIP 权益说明、提现规则说明、注销资产快照统一使用“我的财库”；福利页主标题、主 Tab、空态、领取成功提示和后端领取失败展示统一使用“我的福利”；保留结算页“红包抵扣 / 选择红包”等下单抵扣业务词，避免优惠类型语义被改乱。
  - **验证**: 先新增失败回归测试；修复后 `node --test scripts/__tests__/me-benefit-labels.test.mjs scripts/__tests__/coupon-campaign-rules.test.mjs`、`npx tsc -b --noEmit --pretty false`、production `expo export --platform android --clear` 通过。
...
- [x] **管理后台红包活动状态 Tab 分组**（2026-07-02 新增并完成）
  ...
  - **验证**: 先新增失败回归测试；修复后 `node --test scripts/__tests__/coupon-campaign-status-tabs.test.mjs`、`cd admin && npm exec eslint src/pages/coupons/campaigns.tsx src/pages/coupons/campaign-status-tabs.ts`、`cd admin && npm run build` 通过。
...
- [x] **团购精选货架与详情页**（2026-06-22 新增并完成）
  ...
  - **实际做了**: 新增 `/group-buy` 货架页和 `/group-buy/[activityId]` 详情页；无当前团购时展示商品货架，有当前团购时默认进入“我的团购”并可切换到商品；当前团购面板展示推荐码、二维码、进度和结束入口；详情页展示后台配置的团购详情介绍；商品卡、详情和进度不展示返还基数或返还百分比；购买新团购但已有进行中分享时，用底部抽屉提示需先结束本次分享；页面移除“分享回馈活动”和“仅一级直接推荐”提示；2026-06-30 起团购首页顶部“精选团购”小字仅展示“不退换，仅24小时质量问题补发”规则，不再单独展示规则提示条；团购详情页保留底部付款栏滚动留白，并在详情路由隐藏全局 AI 浮球，避免 Android 真机上高层手势浮层拦截页面滚动。
  - **验证**: `npx tsc -b` 通过。
...
- [x] **企业详情检测报告受控预览**（2026-08-11）
  - **来源**: 用户反馈检测报告点击“查看”会触发文件下载，希望直接预览内容。
  - **实际做了**: 买家 App 不再使用公开接口的原始 `fileUrl`；报告动作改为“预览”，仅打开由报告 ID 构造的受控平台预览地址。后端只允许当前有效、已验证且属于活跃非平台企业的 `documents/` 存储 PDF/图片，以 `inline` 响应返回；不支持预览的历史文件明确禁用。原始存储 URL 不再下发给公开客户端；旧 App 的同名字段只在配置 `PUBLIC_API_BASE_URL` 时返回受控预览 URL，以避免 OTA 前回归；对象丢失、流错误和报告失效会得到安全的 404 或用户可读预览页。
  - **限制**: Expo 系统浏览器会优先内联预览，但 Android/iOS 最终是否提供保存功能仍由系统浏览器决定；“绝不允许下载”需要后续原生 PDF/图片阅读器和真机验收，不应由本次浏览器方案声称保证。
  - **验证**: App/后端聚焦回归测试、App TypeScript 编译、Prisma schema 校验；后端完整 build 如仍存在本改动无关的既有错误，须在发布日志单列。
...
- [x] **Phase 4 补充 · 库存感知复购与低库存展示（2026-05-18）** — 复购低库存降级为 1、0 库存虚拟提示不入真实购物车、购物车/结算禁选无库存、后台低库存阈值、售后退货退款回填库存；2026-09-14 补齐 App/小程序商品发现页恢复焦点时刷新全品类库存缓存，App 同时覆盖系统后台恢复和企业 Tab 切回，避免后台补库存后仍显示旧“已售完”
...
> **2026-08-24 产品决定覆盖旧路径**：H5 不再登录、注册或绑定，只提供小程序/App 双入口；用户只在选择的客户端完成一次登录，推荐关系由客户端继续核验。
...
- [x] **H5-INV06**（2026-08-24 取消并由 H5-INV07 替代）旧“扫码 H5 → 短信登录/注册绑定”验收不再适用。
- [ ] **H5-INV07**（2026-08-24 已正式发布，待真实推荐码验收）无登录双入口：`/invite/{code}` 不收集手机号/验证码、不执行 H5 绑定。手机端展示“打开爱买买小程序”和“下载爱买买 App”两张按钮；桌面端直接展示 App 推荐交接二维码，小程序二维码由用户点击后按需生成，禁止页面加载自动消耗 URL Link 配额。代码经 `staging-next` tree-identical CI/E2E 与本地响应式验收后合入 `main@98511bab`；Website-only run `32754481192` 成功部署，回滚快照和线上 marker 已验证。仍需用真实有效推荐码完成微信 URL Link、App 交接、客户端登录绑定和不可覆盖关系真机验收后才能勾选。
...
- [x] **GB07** App 团购规则文案：团购首页、详情、扫码落地、付款页和当前团购面板统一展示“付款后立即生成推荐码 / 直接推荐其他用户才计入 / 付款冻结返还 / 收货后释放 / 不退换，仅24小时质量问题补发 / VIP 用户购买团购后会累计消费资产”；2026-06-30 起团购首页顶部“精选团购”小字仅承载“团购不退换，仅24小时质量问题补发。”，当前推荐码卡片不再重复展示冻结释放说明。
...
## 🧧 全平台订单队列奖励（2026-07-28 新增）

> **权威源**: `docs/features/global-order-queue-reward.md`
> **边界**: 独立于普通树、VIP 树和直推奖励；默认关闭，不追补生效时间前付款的订单。

- [x] **QR01** 数据与配置：独立订单状态、位置、单位分配和红包分配；到账通知复用统一持久化通知 outbox；10 个后台参数及生产默认值（含单笔位置上限）。
- [x] **QR02** 确认收货原子分配：全平台单队列、N 人滑动、完整金额单元拆位、同用户历史位置、共享订单封顶、平均/可重放正态随机。
- [x] **QR03** 利润资金守恒：支付时规则快照；实际队列出资从平台利润分成扣除；缺少成本仍入队但零出资；利润安全预检覆盖普通/VIP 路径。
- [x] **QR04** 内部待结算与售后：确认收货只记内部待结算，不写用户冻结钱包；来源订单和受益位置订单双窗口通过后直接到账；退款/退货/换货成功双向整单作废。
- [x] **QR05** 钱包与提现：独立 `QUEUE_REWARD` 账户只在奖励实际到账后建账，不进入商品抵扣；统一提现支持队列可用余额，提现处理中金额计入钱包冻结汇总。
- [x] **QR06** 消息与响铃：每笔实际到账独立写统一持久化通知；App 前台按时间游标分页追赶并逐笔播放短铃，同一消息只响一次。
- [x] **QR07** 买家 App：我的页入口、钱包分账户卡、全新 21 槽接力轨道状态页及独立 `/bonus/queue/v2/status`。
- [x] **QR08** 管理后台：独立“订单队列奖励”参数页，10 参数原子保存、权限控制、开启影响确认与利润口径提示；队列利润比例单项范围为 1%–100%，平均分配模式在候选利润预检和最终保存前都补齐隐藏的正态随机字段，保存按钮不再永久置灰且确认保存不会提交空字段；预检阻塞时直接显示原因，后端安全闸门继续阻止超出平台份额或整体利润预算的组合。
- [x] **QR09** 最终收口：硬滑动窗口、配置资源上限、利润异常不推进、多次部分退款累计调账、已提现/提现中追偿、注销守恒、生鲜售后窗口、释放批次饥饿、状态/铃声稳定游标及失败 outbox 重驱均已修复；补充不写数据库的 21→22 人虚拟演示和队列配置实时利润安全闸门；后端 275 套件 / 2940 项、App Jest 38 套件 / 145 项、根契约与法务 207 项、Prisma validate、后端/Admin 构建、App TypeScript、队列后台页面 ESLint 和三路独立复审全部通过。
- [ ] **QR10** Staging 数据库迁移、后台最大允许参数组合负载测试、开启小比例灰度、真机验证 20 笔逐条到账消息/响铃、售后内部待结算/作废与提现；未获确认前不发布。
...
## 🌱 普通/VIP 直推分润与自动 VIP（2026-07-05 新增）

> **触发**: VIP 像门槛，普通用户和 VIP 用户都需要可持续推广收益；普通邀请关系一旦建立不可随意切换，被邀请人成为 VIP 时按邀请人当时身份决定关系失效或转入 VIP 团队。
> **权威源**: `docs/superpowers/specs/2026-07-05-direct-referral-vip-upgrade-design.md` + `docs/superpowers/plans/2026-07-05-direct-referral-vip-upgrade.md`

- [x] **DR01** 数据与配置：普通分享绑定新增关系状态、生效推荐人和失效审计；后台新增普通直推比例、自动 VIP 开关、累计消费自动 VIP 门槛；普通利润结构扩展为平台/六分/直推七分。
- [x] **DR02** 绑定规则：普通/VIP 推荐关系统一沉淀为直推关系，一旦存在推荐人不可换绑；普通邀请注册立即释放积分和成长值。
- [x] **DR03** VIP 升级规则：B 付费购买 VIP 礼包或累计消费达到门槛自动 VIP 时，如果 A 当时是 VIP 则 B 入 A 的 VIP 子树，否则 A-B 关系失效，B 走系统节点；仅付费 VIP 礼包按购买金额与 `referralBonusRate` 快照给有效 VIP 推荐人一次性推荐奖，累计消费自动升级不发该奖。
- [x] **DR04** 持续直推佣金：普通商品订单确认收货后按推荐人身份快照给直推人生成冻结佣金，普通/VIP 比例分别走后台配置；售后退款路径作废或调整对应直推佣金。
- [x] **DR05** 管理后台：积分成长页收敛为独立积分成长资产页，只管理普通/VIP 用户的积分、成长值、等级、行为规则、积分兑换和流水；新增“推荐与拉新”页面承接普通推荐关系、普通分享绑定状态、失效原因、VIP 推荐码查看和自动 VIP 转化设置；用户管理页新增“推荐码”列，按用户当前身份展示普通分享码或 VIP 推荐码并支持二维码/链接查看；用户详情新增“推荐关系”Tab，按单个用户核查当前有效推荐人、收到的普通/VIP 推荐关系、直接邀请的普通用户和直接邀请的 VIP 用户；普通/VIP 系统配置页展示对应直推比例。
- [x] **DR06** 买家 App：推荐中心 `/me/referral` 展示普通/VIP 持续直推收益、自动 VIP 门槛、VIP 关系切换边界和最近推荐用户；`/me/referral-users` 展示全部直接推荐用户；积分成长页 `/me/growth` 只保留积分、成长值、等级和兑换。
...
- [x] **DR09**（2026-07-10）恢复付费 VIP 礼包一次性推荐奖：修复 7 月 5 日统一推荐关系改造误删 `activateVipAfterPayment()` 授奖调用的问题；金额使用 `VipPurchase.amount × referralBonusRate` 购买快照，原 Serializable 激活事务内发放；增加同一 `VipPurchase.id` 防御性查重；累计消费自动升级仍不发一次性奖；对已确认漏发的 AIMM00000000000119 → AIMM00000000000032 单独幂等补发 51.87 元。
...
- [x] **AA13** 修复生产客服实时通讯握手失败：Socket.IO 未单独配置 `ALLOWED_ORIGINS` 时复用全局 `CORS_ORIGINS`，避免生产管理后台/App 域名被 localhost 默认白名单拦截；新增网关跨域配置回归测试并同步部署说明。
- [x] **AA14** 客服实时链路全量加固：Socket 握手前及每次关键操作校验账号/登录会话/实时权限，只读管理员不进入可分配坐席池，并支持同管理员多标签页；管理端断线时补拉当前会话、发送使用持久化 ACK、操作等待服务端确认；主动联系复用会话也保存并广播首条消息；App 防止旧轮询覆盖 Socket 新消息，关闭/重发不误显示成功或制造重复；工单、转接、席位、关闭、清理和 AI 回复使用行锁、CAS 与原子事务，后端客服测试 15 套件 225/225 通过。
...
## AI Visual Agent（v3 重构排程；整体未验收）

权威设计：`docs/superpowers/specs/2026-08-22-universal-product-visual-agent-design.md`，2026-09-04 v3。以下旧切片完成记录保留追溯，不代表下列重构和整体验收完成。

- [x] **PVA-V3-DESIGN**：将代码审查与真实手表反例写入设计，明确五条路线、任务/检查/账务/使用状态、后台配置权威性、通用接入及必测矩阵；仅文档完成。
- [ ] **PVA-V3-P0**：服务端后台任务、关窗/断网恢复、任务查询、报价过期与资料变化、能力探测；管理员价格和暂停不被测试初始化覆盖。
  - 本地切片：初始化保留管理员配置、欢迎赠送非强制依赖、执行使用报价模型快照；专项 31 项通过，已独立审查。P0 整体与真实后台验收尚未完成。
  - 本地执行器切片：确认同事务入队、租约调度、纯查询及作用域隔离历史接口；隔离空库 140 migrations、真实四并发冻结与故障回滚通过。全后端 302 suites / 3494 tests 通过，条件跳过另记；未部署。
  - 本地商家交互切片：历史/过期/恢复/暂不采用/免费合约入口；独立审查发现跨图回包与恢复竞态并修复。真实 Chromium/React 受控网络两用例组、17 项专项及 TypeScript 通过；不是线上模型验收。
- [ ] **PVA-V3-P1**：免费 local-photometric-v2（不伪造 VERIFIED_EMPTY）、主体分割与保真合成、上传进度和回执恢复；原始大图及代表样本真实验证。
  - 本地免费合约切片：不依赖收费 OCR，新上传受管图无需先发布即可生成，旧任务保持原合约；41 项相关测试通过，已独立审查。主体分割、免费后台恢复及真实效果验收尚未完成。
  - 本地上传反馈切片：移除虚假百分比，真实XHR与延迟响应/坏图拒绝测试通过；持久回执待实施，未部署。
- [ ] **PVA-V3-P2**：结构化编辑计划、局部精修、关键部件对比、低清修复分流、营销结果实际使用、候选放弃与历史；手表表冠变化反例必须进入回归。
  - 本地下载切片：已生成候选受权下载、中文失败重试和历史回访通过HTTP/浏览器测试；营销详情图片位尚未实现，结构检查接线及真实模型效果待验证。
  - 本地结构Core切片：双图有限结构报告、六scope收费授权、持久结果重放与账务状态隔离完成；独立审查已修发现项，Provider23/runner30单测和独立PG9项通过。商品集成与真实模型效果另验，不标整体完成。
  - 本地商品接线/后台预算切片：修长幂等键、PENDING抢终态、报告丢失、暂停隐藏FAIL及大图输入；手表focus冻结进quote，旧quote兼容。预算35/商品与报价90项及Admin浏览器1项通过，图片模块整合434项通过。仍待通用API接线和真实模型/迁移部署验收。
  - 通用API本地接线已收口：输出先持久化，结构PENDING在poll/GET/采用共同守门，生成费用结算不放行未检查结果；37专项和全后端3617项通过。隔离空库141 migrations+3项事务测试通过，商家浏览器4组/Admin1组通过；仍无push/部署/真实百炼结构验收。
- [ ] **PVA-V3-P3**：独立图片积分购买与管理闭环、统一客户端/SDK、任务下载与签名 Webhook；支付渠道/套餐/退款条款实施前确定。
- [ ] **PVA-V3-P4**：至少 40 张授权图片、多商家/管理员、网络与恢复故障矩阵，记录每项真实结果及费用；同一精确候选分开报告代码、CI、测试发布、效果验收及生产状态。
...
- [x] **PVA-UPLOAD-TESTER-ACCESS**（2026-09-02 本地候选）：修复受管商品图 15 秒超时、无变化进度条、英文错误/红框无指引、超大图片 400 和超时重试重复资产；新上传受管图可先私密美化，采用时原实拍证据与候选成对挂载。增加 staging 专属双门禁“开通指定测试商家”，按公司/OWNER-MANAGER/商品/模式/费率卡/有效期创建六层精确预算并幂等发放欢迎图片积分，未授权、预算不足或模型暂停时不下发费率卡；补齐营销场景后台配置。商品事实版本进入计划与报价哈希，报价/提交前变化会拒绝或退回冻结积分。同步修复跨商户 quote 轮询隔离，以及暂停新提交时仍可恢复已被 Provider 接受的在途任务。当前仅本地提交前验证，尚未 push 或部署。
- [x] **PVA-ALL-STAGING-MERCHANTS**（2026-09-03 本地候选）：按用户确认，测试环境可用独立开关默认开放全部 ACTIVE 商家 OWNER/MANAGER；查看方案时为当前公司、人员、商品和费率卡幂等创建精确策略并发欢迎图片积分，不使用通配权限。测试环境不限制日/周调用次数或平台共享第三方预算，每次仍须确认图片积分；状态在平台后台可见，生产双开关保持默认关闭。
- [x] **PVA-LEGACY-MEDIA-ADOPTION**（2026-09-04 本地候选）：修复真实候选已生成但旧商品 `assetId=null` 导致无法采用的问题。仅兼容一张规范 HTTPS 历史图片，完整快照 URL、顺序、类型和替代文本；新公开媒体只写受管候选与原实拍证据，管理员回滚按媒体版本恢复旧 URL。视频、多张/混合旧媒体和不安全 URL 继续拒绝。定向后端 43 项测试、管理端合同测试和 production build 已通过；待同一候选 exact SHA 部署后执行一次采用与回退验收，不再次调用模型。
- [x] **PVA-OCR-DISABLED-UX**（2026-09-03 本地候选）：修复免费实景调优的商品事实检查在 Qwen OCR 执行开关关闭时暴露英文内部 Provider 名称。后端返回中文业务原因；商家端把服务关闭态保留在弹窗内、禁用重复点击并继续提供付费精修入口。关闭态不创建扫描、不调用模型、不扣图片积分。
- [x] **PVA-QUOTE-PLAN-REFRESH**（2026-09-03 本地候选）：修复商品草稿自动保存仅更新时间变化却误判美化计划失效，以及付费报价只自动恢复“计划过期”而未覆盖商品事实版本变化。事实哈希升级为绑定标题、副标题、说明、分类 ID、分类名称与媒体版本；冲突时自动刷新计划和费率卡，原方案不再适用时要求重新选择且不冻结图片积分。
- [x] **PVA-WHITE-BACKGROUND-CREDIT-UX**（2026-09-04 本地候选）：上传诊断识别真实透明像素；透明图进入免费白底合成，普通照片改走可报价的智能白底/棚拍路线，不再显示必败的“制作白底图”按钮。只有计划授权背景替换时才要求模型生成白底。付费区新增常驻可用/冻结图片积分、方案消耗、预计余额和“获取图片积分”入口，用请求序号阻止旧余额覆盖新余额；在线购买尚无订单/支付实现时明确标注暂未开放，并精简主流程解释文案。
- [x] **PVA-CROSS-IMAGE-RACE**（2026-09-04 本地候选）：图片建议、费率与报价使用流程代次和源资产双重校验；关闭、换图或换方向后旧响应不能写入新图片状态。报价持久绑定原图快照，轮询完成不再读取可变当前选图；请求中禁用关闭和切换，防止旧图扣费/新图展示的跨图片串线。
- [x] **PVA-CREDIT-BASED-ACCESS**（2026-09-04 本地候选）：平台后台移除“开通指定测试商家”表单；测试环境统一显示“有图片积分即可使用”。商家无需人工授权，商品归属、角色、积分余额和平台全局模型/费率/成本保护是仅有门禁；自动生成的精确预算仅作成本隔离，不是商家权限。
- [x] **PVA-SINGLE-REQUEST-RATE-CARDS**（2026-09-04 本地候选）：删除商家端及平台端逐商家测试授权接口；费率查询在服务端内部幂等准备测试环境费用隔离记录，浏览器只发一个请求。有图片积分的 ACTIVE 商家 OWNER/MANAGER 无需任何单独授权；没有可用模型、费率或完整预算时返回明确中文错误，不再静默显示空方案。
- [x] **PVA-PAID-CANDIDATE-RECOVERY-UX**（2026-09-04 真实复测后本地修复候选）：处理中禁止重复点击确认生成；关闭可采用候选后原入口可重新打开，不可采用营销预览可结束本地会话后选择下一方案；付费方向按 optimizationId 绑定，免费任务不继承或清理旧付费状态；采用后清除旧成功横幅。真实万相任务已验证 160→150/冻结10→150/冻结0，刷新未重复提交。
...
- [x] **MP-PROD01**（Draft PR #1 已推送，未合并 main/未部署）发布安全门禁：main push 不自动部署；production workflow_dispatch + approval；exact SHA；主库备份；后端质量/E2E/readiness；后端先于 admin/seller/H5；workflow-only 不触发后端。即使手动选择 website/admin/seller 单独发布，也必须先验证对应环境 API 的 `releaseSha` 精确等于本次 `github.sha`，不能用 `backend=false` 绕过后端先行。后端候选在不可变 worktree 安装/编译，PM2 停止后才备份/migrate，按旧绝对 cwd/entrypoint 回退；flock、阶段 journal、运行 SHA、公网 health、App 旧登录/商品/购物车/普通+VIP+团购结算/钱包/售后探针和 `pm2 save` 均已接入。GitHub production/staging environment 已按分支隔离。独立终审无 Critical/High；Batch 1 正式发布前仍必须完成 main protection、PITR/离机恢复证明和首次服务器 PM2 只读预检。
- [x] **MP-PROD02**（Draft PR #1 已推送，未合并 main/未部署）Delivery 排除守卫：禁止配送门户、配送数据库、DeliveryModule/wiring/配置进入候选，同时保留商城 Shipment/SF/普通送货上门；静态排除与路由守卫 5/5 通过。
- [x] **MP-PROD03**（Draft PR #1 已推送，最新语义收口尚未 push/未合并 main/未部署/未迁移生产库）小程序后端兼容闭包与 19 条 migration（18 条 staging-derived 同哈希 + 1 条生产集成自动退款副作用 outbox）；原 main 路由零删除，小程序新增路由为并列接口；微信支付/退款/提现、自提、通知、购物车、六类收货副作用 outbox 与自动退款数字资产/旧团长佣金冲回 outbox 已接入。Prisma validate、Nest build、后端 260 suites / 3134 tests 通过；真实 PostgreSQL 120 migrations + 5 suites / 9 tests 通过。生产备份克隆库演练仍由 MP-PROD06 单独阻断。
- [x] **MP-PROD04**（Draft PR #1 已推送，未合并 main/未部署）平台后台与卖家后台最小自提/核销/微信提现界面；配送门户入口为 0；扫码固定为 Node 20 兼容的 `@zxing/browser@0.1.5 + @zxing/library@0.21.3`。依赖锁只新增 4 个 ZXing 节点并把 `path-to-regexp 8.2.0→8.4.0`，未顺带升级 Axios/React Router/Vite 等约 100 个包。Admin CI 契约 12/12 + build + 修改文件 ESLint 0；Seller CI 契约 12/12 + build + 修改文件 `--max-warnings=0`，并恢复全量 lint 可运行（0 error，28 个既有 warning）。Node 24 本地扩展源文件契约为 Admin 28/28、Seller 35/35。卖家核销已用不可变 resolved credentialRef + resolve session 消除“预览 A/核销 B”竞态；两端测试已接入 production CI。后端成功后才允许部署静态后台。
- [ ] **MP-PROD04A** Admin/Seller 既有依赖安全债务单独治理：当前最小 lock 仍继承 main 的 production audit（Admin 12 high、Seller 10 high）；不得在自提切片中混入 100+ 包升级。若生产前升级 Axios/React Router/Vite 等，必须形成独立回滚 commit 并跑全后台浏览器 E2E；否则按已知生产基线风险单独决策，不得冒充 audit 0。
- [x] **MP-PROD05**（Draft PR #1 已推送，未合并 main/未上传微信）完整 miniapp + production release-context；App 源码改动 0，不增加自提下单入口。小程序 lint/typecheck、55 files / 303 tests、staging+production build、72 页产物校验通过（总包 2.41 MiB、主包 1.262 MiB）；production 产物只含 `api.ai-maimai.com`/正式 WSS，无测试域名。已吸收 `origin/staging@acc0e08c` 删除无实际管理能力“微信授权说明”入口的最新小程序修复。正式 release-context 要求 HEAD 精确等于 `origin/main`；测试构建只允许显式选择 `origin/staging` 或临时 `origin/staging-next`，CI/手动构建只生成匹配目标分支的 artifact。
- [ ] **MP-PROD05B** 推荐 H5 双入口（2026-08-24 Website 与配套 App OTA 均已正式发布，真实推荐验收待办）：复用旧 staging 已验证的 `InviteChoiceLanding`，基于最新 main 只移植 website 路由/页面、客户端交接语义、测试与文档；保留旧后端 Auth 路由作兼容但新页面不调用。正式站 marker 已验证为 `main@98511bab`；App 已随 `main@3f953a4b` 的 runtime `1.0.6` OTA 移除旧 H5 登录漏斗统计。待完成真实推荐码扫描、双入口跳转和归因全链路验收后才能勾选。
- [ ] **MP-PROD05C** App 自提只读兼容 OTA（2026-08-24 已发布，真机验收待办）：同账号可在 App 看到小程序创建的自提订单，按“备货中 / 待自提 / 已取货”展示自提点、营业时间、自提人和须知；自提单禁止出现物流、修改收货地址和买家确认收货。当前批次不在 App 展示二维码，明确引导到微信小程序查看凭证。PR #13 与合并后 `main@3f953a4b` 的 checks/E2E 全绿，App 31 suites / 136 tests、静态与合规 273/273、自提专项 13/13、Android/iOS production export 与线上 manifest/资源下载校验全部通过；runtime `1.0.6` production OTA Group `cc86f255-9174-4bc2-84cf-cfc120b116b1` 已发布，上一稳定 Group `88b3edc9-454f-47d3-a82c-40511b04121f` 保留可回退。完成真实设备冷启动、配送回归和自提订单页面验收后才能勾选。
- [ ] **MP-PROD05D** 小程序跨端微信账号防重复（2026-08-25 开始）：后端按 `appId + openid` / `unionId` 三态裁决；已匹配直接登录，未匹配且有 `unionId` 时允许用户明确微信建号，未匹配且无 `unionId` 时只允许手机号绑定。账号衔接页必须有清晰手机号/验证码输入框，删除无条件“作为新用户继续”。最终组合候选已通过后端 260 suites / 3138 tests、Auth 专项 56/56、小程序 56 files / 308 tests、lint/typecheck、staging+production 双构建与 72 页产物校验；尚未 push/deploy/上传体验版。生产误建账号 `AIMM00000000000217` 已在完整备份、强制 ROLLBACK 干跑和业务关系审计后安全标记 DELETED，小程序微信身份已归到原主账号 `AIMM00000000000001`，API health 正常。完成新体验版后仍需覆盖 App 微信既有账号、App 手机号既有账号、全新微信+新手机号、缺失 `unionId`、重复提交和修复账号重新登录真机测试。
- [ ] **MP-PROD05E** 体验版推荐码与语音搜索修复（2026-08-25）：推荐码生产失败已由日志确认微信 `41030`，正式版场景页未发布时仅对该错误从 `release + check_path=true` 回退一次 `trial + check_path=false`，同 AppID 探针已返回有效 JPEG；其他错误不降级。语音结果删除“我听到”，搜索接收页对 Taro 路由 query 安全解码一次，避免“水产”显示为 `%E6...`。后端专项 8/8、语音路由专项 11/11及上述最终组合发布级验证全部通过；仍需 push/deploy和新体验版真机验收。
- [ ] **MP-PROD05F** 无新增 migration 的生产演练门禁（2026-08-25）：`main@5029a0c8` 部署前 exact-SHA attestation 因旧 baseline=101、当前正式库=120被 readiness 正确拒绝，未触发部署/迁移/PM2重启。新增 `NO_OP` 模式支持当前正式库备份的120→120隔离演练，必须验证完整行哈希、退款 outbox、兼容字段和migration checksum完全相同；`UPGRADE`原有101→120规则保持。发布契约本地43/43通过；尚未push/PR/合并，完成后必须以新main SHA重新备份、恢复双库、签发证明并再次审批生产部署。
...
- [x] **MP-PROD06** 生产备份克隆库迁移演练：`ca5e945f` 中间候选生成含 source identity、dump SHA 和 101 migration 基线的原子 manifest 备份；同一备份恢复带 provenance 的 baseline/target，19 条新增 migration 全部成功，target 120/0。23 表主键+稳定整行哈希、历史默认值、退款10任务逐条、target120 checksum、baseline101 subset 全部通过。正式业务库仍为101/0、生产 SHA `a0f47810`、PM2 online、商品接口200；发布后需清理 rehearsal 资源。
- [ ] **MP-PROD06A** 最终 exact-SHA attestation：当前候选 HEAD 已晚于 `ca5e945f`，旧 600 权限 attestation 会正确 fail-closed，不能授权生产。最终 commit/CI 确定后，必须复用可信备份和双库 provenance 重新运行 data verifier，绑定最终 HEAD/tracked-clean/migration tree 后再签发和复验。
- [x] **MP-PROD06C** 生产配置门禁：生产 `.env` 已完成原子备份并补齐正式订阅状态/3模板、代码路径检测、微信提现1005场景、自提开关和独立密钥；实际 dotenv round-trip 与完整 production preflight 通过。未重启 PM2，当前现网进程行为未改变；正式发布仍会在候选 build 后再次 fail-closed 校验。
...
- [x] **MP-PROD06L** 本地 PostgreSQL 18 空库与并发门禁：120 条主库 migration 全量 deploy 通过；售后退款、购物车数量、普通树、利润安全、卖家面单租约真实数据库 5 suites / 9 tests 全通过；临时库已删除。该结果不替代 MP-PROD06 的生产历史数据副本演练。
...
- [x] **MP-PROD09** 双客户端与分支治理规范：`main` 设为唯一长期基线、`staging` 仅作测试 release train；AGENTS/branch-strategy/github操作/版本管理已禁止在固定目录开发和整体合并长期分叉。固定微信目录同步/重绑脚本与 zsh refspec 防回归已纳入候选；旧 `staging@acc0e08c` 已完成远端 archive branch + annotated tag + `delivery/staging` 三重保全，原 staging 指针未移动。
- [x] **MP-PROD09A** GitHub 服务端分支保护：main 已要求 PR、`e2e` + `checks`、对话解决，禁止删除/强推且管理员不绕过；仓库只有 1 名管理员，required approval 设为 0，避免自锁。旧 staging 已 locked，保留同两项检查与线性历史，禁止删除/强推且管理员不绕过；archive 已锁定，`delivery/staging` 已禁止删除/强推并要求对话解决。历史 `Deploy Sites & Backend` workflow ID `255149831` 已全局停用，新的 `Deploy Release Train` 是候选唯一发布入口。
...
- [x] **MP-PROD09C** staging 已验收业务语义收口：42 个核心商城/后台 Service 或 DTO 的 Git blob 精确固定到 `staging@acc0e08c`；Git 自动枚举的 21 个非 Delivery 运行时差异全部形成生产强化、Delivery 排除、测试订阅状态安全归一或格式/兼容说明，manifest blob 反向验证来源 commit，防止伪造。恢复 staging 的地址、购物车、预约/参团、客服、商品/推荐、个人资料、团购后台、组合商品、卖家发货、微信提现渠道查询、注销账号资金围栏、团购返还、分润/推荐、订单支付退款及任务奖励 fail-closed 测试。验证为后端 260/3134、真实 PG 5 suites/9、App 30/127+TS、Admin 12+build、Seller 12+build、miniapp 303+双构建、scripts 256；App/根 src/Delivery 改动 0。
- [x] **MP-PROD09D** 终审补强：Admin/Seller 手动静态单独发布必须先验证运行后端为同一 exact SHA；Website-only 在 SHA 不同时只允许完整 Git 历史证明候选与线上 SHA 的 `backend/` tree 完全一致，任何解析失败或 tree 差异均拒绝。Readiness guard 同时兼容生产 `ResultWrapper` envelope 与 flat 响应，使用 HTTPS、20 秒超时和真实 envelope 行为测试。小程序取货二维码首次持久化失败后，“重新生成”通过独立 retry version强制重跑本地 PNG 持久化，8 位短码继续兜底。对应 workflow 与 pickup contract 回归已补齐。
...
## 2026-09-15 微信自提交易发货闭环

- [x] 从最新 `origin/main@32aff591` 建立独立干净分支 `codex/pickup-wechat-shipping-20260915`，未使用原始脏 `staging`。
- [x] 定位生产自提核销已完成但微信支付单未收到 `logistics_type=4` 上报的根因。
- [x] 本地实现：同一微信支付单全部有效自提子订单核销后，在核销 Serializable 事务内创建统一“用户自提” outbox；微信调用继续在事务外可靠重试，快递链路保持不变。
- [x] 历史保护：没有系统 outbox 的旧自提订单不自动补报；管理端只允许重试既有 `FAILED` 自提任务。
