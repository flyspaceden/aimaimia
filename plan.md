...
- [x] **MP-PROD05**（Draft PR #1 已推送，未合并 main/未上传微信）完整 miniapp + production release-context；App 源码改动 0，不增加自提下单入口。小程序 lint/typecheck、55 files / 303 tests、staging+production build、72 页产物校验通过（总包 2.41 MiB、主包 1.262 MiB）；production 产物只含 `api.ai-maimai.com`/正式 WSS，无测试域名。已吸收 `origin/staging@acc0e08c` 删除无实际管理能力“微信授权说明”入口的最新小程序修复。正式 release-context 要求 HEAD 精确等于 `origin/main`，staging 精确等于 `origin/staging`；CI/手动构建只允许目标分支生成匹配 artifact。
