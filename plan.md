...
- [x] **MP-PROD09D** 终审补强：website/admin/seller 手动静态单独发布必须先验证运行后端为同一 exact SHA；小程序取货二维码首次持久化失败后，“重新生成”通过独立 retry version 强制重跑本地 PNG 持久化，8 位短码继续兜底。对应 workflow 与 pickup contract 回归已补齐。
