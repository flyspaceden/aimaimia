...
- [x] **H5-INV07** 微信内完成 H5 登录后，点击下载时由 Web Crypto 生成 10 分钟一次性下载凭证；系统浏览器原子消费后直达原有下载分发渠道，不重复显示登录/注册表单，也不重复处理推荐关系。数据库仅保存凭证哈希，申请按 H5 会话所属用户校验，消费使用未过期且未使用的 CAS 条件更新；候选值按 landing session 在微信 `sessionStorage` 中先行持久化，响应丢失后刷新仍复用同一值；同一凭证网络重试可复用且不延长首次登记起算的 10 分钟，过期/已使用才换新，不同并发凭证不能覆盖。2026-07-24 修正跨浏览器交接方式：不再依赖 History API 写入的 hash，而是进行真实导航到 `?handoff=`；系统浏览器立即清理该短时 query 后再消费，官网设置 `Referrer-Policy: no-referrer`，无本地 H5 登录上下文的伪造或残留 handoff 会被清理并恢复登录表单。
- [x] **H5-INV08**（2026-07-24 第三次根因修复）真机证明部分安卓微信的“在浏览器中打开”会重新打开二维码初始 `/invite/{code}`，真实导航后的 query 也不会传给系统浏览器。新增 `POST /invite-h5/download-pass/resume`：仅在同邀请码、可信代理解析的同出口 IP、同扩展屏幕上下文、同语言、同安卓大版本/可由 `Build/` 确认的设备型号、来源为微信且 2 分钟内完整候选集合恰好一个时，在事务内 CAS 消费；签发和恢复共用 advisory lock，避免唯一性检查期间又出现新凭证。0 个、多候选、不同设备或并发输家统一返回 `valid=false`。微信点击下载同时复制完整一次性下载 URL；网络等待后续签的新票据改由用户在遮罩按钮上显式复制。外部浏览器自动恢复失败时优先显示“继续下载”，支持用户点击读取或手动粘贴，只有明确选择“使用手机号登录”才展开登录表单。保留原 10 分钟哈希凭证和 `?handoff=` 兼容路径，不新增数据库 migration，不重复绑定推荐关系。
