...
- [x] **H5-INV07** 微信内完成 H5 登录后，点击下载时由 Web Crypto 生成 10 分钟一次性下载凭证；系统浏览器原子消费后直达原有下载分发渠道，不重复显示登录/注册表单，也不重复处理推荐关系。数据库仅保存凭证哈希，申请按 H5 会话所属用户校验，消费使用未过期且未使用的 CAS 条件更新；候选值按 landing session 在微信 `sessionStorage` 中先行持久化，响应丢失后刷新仍复用同一值；同一凭证网络重试可复用且不延长首次登记起算的 10 分钟，过期/已使用才换新，不同并发凭证不能覆盖；伪造或无本地 H5 登录上下文的 hash 会被清理并恢复登录表单。
