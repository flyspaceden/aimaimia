...
- [x] **MP-PROD03**（本地完成，未 push/未部署/未迁移生产库）小程序后端兼容闭包与 19 条 migration（18 条 staging-derived 同哈希 + 1 条生产集成自动退款副作用 outbox）；629 条生产集成基线路由（含 Batch 0 readiness）零删除，小程序新增路由为并列接口；微信支付/退款/提现、自提、通知、购物车、六类收货副作用 outbox 与自动退款数字资产/旧团长佣金冲回 outbox 已接入。Prisma validate/generate、Nest build、生产依赖 audit 0；启用真实 PostgreSQL 后端门禁后 241 suites / 2942 tests 全通过，0 skip。生产备份克隆库演练仍由 MP-PROD06 单独阻断。
- [x] **MP-PROD04**（本地完成，未 push/未部署）平台后台与卖家后台最小自提/核销/微信提现界面；配送门户入口为 0；扫码固定为 Node 20 兼容的 `@zxing/browser@0.1.5 + @zxing/library@0.21.3`。依赖锁只新增 4 个 ZXing 节点并把 `path-to-regexp 8.2.0→8.4.0`，未顺带升级 Axios/React Router/Vite 等约 100 个包。Admin CI 契约 12/12 + build + 修改文件 ESLint 0；Seller CI 契约 12/12 + build + 修改文件 `--max-warnings=0`，并恢复全量 lint 可运行（0 error，28 个既有 warning）。Node 24 本地扩展源文件契约为 Admin 28/28、Seller 35/35。卖家核销已用不可变 resolved credential + resolve session 消除“预览 A/核销 B”竞态；两端测试已接入 production CI。后端成功后才允许部署静态后台。
...
- [x] **MP-PROD05**（本地完成，未 push/未上传微信）完整 miniapp + production release-context；App 源码改动 0，不增加自提下单入口。小程序 lint/typecheck、55 files / 302 tests、staging+production build、72 页产物校验通过（总包 2.41 MiB、主包 1.262 MiB）；production 产物只含 `api.ai-maimai.com`/正式 WSS，无测试域名。正式 release-context 要求 HEAD 精确等于 `origin/main`，staging 精确等于 `origin/staging`；CI/手动构建只允许目标分支生成匹配 artifact。
...
- [x] **MP-PROD06** 生产备份克隆库迁移演练：最终候选生成含 source identity、dump SHA 和 101 migration 基线的原子 manifest 备份；同一备份恢复带 provenance 的 baseline/target，19 条新增 migration 全部成功，target 120/0。23 表主键+稳定整行哈希、历史默认值、退款10任务逐条、target120 checksum、baseline101 subset、final Git HEAD/clean/tree 全部通过，600权限 attestation 创建并复验成功。正式业务库仍为101/0、生产 SHA `a0f47810`、PM2 online、商品接口200；发布后需清理 rehearsal 资源。
