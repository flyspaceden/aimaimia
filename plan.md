...
- [x] **管理后台“积分成长”统一账户视图**（2026-07-04 新增并完成）
  ...
  - **实际做了**: 管理后台侧边栏从“普通成长”改为“积分成长”；顶部统计改为成长账户总数、普通用户 / VIP 用户数量、平台积分余额和平台成长值；账户页新增用户身份筛选，支持查看全部、普通用户或 VIP 用户。普通用户行继续展示普通分享码并支持启停，VIP 用户行只读展示 VIP 推荐码，并提供跳转到 VIP 会员详情和 VIP 奖励树的入口；账户表统一使用“积分余额”列名，手动扣减错误也改为通用积分文案，避免 VIP 行出现“普通积分”误导；行为规则列表隐藏内部英文行为码，将“接入状态”和“状态”合并为“生效状态”开关，已接入规则可直接切换生效/停用，未接入规则禁用开关并提示未接入，适用用户改为中文展示；成长等级列表隐藏内部等级编码，只保留管理员实际要维护的等级名称、成长阈值、展示称号、月兑换上限和启用状态；积分兑换项的红包选择改为“兑换后发放的红包”，只展示启用中、未发完、可由系统直接发放的红包活动，并由后端阻止启用中的兑换项绑定用户主动领取类红包；“普通分享”Tab 保持只服务普通用户拉新，避免和 VIP 推荐体系混用。后端管理接口同步支持 `userType=ALL|NORMAL|VIP`，看板返回普通 / VIP 账户数，并在用户摘要中返回 `vipReferralCode`。
  - **验证**: 新增后端回归测试覆盖 VIP 成长账户可被查询但不暴露普通分享码、手动扣减使用通用积分余额错误、积分兑换不能绑定用户主动领取类红包；新增管理端静态回归测试覆盖“积分成长”命名、用户身份筛选、VIP 推荐码只读展示、跳转 VIP 页面、不新增 VIP 推荐 Tab、行为规则不展示内部行为码、成长等级不展示等级编码、行为规则状态列不重复、行为规则生效状态使用开关且未接入规则不可切换、积分兑换只展示可由系统发放的红包活动。已运行 `cd backend && npm test -- admin-growth.service.spec.ts --runInBand`、`node --test scripts/__tests__/normal-growth-ui.test.mjs`、`npx tsc --noEmit --pretty false`、`cd backend && npm test -- --runInBand`、`cd backend && DATABASE_URL='postgresql://user:password@localhost:5432/nongmai_validate' npx prisma validate`、`cd backend && npm run build`、`cd admin && npm run build`、`npm test`、`git diff --check`。
