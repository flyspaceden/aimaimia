...
- [ ] **MP-PROD05B** 推荐 H5 双入口（2026-08-24 已单独批准，独立候选实施中）：复用旧 staging 已验证的 `InviteChoiceLanding`，基于最新 main 只移植 website 路由/页面、客户端交接语义、测试与文档；保留旧后端 Auth 路由作兼容但新页面不调用。App 旧 H5 登录漏斗源码同步下线，但不随 website 部署冒充已发布 OTA。候选需先进入 `staging-next` 做普通/VIP 真机扫码，再以同一业务树进入 main。
...
- [x] **MP-PROD09D** 终审补强：website/admin/seller 手动静态单独发布必须先验证运行后端为同一 exact SHA；readiness guard 同时兼容生产 `ResultWrapper` envelope 与 flat 响应，使用 HTTPS、20 秒超时和真实 envelope 行为测试，不能因读取错层级而永久拒绝。小程序取货二维码首次持久化失败后，“重新生成”通过独立 retry version 强制重跑本地 PNG 持久化，8 位短码继续兜底。对应 workflow 与 pickup contract 回归已补齐。
