...
- [x] **DR05** 管理后台：积分成长页收敛为独立积分成长资产页，只管理普通/VIP 用户的积分、成长值、等级、行为规则、积分兑换和流水；新增“推荐与拉新”页面承接普通推荐关系、普通分享绑定状态、失效原因、VIP 推荐码查看和自动 VIP 转化设置；用户管理页新增“推荐码”列，按用户当前身份展示普通分享码或 VIP 推荐码并支持二维码/链接查看；普通/VIP 系统配置页展示对应直推比例。
