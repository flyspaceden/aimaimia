...
- [x] **QR08** 管理后台：独立“订单队列奖励”参数页，10 参数原子保存、权限控制、开启影响确认与利润口径提示。
- [x] **QR09** 最终收口：硬滑动窗口、配置资源上限、利润异常不推进、多次部分退款累计调账、已提现/提现中追偿、注销守恒、生鲜售后窗口、释放批次饥饿、状态/铃声稳定游标及失败 outbox 重驱均已修复；补充不写数据库的 21→22 人虚拟演示和队列配置实时利润安全闸门；后端 275 套件 / 2935 项、App Jest 38 套件 / 145 项、根契约与法务 207 项、Prisma validate、后端/Admin 构建、App TypeScript、队列后台页面 ESLint 和三路独立复审全部通过。
