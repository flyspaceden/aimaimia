...
- [ ] **DR09**（2026-07-10）恢复付费 VIP 礼包一次性推荐奖：修复 7 月 5 日统一推荐关系改造误删 `activateVipAfterPayment()` 授奖调用的问题；金额使用 `VipPurchase.amount × referralBonusRate` 购买快照，原 Serializable 激活事务内发放；增加同一 `VipPurchase.id` 防御性查重；累计消费自动升级仍不发一次性奖；对已确认漏发的 AIMM00000000000119 → AIMM00000000000032 单独幂等补发 51.87 元。
