...
- [x] **管理后台企业安全删除与生产标签修复**（2026-08-20）
  ...
  - **实际做了**: 新增 `DELETED` 软删除状态、删除前业务阻塞检查（结算会话只统计仍在有效期内的 `ACTIVE` 会话）、企业名称二次确认、独立 `companies:delete` 权限、回收站恢复与卖家会话失效；买家公开企业/商品/购物车/关注/预约/考察团/推荐/团购/结算统一拦截非 `ACTIVE` 企业。新迁移和 production bootstrap 幂等补齐 5 类企业标签 + 1 类商品标签及 `tags:read/manage` 权限，仅清理无标签的空占位类别，不运行生产禁用的 demo seed。
