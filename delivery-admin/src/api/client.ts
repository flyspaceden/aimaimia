...
// 请求拦截：附加 delivery admin JWT
client.interceptors.request.use((config) => {
  const token = localStorage.getItem('delivery_admin_token');
  if (token) {
...
const clearAuthAndRedirect = () => {
  ...
  localStorage.removeItem('delivery_admin_refresh_token');
  // 清除 zustand persist 缓存
  localStorage.removeItem('nongmai-delivery-admin-auth');
...
client.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      !originalRequest.url?.includes('/delivery-admin/auth/refresh') &&
      !originalRequest.url?.includes('/delivery-admin/auth/login') &&
      !originalRequest.url?.includes('/delivery-admin/auth/logout')
    ) {
      originalRequest._retry = true;
      const refreshTokenStr = localStorage.getItem('delivery_admin_refresh_token');
      ...
      try {
        ...
        const res = await axios.post(
          `${import.meta.env.VITE_API_BASE_URL || '/api/v1'}/delivery-admin/auth/refresh`,
        ...
        // 更新本地存储
        localStorage.setItem('delivery_admin_token', accessToken);
        if (newRefreshToken) {
          localStorage.setItem('delivery_admin_refresh_token', newRefreshToken);
        }
