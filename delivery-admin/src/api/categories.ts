...
export const createCategory = (data: {
  ...
  sortOrder?: number;
  returnPolicy?: 'RETURNABLE' | 'NON_RETURNABLE' | 'INHERIT';
}): Promise<AdminCategory> => client.post('/delivery-admin/categories', data);

/** 编辑分类 */
export const updateCategory = (
  id: string,
  data: { name?: string; sortOrder?: number; returnPolicy?: 'RETURNABLE' | 'NON_RETURNABLE' | 'INHERIT' },
): Promise<AdminCategory> => client.put(`/delivery-admin/categories/${id}`, data);
...
export const toggleCategoryActive = (id: string): Promise<AdminCategory> =>
  client.post(`/delivery-admin/categories/${id}/toggle-active`);

/** 批量排序 */
export const batchSortCategories = (
  items: { id: string; sortOrder: number }[],
): Promise<void> => client.put('/delivery-admin/categories/batch/sort', { items });
