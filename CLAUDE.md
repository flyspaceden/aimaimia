...
| 账号注销 | **即时注销、不可撤销（2026-06-04 改版，取消冷静期）**：通过身份核验后提交即在单个 Serializable 事务 + advisory lock `AD-${userId}` 内同步完成——清零作废全部资产（钱包/积分/红包/优惠券/抽奖/未消费礼包/冻结分润 + **可提现现金，归平台，用户书面接受法律风险**）+ 清关联表（购物车/收藏/关注/AI会话/地址软删）+ 个人资料软删（昵称→"已注销用户"、phone/email/avatar 清空）+ AuthIdentity 改 `deleted:${userId}` 释放手机号 + 强制登出（revoke 所有 Session）+ `status=DELETED`；已付款订单继续履约不退款（依《消保法》§24）、进行中售后继续受理（退款走原支付通道回支付宝/微信）；VIP 节点保留（下级链路不受影响，未发放分润归平台）；OWNER 必须先转让企业；推荐码失效（代码层判 `deletionExecutedAt`）；Order/Invoice/RewardLedger/VipTreeNode/ReferralLink 依法保留（3年/5年/6月）；身份核验：绑手机号用 `SmsPurpose.DELETION` 短信验证，仅微信者弹窗输入"确认注销"；无 cron / 无冷静期横幅 / 无 cancel 接口。详见 `docs/superpowers/specs/2026-06-04-account-deletion-immediate-design.md` |
...
10. **推送 GitHub 前必须向用户确认 + 保持版本可回退**：
    ...
    - **App（`app/` 下）OTA 同样要先问**：push 只触发 GitHub Actions（workflow 中没有 app 部署，见当前 `.github/workflows/deploy-release.yml`），买家 App 上线必须走 EAS，是否发 OTA 由用户决定
    - **版本回退友好**：
    ...
    - **具体操作规则不在此重复**，以下文件为真相源：
      - `.github/workflows/deploy-release.yml` — 当前唯一受控发布入口；历史 `.github/workflows/deploy-website.yml` 已全局停用
