...
| QR08 | 配置比例把平台利润扣成负数，或管理员保存前看不到队列叠加后的真实利润影响 | 🔴 HIGH | 1%–10% 单键校验；配置快照最终态校验队列比例不得超过普通与 VIP 平台比例；利润安全预览把启用的队列比例纳入所有买家/推荐人组合。队列配置页实时展示普通/VIP平台份额扣除前后值与最弱场景安全余量，预检未完成或不安全时禁用保存；后端 Serializable 校验继续作为不可绕过的最终闸门 | ✅ 已检查 |
