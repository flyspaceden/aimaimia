...
| CAP04 | 团队池结算总额与实际出账流水不一致 | 🔴 HIGH | 审查发现月度团队池 1% 中普通成员 60% 会生成结算流水，但 `CaptainMonthlySettlement.totalAmount/teamPoolAmount` 只统计团长 40%，导致管理后台税前合计低于实际会支付的流水总额。已修：结算 `teamPoolAmount/totalAmount` 按实际生成的团长 40% + 成员 60% 流水合计，`meta.teamPoolSummary` 单独记录理论池、团长份额和成员份额；审核生成流水时团长只记自己的 40%，成员按分配明细入账。 | ✅ 已修复 |
