...
| 编号 | 风险 | 级别 | 修复/边界 | 状态 |
|---|---|---|---|---|
| WMC01 | 两个商品的数量请求或旧 GET 乱序返回时，用整份购物车快照互相覆盖，造成其他商品数量跟着变化、数值来回跳 | 🔴 HIGH | 小程序数量写入改为按 `cartItemId` 独立串行队列；不同商品可并行，同一商品快速点击合并为最新目标值。数量接口仅返回该行确认，不再用 mutation 的整车响应覆盖缓存；每次点击先取消旧购物车 GET，全部队列空闲后才拉一次权威整车。写入失败立即回到该行最后一次服务端确认数量，权威 GET 再失败也不保留错误乐观值。 | ✅ 快速点击、跨商品乱序、失败回滚与响应串线回归通过；待 staging 真机复测 |
