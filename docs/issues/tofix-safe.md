...
## 2026-09-04 图片 Agent v3 配置一致性（本地修复，未部署）
...
- PVA-V3-DOWNLOAD：候选下载拒绝越权、退役及拒绝结果；校验原字节哈希、格式、尺寸、受管路径及读后状态，附件private/no-store，不走公开URL或重新生成。22项后端单测/HTTP测试和Chromium下载重试通过，仅本地。
- PVA-V3-CONFIRM-RACE：真实全 Schema 数据库四并发确认复现 P2034；仅对明确事务回滚最多五次重试，单一冻结与注入失败全回滚测试通过。任务历史按公司/商品/接入范围隔离，外域游标拒绝。仅本地修复，待测试部署与真实链路验收。

- PVA-V3-CONFIG：测试自动初始化可能覆盖管理员暂停、价格或模型；改为相同配置锁下仅创建缺失版本，既有配置保持权威。
- PVA-V3-SNAPSHOT：确认后按实时费率模型执行可能与报价不符；提交和查询均使用不可变报价模型快照。缺失快照不推测模型，未提交释放，已绑定进入对账。
- PVA-V3-FREE：新免费合约不依赖 OCR，也不把失败当作无文字证据；新上传图仅取消预先公开挂图要求，保留商品/商家/计划/源哈希检查。运行前核对下载字节，旧任务按持久化版本处理。41 项相关测试及独立审查通过，未部署。
- 配置与执行专项 31 项通过，已由独立 Agent 审查并由主 Agent 复核；部署与真实页面验收另行记录。
...
- 图片条码扫描把三通道 RGB 当单通道亮度，导致清晰 EAN-13 漏检：已本地修复两个入口并增加真实像素正例。尚未部署。
- 商品棚拍实测改变了手表表冠位置而系统仍允许商家确认采用：待修。仅图像宽高比例及 QR/条码检查无法证明商品结构一致，需要独立结构对比或保留主体像素的合成方案；不通过平台预审批解决。
- 免费调优要求条码 NONE，而当前识别器只有 DETECTED/INCONCLUSIVE：功能不可达，待调整免费处理合约；不可通过伪造 NONE 绕过。
...
| 编号 | 风险 | 级别 | 修复/边界 | 状态 |
|---|---|---|---|---|
| APC01 | 修改加价率只改配置、不改 SKU，后台编辑预览与 App/结算真实售价分裂 | 🔴 HIGH | `MARKUP_RATE` 单项、批量和版本回滚统一先生成带数据指纹的影响预览；确认 token 未过期后，在利润安全 advisory lock + Serializable 事务内同时写配置、重算非平台非草稿 SKU，并同步 `Product.basePrice/cost`。 | ✅ staging / production 已验收 |
| APC02 | 审计日志单项回滚加价率绕过价格影响确认 | 🔴 HIGH | 禁止从通用审计日志回滚 `MARKUP_RATE`，必须进入平台设置版本历史，按目标版本加价率重新预览并同步商品价格。 | ✅ staging / production 已部署 |
| APC03 | 卖家创建读取 60 秒缓存或客户端 `basePrice`，管理端普通商品手填 `price`，平台员工又从卖家端覆盖奖励价 | 🔴 HIGH | 卖家创建与配置更新共用利润安全事务锁并直接读事务快照；普通商品 DTO 不再接受客户端 `basePrice/price`，管理端按成本自动计算并展示当前价/保存后价；平台公司所有卖家商品写入口 fail-closed，只能走独立奖励商品模块人工定价。 | ✅ staging / production 已部署 |
| APC04 | 删除最低价 SKU 前先汇总 `basePrice`，留下已停用规格的旧最低价 | 🟠 HIGH | SKU 更新先完成新增/更新与旧规格停用，再从剩余 ACTIVE SKU 汇总最低售价和最低成本。 | ✅ staging / production 已部署 |
...
| APC06 | 一次性数据修复误触生产、预览与执行集合漂移或失败后留下半套价格 | 🔴 HIGH | `products:reprice` 默认 dry-run，逐 SKU 输出新旧价格、回滚 SQL 和 preview token；执行必须显式携带当前加价率与 dry-run token，在共享利润安全 lock 内重建同一集合并校验最终 SKU 经济数据，token 不一致即拒绝；MARKUP_RATE 缺失/非法一律 fail-closed。ACTIVE/INACTIVE SKU 一起重算，写后再次扫描必须为 0 不一致。 | ✅ staging / production 已执行，剩余不一致为 0 |
...
## 2026-09-09 资金管理页面拆分安全复核

| 编号 | 风险与根因 | 修复与验证 | 状态 |
|---|---|---|---|
| FUND-UI-01 | 服务器 5xx 或断网可能发生于记账完成后，误当明确失败会允许改动原请求 | 客户端保留 HTTP 状态；结果未知锁定表单、原输入与幂等键，原键重试；Playwright 覆盖 503 后同键同内容重试 | 本地已修 |
| FUND-UI-02 | 更正旧付款单取消成功、新单创建失败时可能误认为全部失败 | 显示两阶段实际状态；旧单已取消后重试只创建新单，不再次取消；用明确 409 拒绝覆盖失败路径 | 本地已修 |
| FUND-UI-03 | 只读管理员通过银行流水搜索推断敏感凭证 | 精确流水筛选与统一关键词银行流水匹配均使用付款/冲正权限裁决；PostgreSQL 覆盖只读拒绝 | 本地已修 |

| FUND-UI-04 | 付款详情的确认/取消/冲正/回款在未知响应后重开窗口会换键 | 保留各操作完整输入及原键，请求中与未知态禁改表单/凭证和关闭操作，未知态只允许原请求重试；四种动作均补 503 回归 | 本地已修 |

本轮仅增加读查询与界面操作，写入仍调用既有 Serializable/幂等资金服务，不变更资金核心或历史数据。

## 2026-09-15 微信自提交易发货安全检查

| 编号 | 风险与根因 | 本地修复与边界 | 状态 |
|---|---|---|---|
| WX-PICKUP-01 | 商城核销已把自提订单改为 `PICKED_UP/RECEIVED`，但微信交易发货 outbox 明确排除 `PICKUP`，导致微信支付单长期停在“待发货”、资金无法进入结算 | 最后一个有效自提子订单核销时，在同一 Serializable 事务创建 `logistics_type=4` 的统一发货 outbox；HTTP 仍在事务外重试。未全部核销不提前上报，混合履约/缺关系 fail-closed，快递上报不变 | 本地已修并通过独立审查，待发布验收 |
| WX-PICKUP-02 | 直接开放自提“重新上报”会把已经在微信后台手工处理的历史订单再次发货，消耗仅有的重新发货机会 | 只有系统已创建且 `FAILED` 的自提 outbox 可从管理端重试；历史无 outbox 订单不自动回填、不提供通用补报 | 本地已修并通过独立审查，待发布验收 |
| WX-PICKUP-03 | `WechatShippingOutbox.payload` 存入 JSONB 后对象键顺序变化，直接 `JSON.stringify` 哈希会把同一快照误判为篡改并阻止发送 | 哈希统一改为递归排序对象键的规范化 JSON；数组顺序保持业务语义。真实 PostgreSQL 写入/读回和 worker 发送测试锁定该行为，旧算法任务会先按权威数据重建为新哈希 | 本地已修并通过独立审查，待发布验收 |

独立复审未发现新增 Critical/High/Medium。真实 PostgreSQL 18 在完整 126 条迁移上验证：同一支付会话两个子订单并发核销经 Serializable/P2034 重试后最终恰好一个 outbox；JSONB 写入读回后完整性哈希稳定，worker 成功收口。
