...
| 编号 | 风险 | 级别 | 修复/边界 | 状态 |
|---|---|---|---|---|
| WMC01 | 两个商品的数量请求或旧 GET 乱序返回时，用整份购物车快照互相覆盖，造成其他商品数量跟着变化、数值来回跳 | 🔴 HIGH | 小程序数量写入改为按 `cartItemId` 独立串行队列；不同商品可并行，同一商品快速点击合并为最新目标值。数量接口仅返回该行确认，不再用 mutation 的整车响应覆盖缓存；每次点击先取消旧购物车 GET，全部队列空闲后才拉一次权威整车。写入失败立即回到该行最后一次服务端确认数量，权威 GET 再失败也不保留错误乐观值。整车查询按 `createdAt ASC, id ASC` 固定排序，刷新不能再让商品卡片换位。 | ✅ 快速点击、跨商品乱序、失败回滚、响应串线与稳定排序回归通过；待 staging 真机复测 |
| WMC02 | 数量写入刚开始但 React 尚未重渲染时，用户继续清空、删除、勾选或结算，形成前端 TOCTOU | 🔴 HIGH | 结构操作使用同步 ref 写屏障；数量协调器同步暴露 pending，清空、删除、勾选和结算在事件执行时再次检查，不只依赖下一次 render 的 `isPending`。单个商品更新不再禁用其他商品加减按钮。 | ✅ 小程序契约测试与双构建通过；待 staging 真机复测 |
| WMC03 | 客户端继续以 `skuId` 作为购物车行身份，奖品/普通行或跨账号构造 ID 时可能更新错误对象 | 🔴 HIGH | 新增 `PUT /cart/item-ids/:cartItemId`；服务端在 Serializable 事务内强制 `{id, 当前用户cartId, isPrize:false}` 后再校验 SKU、企业、库存和限购。旧 `skuId` 路由保留给 App/旧客户端。历史 `20260228130000` partial unique index 已复核存在，不重复建迁移。 | ✅ 后端 31 项购物车回归与 PostgreSQL 16 双商品并发测试通过；待部署验收 |
