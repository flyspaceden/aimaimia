...
## 2026-07-24 微信 H5 下载跨浏览器恢复安全检查

| 编号 | 风险 | 级别 | 说明 | 状态 |
|------|------|------|------|------|
| H5D01 | 同 Wi-Fi、同型号手机被错误识别为已登录用户 | 🟠 HIGH | 自动恢复只用于公开 App 下载，不返回用户或登录信息；后端要求同邀请码、可信代理解析的同出口 IP、同扩展屏幕上下文、同基础语言、同安卓大版本/可由 `Build/` 确认的设备型号、来源 UA 为微信、目标 UA 为非微信且签发后 2 分钟内恰好一个候选。候选查询不设 `take` 截断，必须检查完整有效集合；0 个或多个候选均拒绝猜测，转剪贴板高熵凭证。 | ✅ 已检查 |
| H5D02 | 自动恢复或直接凭证被并发重复消费 | 🟠 HIGH | 两条消费路径最终都使用 `downloadPassUsedAt=null + expiresAt>now + authedUserId not null` 的单语句 CAS；下载凭证签发和设备恢复共用事务级 advisory lock，候选读取期间不能插入新的有效凭证；两个浏览器并发时仍只有一个 `count=1`。 | ✅ 已检查 |
| H5D03 | 公开恢复接口泄露登录状态或可换取登录 Token | 🟠 HIGH | 接口仅返回 `valid:boolean`，不返回用户、landing session、推荐关系或失败原因；限流 20 次/分钟。下载凭证只授权公开下载，不能换取买家 Token；剪贴板读取必须由用户主动点击，且只解析合法 256 位票据。 | ✅ 已检查 |
| H5D04 | 伪造 `X-Forwarded-For` 冒充同设备出口 IP | 🟠 HIGH | controller 不再直接信任请求头首项，统一使用 Express 按 `TRUST_PROXY` 可信代理链解析后的 `req.ips[0] / req.ip`；生产 Nginx 仍按单一可信反代配置。 | ✅ 已修复 |
