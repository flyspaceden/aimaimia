...
| 软件 | 版本 | 用途 |
|------|------|------|
| Node.js | >= 20.9.0（推荐 22 LTS） | NestJS 11 与 sharp 0.35 运行时；发布脚本低于该版本会 fail-closed |
| PostgreSQL | 15+ | 主数据库 |
| Redis | 7+ | 队列/缓存 |
...
### 2026-08-21 企业安全删除与正式标签目录（发布中）

- **范围**：main 仅移植企业软删除/回收站、企业标签选择、5 类企业标签 + 1 类商品标签目录，以及软删除所需的公开可见性守卫；不合并 staging 上的小程序、资金闭环或面单功能历史。
- **数据库**：新增兼容性迁移 `20260821020000_add_company_soft_delete_and_production_tags`；增加 `CompanyStatus.DELETED`、可空删除字段、标签目录和权限数据，不删除交易历史。干净 PostgreSQL 16 已验证 101 条 main migration 全部可顺序应用。
- **部署防护**：生产后端安装后校验顶层依赖与 Prisma/Nest CLI，官方源不完整时切镜像；Prisma 禁止隐式下载，并改为服务器构建成功后才执行 migration。
- **验收边界**：代码推送、GitHub Actions、生产 migration/PM2/公网 API 标签目录与管理后台页面需分别核验；管理员真实删除/恢复企业仍需用无业务测试企业人工验收。
