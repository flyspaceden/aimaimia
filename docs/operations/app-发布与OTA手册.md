...
生产 OTA 完整历史（第 1-49 条，含 Group ID / commit / 内容）以 memory `project_app_release_status.md` 的「Production Branch」节为权威，或跑 `eas update:list --branch production` 查询。

最近一条（2026-07-05，第 49 条）：首页/我的页模块换位（staging `00abad43` / main `ee979b3b`）——首页原 VIP 礼包轮播位置改为用户信息卡片，保留头像框、用户编号复制、扫一扫/编辑、推荐码入口和数字资产排行榜入口；「我的」页顶部改为 VIP 礼包轮播，订单、钱包/VIP 卡和常用工具顺序保持不变。runtime 1.0.5 production OTA：Group `cc033e44-f30f-40a6-898a-5647de8d426e`，Android update `019f30f5-9310-7334-9706-ab2efa6d285f`，iOS update `019f30f5-9310-7f6a-98fd-01fac4e2938f`；EAS Dashboard `https://expo.dev/accounts/flyspaceden/projects/ai-aimaimai/updates/cc033e44-f30f-40a6-898a-5647de8d426e`；命令带全 5 个生产 `EXPO_PUBLIC_*`（ENV=production、USE_MOCK=false、API_BASE_URL=api、ALIPAY_SANDBOX=false、WECHAT_PAY_AVAILABLE=true）。发布前确认 `app.json` version/runtime 为 `1.0.5`，本次 staging/main 都从干净 `origin/staging` / `origin/main` worktree 发布，只提交本次 8 个目标文件，未改 `app.json` / `eas.json` / `plugins/` / `android/` / `ios` / `package.json` / `package-lock.json`；staging 与 main 发布工作树均通过 `node --test scripts/__tests__/home-layout-order.test.mjs scripts/__tests__/me-identity-card-layout.test.mjs scripts/__tests__/home-vip-promo-copy.test.mjs`（10/10）、`npx tsc --noEmit --pretty false`、`git diff --check`，并完成触达文件响应式风险扫描；production Android 预检 `expo export --platform android --clear` 通过，输出 `43 assets / 1 bundle`，Android launch HBC `_expo/static/js/android/entry-273ad3a0a548dfa211b012a76f24dcf6.hbc` 约 `8.79 MB`；EAS publish 输出 Android/iOS 均为 `46 assets`，Android launch HBC `_expo/static/js/android/entry-273ad3a0a548dfa211b012a76f24dcf6.hbc` 约 `5.68 MB`，iOS launch HBC `_expo/static/js/ios/entry-97e033113b65ba5eaeb5db45fd81ab16.hbc` 约 `5.68 MB`；发布后 `npx eas-cli@latest update:list --branch production --limit 3` 确认该 Group 为 production branch 最新记录。注意：本条只覆盖 runtime **1.0.5**，不会覆盖仍在 runtime 1.0.4 / 1.0.3 / 1.0.2 / 1.0.1 的已安装包；已安装 1.0.5 的用户需要冷启动后拉取 OTA，下一次冷启动生效。若需紧急回滚，可 `npx eas-cli@latest update:republish --group 015237bd-d1d7-4fa2-91dd-704701a73242 --destination-branch production --message "回滚首页/我的页模块换位 OTA：恢复第48条 VIP 成长说明版本" --non-interactive`。
