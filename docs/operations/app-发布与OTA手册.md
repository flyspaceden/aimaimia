...
| Group ID | 内容 | 备注 |
|---|---|---|
| `ce03bc35-13a4-42b6-8a29-d6e303b3156a` | 售后退款状态同步修复：①买家 App 售后详情在 `REFUNDING` 时 10s 轮询，进入 `REFUNDED/FAILED/REJECTED` 后停止并刷新订单/钱包/售后缓存；②新增 `afterSaleRefundSync` 纯函数与单测，统一退款中/终态判断；③配套后端售后退款 pending 短延迟查单 + 管理后台详情弹窗 15s 查单随 staging commit 自动部署 | **当前生效** ✅，commit `6ebae51`（2026-06-01），App 端纯 JS/TS 无原生改动；后端/admin 依赖同 commit 的 staging 部署完成后生效；Android update `019e8186-1ddb-7784-bee7-b558b4ca42dd`，iOS update `019e8186-1ddb-71e0-93e7-c9ddc3d698c3`；带 `EXPO_PUBLIC_ENV=staging` + `EXPO_PUBLIC_ALIPAY_SANDBOX=true` + `EXPO_PUBLIC_WECHAT_PAY_AVAILABLE=true` |
