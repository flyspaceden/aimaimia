...
| Group ID | 内容 | 备注 |
|---|---|---|
| `8c1f0257-947f-4a47-b27e-f7300d45191e` | VIP 购买页底部 BlurView tint 修价格看不清：`app/vip/gifts.tsx:605` `BlurView intensity={40}` 从 `tint="dark"` 改为 `tint="light"`——金底上 dark tint 在模糊层叠暗化效果，把深金价格 `#B8860B` 和深棕标签 `#3D2E1A` 都压成对比度不足；light tint 让模糊层倾向亮色，深色文字恢复正常显示 | **当前生效** ✅，commit `1e081be`（2026-05-27），纯 JS/TS 无原生改动，无后端依赖；Android update `019e6bd9-58b2-7af5-af6b-1b0da1796681`，iOS update `019e6bd9-58b2-7d63-986f-d8e4aaf18f82`；带 `EXPO_PUBLIC_ENV=staging` + `EXPO_PUBLIC_ALIPAY_SANDBOX=true` |
