...
- 微信开发者工具固定目录只能在远端 staging 已验证后同步；同步后必须干净且 `HEAD == origin/staging`。
...
三个远端 SHA 均须等于 `OLD_STAGING_SHA`，任一步失败会立即停止。此步骤本身也需用户批准；本轮只记录方案，不自动执行。

新的通用商城 `staging-next` 全量验收后，替换和固定微信目录重新绑定也必须单独获批。执行时由操作者先把已验证的 40 位 SHA 显式写入 `NEW_STAGING_SHA`；下面命令会用旧 tip 作 lease，远端已漂移时自动拒绝：
...
4. 生产发布通过 `Deploy Sites & Backend` 的 `workflow_dispatch` 手动触发，并填写显式生产确认；workflow 绑定 exact SHA 和 production environment approval。
