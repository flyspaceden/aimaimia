...
├── delivery-admin/ → 配送管理后台（delivery-admin.ai-maimai.com）
...
| 分支 | 部署到 | 用途 |
|------|--------|------|
| `staging` | **测试环境** test-admin/test-seller/test-delivery-admin/test-delivery-seller/test-api.ai-maimai.com | 功能测试、回归验证 |
| `main` | **生产环境** admin/seller/delivery-admin/delivery-seller/api.ai-maimai.com | 真实用户使用 |
...
- 改了 `delivery-admin/` → 部署到 **https://test-delivery-admin.ai-maimai.com**
- 改了 `seller/` → 部署到 **https://test-seller.ai-maimai.com**
- 改了 `delivery-seller/` → 部署到 **https://test-delivery-seller.ai-maimai.com**
- 改了 `backend/` → SSH 到服务器更新 `aimaimai-api-test`（PM2），自动跑主库 `prisma migrate deploy` + 配送库 `prisma migrate deploy --schema prisma-delivery/schema.prisma` + reload
...
- 配送管理后台：https://test-delivery-admin.ai-maimai.com
- 配送中心：https://test-delivery-seller.ai-maimai.com
...
- `delivery-admin/` → https://delivery-admin.ai-maimai.com
- `seller/` → https://seller.ai-maimai.com
- `delivery-seller/` → https://delivery-seller.ai-maimai.com
- `backend/` → 服务器 `aimaimai-api-prod`（PM2），自动跑主库 + 配送库 Prisma migrate，然后 reload
...
2. **deploy_target**：
   - `all` — 部署全部（website + admin + delivery-admin + seller + delivery-seller + backend）
   - `website` / `admin` / `delivery-admin` / `seller` / `delivery-seller` / `backend` — 只部署其中之一
...
- 数据库 migration（`backend/prisma/migrations/` 和 `backend/prisma-delivery/migrations/`）会随 backend 部署自动执行，**不会有交互提示**，写 migration 时必须保证向后兼容
