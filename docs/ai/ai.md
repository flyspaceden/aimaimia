...
- 2026-09-03 全测试商家默认开放（本地候选）：当 `AI_VISUAL_AGENT_TEST_ACCESS_ENABLED=true`、`AI_VISUAL_AGENT_TEST_ALL_MERCHANTS_ENABLED=true` 且 API hostname 精确为测试域名时，任一 ACTIVE 测试商家的 OWNER/MANAGER 点击“查看可用方案”会先走带卖家审计的显式 POST，再幂等创建该商家/人员/商品/费率卡的六层精确策略并发放欢迎图片积分；费率卡 GET 保持只读，不使用通配权限。按测试要求，日、周及平台共享预算使用数据库整数范围内的等效无限上限，不限制调用次数；每次调用仍需商家明确确认图片积分。自动模式使用独立费率编码，并在列卡、报价和确认阶段持续检查总开关；关闭后立即阻止新调用，每分钟在报价锁内只释放 `RESERVED + invocationId=null` 的自动报价，不妨碍已被 Provider 接受的任务恢复。生产环境两个测试开关必须保持 false。
