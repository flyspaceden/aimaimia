...
| `flavorTags`（界面显示“标签”） | 适度人工维护 | 商品标签 / 属性 | 界面使用跨品类名称；内部字段保留，既可记录 `防水 / 便携`，也可记录 `鲜甜 / 脆甜` |
| `seasonalMonths`（界面显示“月份（选填）”） | 半自动 | 分类规则 + 人工覆盖 | 界面不再限定“应季”；内部仍保存月份数字，未适用的商品可不填 |
...
> **界面命名边界（2026-08-21）**：Web 后台把 `flavorTags / seasonalMonths / usageScenarios / dietaryTags` 分别展示为“标签 / 月份（选填）/ 场景 / 属性”；staging 独有的配送卖家中心同步相同文案。本轮只中性化页面名称和示例，不重命名字段、不改 DTO、数据库或现有推荐计算。
...
## 17. AI Visual Agent（v3 重构设计，整体验收未完成）

2026-09-04 设计更新：以 [商品图片智能美化 Agent v3](../superpowers/specs/2026-08-22-universal-product-visual-agent-design.md) 为当前设计依据，旧记录中与新设计冲突的“免费必须空事实 OCR”“营销结果仅私密预览”“浏览器驱动任务完成”等不再作为目标方案。既有测试部署为 4b5031aa、本地条码修复为 42d9117a（本次审查快照）；本次文档修订不修改运行状态。

实施顺序为 P0 服务端任务/恢复/报价/配置 → P1 免费和主体保留背景 → P2 精修、创意、结构检查和低清路由 → P3 积分购买及 SDK/Webhook → P4 同一候选真实验收。P0/P1 已开始本地实施；模型选型及购买积分条款须按设计补齐。下方历史的“已完成”只代表对应切片，不代表整体产品验收通过。

### 2026-09-04 v3 本地实施：结构检查 Core（实际效果未验收）

通用API接线收口：先把输出保存为隐藏PENDING，再从受管对象读取并检查；poll/GET/采用共同检查结构终态，父生成账单人工结算不能放行新v2 PENDING。明确OCR冲突拒绝，UNKNOWN不结算/不采用；完整已结算v1才保留历史兼容，未知/损坏报告不当legacy。通用调用不借用爱买买测试预算，使用自身六层scope；透明检查副本与生成器白色matte一致。限定独立终审确认2 High+1 Medium已修，针对37项通过。

本轮整合：后端307套/3617项通过，22项条件数据库测试默认跳过；单独新建PostgreSQL16空库141条迁移全部成功，确认冻结/幂等/故障回滚3项通过，另有Core隔离租约和结构9项先前实跑证据。商家浏览器4组及管理员浏览器1组通过（受控接口；上传为真实XHR），Admin/Seller构建通过。本轮未push/部署，未实际调用百炼结构模型。

商品接线补充：付费结果调用结构runner，固定短幂等键绑定报价和两图；检查执行未完成/基础设施未知标PENDING，不能抢先结算和完成候选。明确结构FAIL或OCR文字不一致拒绝候选；报告、比较调用ID、账务异常状态写入候选证据。服务暂停仍先读缓存FAIL；大图缩到1024再编码，避免整图PNG膨胀导致检查被跳过。新报价从服务端商品标题/分类仅推导结构focus枚举并绑定quoteHash，手表专项不可用NOT_APPLICABLE冒充通过；旧报价显式GENERAL。通用公共API接线另行收口。

透明图比较副本使用与图像生成器一致的白色matte，避免直接移除alpha显露隐藏RGB造成错误比较；不写回原图、不改变不透明实景背景。专项10项通过并独立快审，未部署。

后台配置补充：staging且全商家模式及结构运行开关均启用时，只创建缺失的六层结构预算（保守预占1分，不是实际费用），不覆盖管理员暂停/生效期；生产不自动写入。管理员可配置结构路线与模式。相关预算/DTO35项、商品接线/报价focus90项、Admin Chromium1项通过；整合图片模块434项通过，条件PG测试分开记录。以上仍非真实模型和线上验收。

服务状态读取同时检查现有四个图像模型档，Qwen-only 配置不再因默认万相关闭而被误报为无可用模型；状态查询不写库，专项通过。状态中的默认模型/默认积分元数据不是实际报价。

测试发布配置已补齐两个结构检查开关：GitHub staging environment vars →受控JSON→备份后原子写入测试.env。旧18项配置仍兼容，新开关缺省false；未知字段/非法布尔仍拒绝，生产不执行该writer。发布时开启仅针对已获准测试，不等于生产已启用。

- 百炼 `qwen3-vl-flash` 双图、非思考 JSON 路线按持久化六层预算/提交租约执行；图片与计划在异步I/O前复制，提示只使用与哈希一致的白名单字段。字节和像素上限、完整解码、响应上限均在调用前后核验。
- 检查身份、数量、部件关系、表冠/表带、文字、固有颜色及材质。模型返回字段由服务端固定裁决；合法MISMATCH不能被邻近坏字段吞掉，缺字段不判PASS。背景变化仅指原场景实质替换，不把光照或小杂物整理混作换景；实际判断准确率仍待图片验证。
- 报告、requestId、有限usage在Serializable租约CAS下持久化；同输入/计划/范围重复读取不再次调用。UNKNOWN及写入结果不明不重发。BILLING_EXCEPTION保留已有报告，重放再次采用共享裁决器，不能把已有UNCERTAIN/FAIL提升成PASS。
- 独立审查发现并修复未绑定提示字段、损坏图只读metadata、局部坏字段遮冲突、材质检查缺失、账务状态遮报告和矛盾PASS重放。Provider23/runner30项单测、独立PostgreSQL Core表9项预算/并发/故障测试已通过；后者不是完整业务数据库迁移验收。
- 结构Core已本地实现；商品接线与后台预算配置另行收口，尚未启用真实模型或部署。仅有mock语义测试不能证明能识别真实手表表冠错误。

### 2026-09-04 v3 本地实施：持久化付费任务

- 新增 `VisualTaskExecution`：确认冻结积分的同一 Serializable 事务创建任务，后台定时执行器使用数据库单语句抢占、租约续期和 generation/token CAS。PRODUCT/PUBLIC 按资产及范围匹配 handler；HTTP 查询不再推进模型执行。
- 未知提交不自动重发；可证明未提交的持久化 RESERVED 调用通过 CAS 释放，已受理任务只查询原供应商任务。取消/过期/释放停止调度。
- 新增商家商品任务历史与纯状态查询，游标必须属于当前公司、接入范围及商品。报价过期、资料变化使用结构化错误；并发确认仅对明确回滚的 Prisma P2034 最多重试五次，不重试未知网络错误。
- 新建隔离 PostgreSQL 16 空库应用 140 条迁移成功；真实测试验证四并发确认仅冻结一次、确认重放、任务 INSERT 失败全部回滚。另有任务表租约 SQL 的三项并发/接管测试（后者使用最小父表，不能替代全 Schema 测试）。临时数据库已清理。
- 主 Agent 全后端回归：302 suites / 3494 tests 通过，6 suites / 13 tests 条件跳过；上述显式隔离数据库测试单独执行。TypeScript 通过。仅本地，未部署；真实模型、页面全流程和效果验收尚未完成。

迁移回退：新增任务表不删既有业务字段。旧代码可保留新表运行，但回退到浏览器推进版本会失去无人值守恢复能力；切换时不得重复提交在途供应商任务。不要删除表或清空已确认任务；需要回退时先核验在途状态与账务，再回滚代码，表保留用于恢复/审计。

### 2026-09-04 v3 本地实施：报价配置权威性

- 自动测试初始化只创建缺失费率，不覆盖管理员既有价格、模型、暂停状态和生效期限；使用与管理修改一致的 advisory lock 和 Serializable 事务。
- 停用或缺失欢迎策略不阻塞已有积分账户；不把数据库故障吞掉当作无需赠送。
- 已确认任务按 `rateCardSnapshot.modelProfile` 选择提交及查询模型，不能因管理员后来改价或换模型而改变旧任务供应商。缺失快照进入明确失败/对账，不猜测供应商。
- 主 Agent 复核及独立审查后，配置专项 19 项、模型快照执行专项 12 项通过。仅本地代码；尚未部署和真实管理端验收。

### 2026-09-04 v3 本地实施：免费本地调优合约

商家交互切片已接入免费合约、历史记录、恢复、过期更新和暂不采用。真实 Chromium 挂载测试两个用例组通过（受控网络，非线上模型），另有 17 项专项契约/辅助行为测试；具体范围见 `docs/architecture/seller.md`。整体仍未部署/验收。

上传反馈另以真实 multipart/XHR 验证：取消计时器伪进度，传输100%后仍显示服务器处理中，响应成功才开放美化入口；不支持格式/损坏PNG无额外网络上传。上传回执与跨重启恢复尚未完成。

候选下载切片：新增登录商家作用域下载接口，校验任务/资产有效性、文件哈希及真实解码，私有无缓存二进制响应；前端下载不重发生成和扣费。单测18项、真实Nest HTTP4项和Chromium1项通过，独立审查通过，未部署。营销图已支持本地代码下载路径，不代表买家详情图片位或生产功能完成。

- 新计划声明 `local-photometric-v2`，固定亮度 1.025、对比度 1.015、饱和度 1、锐化 sigma 0.35；不改变尺寸和构图，不调用收费 OCR 或图像模型，不扣图片积分。不等于强效果精修，视觉参数仍待代表样本验收。
- 所有风险档可使用这项受限本地处理；没有扩大生成模型的 allowedModes。扫描证据如实记录 `NOT_REQUIRED / NOT_EVALUATED`，不伪造“没有文字/条码”。
- 新上传的同商家受管资产只需绑定有效商品计划即可生成，不要求先写商品公开图片；生成前再次检查所有权、资产状态、计划和下载字节 SHA256。只有明确采用时沿用既有事务/CAS写入候选与原图证据。
- 旧 v1 任务按持久化合约恢复，仍保留原扫描条件，不能因为当前计划已升级就自动换处理逻辑。真实 Sharp 像素处理及请求/采用专项共 41 项通过，独立审查通过；尚未部署与真实商家免费流程验收。

### 2026-09-04 补充真实验收：尚未整体通过

- 基线 `4b5031aa`：手表原图执行一次 CATALOG_STUDIO，生成期间可用积分 150→140、冻结 0→10；生成按钮与确认框处于禁用状态。候选标题正确为“商品棚拍候选”，关闭后可从原入口恢复同一候选。
- 白底效果失败：生成了白底，但手表表带变为横向、表冠从右侧移至顶部；现有比例/QR/条码检查未发现结构变化，仍展示可采用。此次未采用、未更换商品图。不能将生成成功视为效果或事实一致性通过。
- 免费路线仍阻塞：部署变量 QWEN_OCR_ENABLED/EXECUTION_ENABLED 为 false；实际条码扫描器不返回 NONE，免费路线要求 NONE，因此仅启用 OCR 仍无法运行。不得把识别失败当作无条码证据。
- 条码像素输入修复（本地）：商品扫描与通用候选验真原先将 RGB 三通道字节交给 ZXing 的单通道亮度接口；改为显式单通道灰度。真实 EAN-13 PNG 测试在旧代码失败、修复后通过；解码不确定仍返回 INCONCLUSIVE。本修复不开放免费调优，不代表结构一致性验收通过。
- 待验收：原始大图上传与等待反馈、第二商家完整路径、免费路线可用性、结构一致性以及不采用候选后的再次生成路径。

设计权威来源：[商品图片智能美化 Agent v3](../superpowers/specs/2026-08-22-universal-product-visual-agent-design.md)。目标是构建可接入爱买买、餐厅和第三方系统的视觉服务；商家确认报价后使用独立图片积分调用模型，商品事实保护是待按路线验证的要求，不能视为现有算法已保证。

以下为历史实现记录，保留用于追溯；运行状态以本节顶部日期快照及实际部署证据为准。
...
- 2026-09-03 全测试商家默认开放（2026-09-04 再收口）：当 `AI_VISUAL_AGENT_TEST_ACCESS_ENABLED=true`、`AI_VISUAL_AGENT_TEST_ALL_MERCHANTS_ENABLED=true` 且 API hostname 精确为测试域名时，任一 ACTIVE 测试商家的 OWNER/MANAGER 查看方案时，由费率查询服务在后端幂等准备该商家/人员/商品/费率卡的六层费用隔离策略并发放欢迎图片积分；浏览器不再调用“测试授权”写接口，管理员也没有逐商家授权入口。按测试要求，日、周及平台共享预算使用数据库整数范围内的等效无限上限，不限制调用次数；每次调用仍需商家明确确认图片积分。自动模式使用独立费率编码，并在列卡、报价和确认阶段持续检查总开关；关闭后立即阻止新调用，每分钟在报价锁内只释放 `RESERVED + invocationId=null` 的自动报价，不妨碍已被 Provider 接受的任务恢复。生产环境两个测试开关必须保持 false。
- 2026-09-04 staging 万相大输出恢复（修复候选）：首次真实 `wan2.7-image-pro` 商品图调用只提交了一个 Provider task；万相成功返回约 10.7MB 候选，但旧实现误用商家上传 10MB 入口限制，报价与调用保持 `RECONCILING`、10 图片积分保持冻结，未重发任务。Core 新增共享 `VisualAgentManagedOutputService`：Provider 输出最大 20MB/4000 万像素，统一校验 MIME 与真实格式；PNG/WebP 在 10MB 内保持原编码，其余先尝试无损 WebP，再按 95/88/82 三档压缩，最终候选仍必须不超过 10MB且尺寸不变。全局同时只规范化 1 张、等待队列最多 8 张，过载时保持原任务稍后恢复。审计摘要记录压缩前后 SHA-256、MIME、字节数、尺寸和编码版本；通用 API 与爱买买适配器都使用 managed buffer 做事实验证。候选存储、Invocation 完成、图片积分结算和候选 finalize 均可从 `VERIFYING / RECONCILING / SUCCEEDED` 与 `RESERVED / RECONCILING / SETTLED` 的部分完成状态幂等续跑；恢复只查询已绑定 Provider task，禁止再次 submit。
- 2026-09-04 历史商品图采用兼容（本地候选）：已上架商品若仍只有一张旧版 HTTPS URL 图片，允许商家采用已验真的 AI 候选；旧 URL、顺序、媒体类型和替代文本写入不可变历史快照，新公开媒体只保留受管候选和原实拍证据。管理员可按 `mediaVersion` CAS 精确恢复该旧 URL。视频、多张旧 URL、旧 URL 与其他媒体混合、非 HTTPS、含账号凭据或非规范 URL 继续拒绝，且服务端不会抓取旧 URL。
- 低清图片不采用全局开关：主体仍清楚且不含包装文字、型号、条码或关键瑕疵时，可带“AI 可能补造纹理”的明确提示进入超清修复；含文字/条码/型号/可见瑕疵的事实图片只允许保守增强并保留原图证据；主体、数量或边界已无法可靠辨认时要求重拍。超清结果仍须经过事实门禁、商家显式采用和平台事后巡检，不能把模型补出的细节当作商品事实。
- 2026-09-04 商家按钮与积分体验修复（本地候选）：Qwen OCR 未开启使用专用业务错误码和中文关闭态，不再暴露内部 Provider 名称或把普通 503 永久禁用；付费报价只以标题、副标题、说明、分类 ID/名称和媒体版本作为事实版本，草稿 `updatedAt` 自动保存不再误使计划失效，真实变化会在冻结图片积分前自动刷新计划和费率卡。图片积分条常驻显示可用、冻结、方案消耗和生成后预计余额，并以请求序号避免旧响应覆盖新余额；在线购买未实现时明确说明暂未开放。透明前景才进入免费 alpha 白底合成，普通照片进入付费 `CATALOG_STUDIO`；Wan/Qwen 只有在服务端 `allowedOperations` 含 `BACKGROUND_REPLACE` 时才要求白底，否则只简化原背景。
- 2026-09-04 有积分即可用（本地候选）：测试环境不再展示或要求逐商家/逐商品人工授权。ACTIVE 商家 OWNER/MANAGER 登录后，只要图片积分足够、商品归属正确且平台全局模型/费率/总成本保护有效，就能查看报价并显式确认调用。服务端为当前对象和操作人自动生成的精确预算记录只承担费用隔离和熔断职责，不属于商家权限。
- 2026-09-04 单请求报价收口（本地候选）：删除商家端和平台端的逐商家测试授权接口。商家只发起一次明确的方案解析 POST，服务端先验证商品、原图、计划和方向，再幂等准备测试环境所需的精确费用隔离记录并返回可用方案；自动测试模式不设置逐商家到期时间，已完整准备的范围走无写入快路径，部分失败可通过幂等重试补齐。无可用模型、费率或完整预算时返回明确中文服务错误，不再以空数组静默回到初始页面。
- 2026-09-04 真实任务交互复测（本地修复候选）：真实万相任务从 160 图片积分正确进入可用 150/冻结 10，完成后结算为可用 150/冻结 0；刷新可恢复同一 Provider 任务且不重复提交。商家端处理中禁用再次确认，关闭可采用的成功候选后可从原入口重新打开；不可采用的营销预览关闭时只结束本地会话、保留服务端历史并允许选择下一方案。候选标题、处理中说明和采用文件名均按当前 optimizationId 绑定的保留实景、棚拍、细节精修或营销方向显示，免费任务不会继承旧报价方向；采用后立即清除旧成功状态。
