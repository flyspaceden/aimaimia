...
## 17. 真相锁定商品视觉 Agent（2026-08-24，main 候选，未发布）

设计权威来源：[全品类商品视觉优化 Agent](../superpowers/specs/2026-08-21-product-image-optimization-design.md)。目标是改善全品类商品图，但不允许模型重绘商品、包装、型号、数量、颜色或二维码。
...
- 保真白底候选：商品证据源先经方向校正、去元数据、无损 WebP 规范化，再做 QR 扫描；仅实际含透明像素的证据源可使用最近邻几何变换、无损 PNG 和白色画布 alpha 合成校验。候选状态与显式采用尚未接通，因此对商家接口继续关闭。
