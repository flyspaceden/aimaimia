...
```ts
        // 微信 REFUNDING 退款：不能重复发起退款。
        // 先 queryRefund 查真实状态；仍 PROCESSING/查不到则保留 REFUNDING 等下一轮。
        if (claim.status === 'REFUNDING' && claim.paymentChannel === 'WECHAT_PAY') {
          if (!this.wechatPayService?.isAvailable()) {
            this.logger.warn(
              `微信 pending 退款查单服务不可用，保留 REFUNDING: refundId=${this.maskBizId(refund.id)}, merchantRefundNo=${this.maskBizId(claim.merchantRefundNo)}`,
            );
            continue;
          }
...
c) 售后 `retryRefund` 对 `REFUNDING + WECHAT_PAY` 也必须先 `queryRefund`，查不到不重发；`FAILED` 才允许重新发起同一 `out_refund_no` 的幂等退款请求。
...
- [ ] **Step 4: 上线前确认生产服务器可访问微信支付 API + 平台证书可刷新**

> 仅在真实凭据已写入 `docs/operations/密码本.md` 并同步到服务器 env 后执行。`WechatPayService` 每日 03:00 会主动 `fetchCertificates(apiV3Key)` 刷新微信平台证书缓存，但首次 notify 验签 cache miss 仍依赖服务器能访问 `api.mch.weixin.qq.com`。

Run（生产/测试服务器上执行；HTTP 401/403 可以接受，目标是验证 DNS + TCP + TLS 可达）：
```bash
curl -sv --connect-timeout 5 https://api.mch.weixin.qq.com/v3/certificates -o /dev/null
```

Expected: 能建立 TLS 连接并收到微信网关响应；不能是 DNS 失败、连接超时或 TLS 握手失败。

真实联调 checklist：
- 启动后端后调用 `WechatPayService.refreshPlatformCertificates()` 或等待每日 cron，日志应显示平台证书缓存刷新成功。
- 用真实凭据完成一次 "启动 → 收到微信支付/退款测试 notify → raw body 验签通过 → appid/mchid/金额校验通过" 链路。
...
- ~~wechat notify 缺 timestamp 重放窗口 / 平台证书刷新依赖外网~~ → 二次审计补充，`parseNotify` 验签前拒绝超过 5 分钟的 `wechatpay-timestamp`；`WechatPayService.refreshPlatformCertificates` 每日主动刷新平台证书缓存；Task 11 增加服务器到 `api.mch.weixin.qq.com` 连通性验证。
- ~~AS_SHIP_PAY_ 短码化误伤支付宝售后运费~~ → 二次审计补充，`AfterSaleShippingPaymentService` 只对微信 provider 使用短码，支付宝 provider 保持 `AS_SHIP_PAY_${afterSaleId}` 旧格式。
- ~~微信 queryOrder 返回 null 仍尝试 closeOrder~~ → 二次审计补充，`cancelSession` 直接拒绝取消，`CheckoutExpireService` 跳过本次 cron，避免把网络/SDK 异常当作未支付。
- ~~微信 REFUNDING 补偿 cron 重复发起退款~~ → 二次审计补充，`retryStaleAutoRefunds` 对微信 REFUNDING 退款只走 `queryRefund(outRefundNo)`，查不到或微信服务临时不可用也保持 REFUNDING 且不回落重发；售后 `retryRefund` 对 REFUNDING 微信单同样先查单；退货运费微信 REFUNDING 二次入口不再重复调用退款 API，查单连续无结果超过硬阈值后转 FAILED 并等待人工/系统重试。
