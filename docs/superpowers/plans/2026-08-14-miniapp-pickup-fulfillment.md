...
- [x] D2c（代码/本地验证完成，待 staging 真机）：修复买家凭证二维码真机绘制失败；新增卖家“扫码核销台”（二维扫码枪、电脑摄像头、人工短码均先复核后核销），补充全局凭证解析接口、归属/限流/并发回归。商品/SKU 条码只做可选拿货核对，不能替代一次性取货凭证；仍须在 staging 以真实二维码、扫码枪/摄像头和短码完成设备验收。
