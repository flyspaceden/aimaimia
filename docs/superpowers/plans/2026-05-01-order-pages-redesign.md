...
**目标**：列表 DTO 的商品 snapshot 增加 `skuTitle / companyId / isPrize / isPostReplacement` 字段，让前端"淘宝展开风"卡片有数据可显示。
...
const snapshot = (item: any) => {
  ...
  return {
    ...
    isPostReplacement: !!item.isPostReplacement, // 新增（OrderItem 字段，见 5.2 spec）
  };
};
```
...
- [ ] **Step 3: Controller 挂路由**

```ts
// order.controller.ts
@Get('checkout/me/pending')
@UseGuards(JwtAuthGuard)
async getMyPendingCheckout(@CurrentUser() user: any) {
  return this.checkoutService.getPendingForUser(user.id);
}
```
...
- [ ] **Step 2: Controller 挂路由**

```ts
@Post('checkout/:sessionId/resume')
@UseGuards(JwtAuthGuard)
async resumeCheckout(@CurrentUser() user: any, @Param('sessionId') sessionId: string) {
  return this.checkoutService.resumeSession(user.id, sessionId);
}
```
...
```ts
// src/repos/OrderRepo.ts 内
async getPendingCheckout(): Promise<Result<PendingCheckout | null>> {
  return this.http.get<PendingCheckout | null>('/orders/checkout/me/pending');
}

async resumeCheckout(sessionId: string): Promise<Result<{ paymentParams: any }>> {
  return this.http.post<{ paymentParams: any }>(`/orders/checkout/${sessionId}/resume`, {});
}
```

加 PendingCheckout 类型到 `src/types/domain/Checkout.ts`：
...
```bash
npx tsc -b
git add src/repos/OrderRepo.ts src/types/domain/Checkout.ts
git commit -m "feat(repos): OrderRepo 加 getPendingCheckout / resumeCheckout"
```
...
export function PendingCheckoutBanner() {
  ...
  const handleResume = async () => {
    ...
    // await PaymentRepo.invokeAlipay(r.data.paymentParams.orderStr);
    // 此处实际调用方法以项目现有支付封装为准（参考 app/checkout.tsx 内的调用）
    show({ message: '调起支付中...', type: 'info' });
  };
...
export default function CheckoutPendingScreen() {
  ...
  const handleResume = async () => {
    const r = await OrderRepo.resumeCheckout(pending.sessionId);
    if (!r.ok) return show({ message: r.error.displayMessage ?? '续付失败', type: 'error' });
    // 调起支付宝（参考 app/checkout.tsx 已有调用方式）
    show({ message: '调起支付中...', type: 'info' });
  };
...
### Task 23: app/checkout.tsx 6001 改造

**Files:**
- Modify: `app/checkout.tsx:460-465`

- [ ] **Step 1: 修改 6001 分支**

定位 `app/checkout.tsx:460` 的 6001 分支，把 `await OrderRepo.cancelCheckoutSession(sessionId);` 删除并替换为：
...
真机：购物车下单 → 调起支付宝 → 点 X → 确认跳到 /checkout-pending 页（不是 toast"支付失败"）。

- [ ] **Step 3: Commit**

```bash
git add app/checkout.tsx
git commit -m "fix(app/checkout): 支付宝 6001 取消改为跳 /checkout-pending（保留 Session）"
```
