...
| 团购数字资产 | 不新增团购特例，沿用现有数字资产/消费资产规则；本次不改 |
...
团购订单不定义新的数字资产特例，本次不改现有数字资产逻辑。

现有数字资产体系如何判断用户资格、何时入账、如何计算金额，继续按当前实现执行。团购只是一个订单业务类型，不因为团购本身新增“付款即入账”或“VIP 团购专属入账”规则。

金额口径仍可按正常消费理解：按商品实付金额累计，不含运费。由于团购订单禁止使用平台红包、消费积分、团购返还、VIP 折扣和其他优惠，团购商品实付金额通常等于团购价。
...
- 已按现有数字资产规则计入的团购相关流水不删除，必要时通过反向流水冲正。
