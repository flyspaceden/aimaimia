...
2. 即使用户已有当前团购，也能继续浏览团购商品，并可选择继续分享或主动结束后购买新团购。
...
团购商品不是普通 `ProductCard`。每个团购商品使用一张大卡展示，主图占 58%-65% 高度，右下角叠一个“团购吊牌”显示团购价、是否包邮、返还基数和有效条件摘要。吊牌不是装饰，它承载用户下单前最需要核对的信息。
...
- 动效只保留一处：商品护照卡进入页面时轻微上浮，分享进度轨状态变化时做一次短促高亮；避免全页粒子和过度光效。
...
团购商品列表不使用普通双列卡片。默认使用横向或纵向的大号 `商品护照卡`，根据商品数量和屏幕宽度切换：

- 1 个活动：首屏直接展示完整大卡，下方露出规则摘要和购买按钮。
- 2-4 个活动：横向分页大卡，类似 VIP 礼包页的主卡轮播，但颜色、卡片比例和信息层级使用团购主题。
- 5 个以上活动：首屏仍用横向精选大卡，下面增加紧凑列表入口；不做瀑布流。
...
- 活动结束时间。
- 返还基数：默认等于团购价，不含运费。
- 档位摘要：例如 `3 位有效好友 · 按档返还`，不得写敏感词。
- 简短规则提示：例如“满足分享条件后返还本次团购价，不含运费”。
...
详情页延续 `商品护照卡` 的视觉语言，但信息更完整：

- 商品基础信息、SKU、团购价。
- 运费规则：包邮或按平台运费规则计费。
- 商品规格：重量、数量、产地 / 企业、发货方式，如后端有字段则展示真实字段，无字段不虚构。
- 购买条件吊牌：现金购买、不可抵扣、同一时间一个当前团购。
- 返还说明卡：按后台档位展示，如 `第 1 位 10%`、`第 2 位 20%`、`第 3 位 70%`，只写“有效好友”，不写收益导向。
- 活动规则摘要。
- 关键限制：
  - 仅现金购买。
...
| 分享进行中 | 分享码已生成，等待直接推荐好友订单有效 | 分享码 / 继续分享 / 结束本次分享 |
...
- 进度使用三段轨道或按后台档位数量动态生成轨道，每段展示“待确认 / 已有效 / 已失效”。这是状态信息，不做排行榜。
...
后台允许配置任意数量档位，但全部档位比例总和必须等于 100%。默认三档为 10%、20%、70%。

为避免并发超收，分享码同一时间最多允许存在“档位数”数量的候选订单。候选订单退款、退货、换货或取消后释放候选名额。达到档位数个有效订单后，团购实例完成，分享码永久失效。
