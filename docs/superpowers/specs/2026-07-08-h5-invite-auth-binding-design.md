...
> 2026-07-24 补充：微信内完成登记后转系统浏览器下载，使用短时一次性下载凭证；它不承载推荐码，也不再触发推荐绑定。交接通过一次真实页面跳转的 `?handoff=` query 完成，不能依赖 hash。
>
...
微信内 H5 登录/注册并完成推荐关系处理
  ...
  -> 系统浏览器消费凭证并直接进入原有下载分发渠道
```

凭证只表示“这个 H5 会话已经完成登记，可以前往下载”；不包含推荐码、手机号、登录 Token 或用户资料。页面使用 Web Crypto 生成 256 位随机值，服务端只保存 SHA-256 哈希。微信页在发出登记请求前按 `landingSessionId` 将候选值写入本页 `sessionStorage`，因此即使服务端已写入但响应丢失后刷新，也会复用原会话和同一候选值，不会生成冲突的新凭证。由于微信跨到系统浏览器时不可靠保留仅由 History API 写入的 hash，凭证必须通过一次真实导航放入 `?handoff=` query；它是高熵、10 分钟、一次性的 opaque 值，并在系统浏览器消费前立即从地址栏删除。官网加 `Referrer-Policy: no-referrer`，避免该短时值作为 Referer 继续传播；服务器首次请求日志仍可能记录该 opaque 值，这是跨应用交接所需的最小暴露面。微信页只有同时存在本地 H5 Access Token 和 landing session 时才接受 handoff；伪造或残留 handoff 会被清除并恢复登录表单。相同随机值的网络重试可复用原有效凭证，但有效期固定从首次登记起算 10 分钟；已消费或过期时，微信页才生成新随机值。不同随机值的并发申请不能覆盖已有有效凭证。凭证无效、过期或已使用时，只提示返回微信页面重新点击下载，不重新显示登录/注册表单，也不泄露失败原因。
