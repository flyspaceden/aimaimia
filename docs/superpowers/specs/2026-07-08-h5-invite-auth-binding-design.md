...
> 2026-07-24 补充：微信内完成登记后转系统浏览器下载，使用短时一次性下载凭证；它不承载推荐码，也不再触发推荐绑定。`?handoff=` query 继续作为直接交接路径；针对部分安卓微信只会在系统浏览器重开二维码原始 URL 的行为，另以 2 分钟短时唯一设备上下文恢复，并保留剪贴板高熵凭证兜底。
>
...
微信内置浏览器和系统浏览器不共享 `sessionStorage`。用户已经在微信内完成手机号登录/注册时，推荐关系也已经由 `bindAfterAuth()` 写入后端；此时转浏览器不应再次显示手机号验证码表单，更不应再次尝试绑定推荐关系。

```text
微信内 H5 登录/注册并完成推荐关系处理
  -> 点击“下载 App”
  -> 浏览器以 Web Crypto 生成下载凭证，已登录 H5 会话登记其 SHA-256（10 分钟、一次性）
  -> 真实跳转至带 ?handoff=凭证 的当前邀请页，提示“在浏览器中打开”
  -> 系统浏览器保留 query：直接消费凭证并进入原有下载分发渠道
  -> 系统浏览器只重开原始 URL：短时唯一设备上下文恢复；失败则读取/粘贴已复制链接
```

凭证只表示“这个 H5 会话已经完成登记，可以前往下载”；不包含推荐码、手机号、登录 Token 或用户资料。页面使用 Web Crypto 生成 256 位随机值，服务端只保存 SHA-256 哈希。微信页在发出登记请求前按 `landingSessionId` 将候选值写入本页 `sessionStorage`，因此即使服务端已写入但响应丢失后刷新，也会复用原会话和同一候选值，不会生成冲突的新凭证。由于微信跨到系统浏览器时不可靠保留当前 hash/query，页面仍以真实导航写入 `?handoff=`，同时在原始点击手势内复制完整一次性下载 URL。系统浏览器若只重开二维码原始 URL，必须先尝试 2 分钟短时恢复，不能先创建新的未登录 landing session；仅当同邀请码、可信代理解析的同出口 IP、同扩展屏幕上下文、同语言、同安卓大版本和可从 `Build/` 提取的设备型号，并且完整候选集合中恰好一个微信来源时才原子消费。0 个或多个候选均拒绝猜测并转剪贴板兜底；外部浏览器首屏显示“继续下载”，只有用户明确选择后才显示手机号表单。

`ticket` 是高熵、10 分钟、一次性的 opaque 值，并在系统浏览器直接消费前立即从地址栏删除。官网加 `Referrer-Policy: no-referrer`，避免该短时值作为 Referer 继续传播；服务器首次请求日志仍可能记录该 opaque 值，这是跨应用交接所需的最小暴露面。微信页只有同时存在本地 H5 Access Token 和 landing session 时才接受 handoff；伪造或残留 handoff 会被清除并恢复登录表单。相同随机值的网络重试可复用原有效凭证，但有效期固定从首次登记起算 10 分钟；已消费或过期时，微信页才生成新随机值。下载凭证签发与短时唯一候选恢复共用事务级 advisory lock，候选读取后不能插入新凭证；不同随机值的并发申请也不能覆盖已有有效凭证。若续签发生在网络等待之后，页面不假设剪贴板仍可自动写入，而是要求用户点击遮罩按钮显式复制新链接。
...
```http
POST /api/v1/invite-h5/download-pass
Authorization: Bearer <H5 access token>
```

```json
{
  "landingSessionId": "ih5_xxx",
  "ticket": "base64url-encoded-256-bit-random-value"
}
```

前端以 Web Crypto 生成 `ticket`；服务端只在 `InviteH5LandingEvent.authedUserId` 与当前 Token 用户一致时登记该值的 `SHA-256(ticket)`、过期时间和消费时间，不回传原值。成功返回 `{ "status": "READY", "expiresAt": "..." }`。同一有效 `ticket` 重试返回同一可用状态，但不延长首次登记时确定的 10 分钟有效期；同一已过期或已消费 `ticket` 返回 `{ "status": "RENEW_REQUIRED" }`，由微信页生成新值后重试。不同 `ticket` 不能覆盖已有有效凭证。

系统浏览器消费凭证：

```http
POST /api/v1/invite-h5/download-pass/consume
```
...
该接口公开但受 IP 限流和 256 位随机凭证保护。消费使用单条 `updateMany` 条件更新：未消费、未过期且已完成 H5 登录才标记为已使用；无论不存在、过期或已使用，均只返回 `{ "valid": false }`。

系统浏览器只拿到原始邀请 URL 时恢复下载：

```http
POST /api/v1/invite-h5/download-pass/resume
```

请求复用 landing 设备上下文字段。后端只使用 Express 根据 `TRUST_PROXY` 得出的 `req.ips[0] / req.ip`，不直接信任客户端 `X-Forwarded-For`。签发和恢复共用事务级 advisory lock；恢复查询完整有效候选集合并要求恰好一个匹配，再以 CAS 标记已消费。接口只返回 `{ "valid": boolean }`，不暴露用户、会话、推荐关系或失败原因。
...
- 用户在微信内成功登录后点击“下载 App”时，页面只显示“在浏览器中打开”引导；系统浏览器消费下载凭证后自动进入下载，不再渲染第二次登录表单。
...
- 页面打开统计不能作为发奖依据。
- 日志中手机号必须脱敏。
- 下载凭证必须使用加密安全随机值，数据库只保存哈希；申请时用 `landingSessionId + authedUserId` 条件更新校验归属，消费时用未过期且未使用的 CAS 条件原子更新，TTL 固定为 10 分钟。
...
- 验证码错误时不创建账号、不绑定。
- 页面打开统计不返回推荐人信息。
- 已登录 H5 会话只能为本人申请下载凭证；其他用户或未知会话被拒绝。
- 下载凭证在 10 分钟内只能成功消费一次；重复、过期或伪造凭证只返回不可用。
- 微信打开同一邀请页后，系统浏览器消费凭证直接进入下载渠道，且不会新增扫码打开统计或再次触发推荐绑定。
