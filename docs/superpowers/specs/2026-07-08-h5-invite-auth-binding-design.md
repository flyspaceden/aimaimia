...
> **2026-08-11 已被产品决策覆盖**：对外 `/invite/{code}` 已不再收集手机号或验证码、也不在 H5 绑定推荐关系。扫码后改为无登录分流：微信中由用户打开小程序并在小程序完成微信登录和绑定，或下载 App 并由既有剪贴板/延迟匹配机制在 App 内完成。本文仅保留历史实现背景；当前入口规则见 `docs/architecture/frontend.md`。
>
> **For agentic workers:** 本文档已不再是当前对外入口的权威来源；不得恢复 H5 扫码后的手机号验证码登录或网页绑定。
