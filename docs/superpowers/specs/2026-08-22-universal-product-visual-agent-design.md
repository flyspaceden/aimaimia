...
> 状态：**v2 设计稿，待按本设计重构实现。**
>
> 本文覆盖此前“商品图片优化”与“通用 Visual Agent”设计中关于产品目标、模型路由、商家付费、公共 API、Adapter、验真和上线门禁的部分。现有本地代码具备受管资产、候选、预算/调用账本、Client Key 和爱买买的即时发布/事后回滚基础；它**不是**已经具备强大生成式美化能力的上线服务。
>
> 本文不授权数据库迁移、创建真实 Client Key、开通/订阅模型、真实付费调用、push、staging 部署或生产发布。

## 1. 一句话定义与成功标准

`AI Visual Agent` 是一个独立、可通过 API 接入的视觉服务：它理解一张业务图片应该保留真实场景、改成电商主图，还是只适合作营销图；给出清晰报价；在商家明确确认后调用受控模型；验证商品/菜品事实；把候选与完整审计记录交回接入系统，由接入系统执行明确的采用、发布和事后治理策略。
...
`MARKETING_SCENE` 不等于自由提示词。第一批只开放服务端审核过的固定摆拍模板：`HARVEST_PLATE`（采摘后的农产品放入白色陶瓷盘、自然厨房光线）、`HANDHELD_HARVEST`（双手捧起采摘农产品）和通用 `LIFESTYLE_TABLETOP`。模板允许改变场景、摆放方式、藤蔓/枝叶和展示数量，但必须保持商品类别、品种外观、成熟度范围、颜色和材质特征；展示数量不得宣称为包装数量或重量。
...
6. 草稿/未上架商品可显式采用；爱买买的上架商品在商家确认采用后即时替换买家可见图，并留下版本化历史供平台事后巡检和必要时回滚。其它系统由 Adapter 明确自己的发布规则，不由 Core 强行预审批或自动发布。
...
第一版营销场景候选仅提供私有预览，商家端和 Adapter 的事实主图采用接口必须拒绝 `MARKETING_IMAGE`。后续如开放详情图发布，应使用独立的营销图位和 AIGC 标识，不得复用事实主图 CAS。
...
6. 明确事实冲突或 Provider 输出无法获取时拒绝采用；低置信但未发现明确冲突时，Adapter 可以允许商家采用并标记为高优先级事后巡检，不能把“不确定”伪装成“无事实”。营销图不得在缺少对应事实策略时升格主图。
7. 商家确认与接入系统的发布/巡检策略是最后一道门，不替代算法验证；Core 不假设每个系统都需要同一种人工预审批。
...
| 阶段 | 交付 | 完成定义 | 当前状态 |
|---|---|---|---|
| A | 受管资产、私有候选、采用/回滚、基础 Core | 原图可追溯，候选不自动发布；Adapter 决定发布 | 已有基础代码 |
...
5. staging 用同一候选 SHA 完成真实 Provider 请求、成本对账、商家采用、事后回滚和错误恢复；生产发布另获授权。
