...
### 4.2.1 平台中心仓共享自提点（2026-08-16 补充）

保留上述“企业自有自提点”的全部规则，同时增加由平台公司拥有和维护的 `PLATFORM_HUB` 点位。它表达的是**一个真实的中心仓物理地点**，不是把任意企业点位伪装成平台点，也不能借此跨企业篡改订单归属。

1. `PickupPoint` 新增不可随意转换的 `kind = MERCHANT | PLATFORM_HUB` 和服务范围 `coverage = OWNER_COMPANY | ALL_ACTIVE_COMPANIES | SELECTED_COMPANIES`。`MERCHANT` 只能服务所属企业；`PLATFORM_HUB` 必须由 `Company.isPlatform=true` 的平台公司拥有，且只能服务全部正常经营企业或由平台显式授权的企业集合。
2. `SELECTED_COMPANIES` 使用 `PickupPointServiceCompany` 关联表保存授权企业；唯一约束 `pickupPointId + companyId`，并禁止把停业企业或中心仓所属平台公司以外的主体伪装为中心仓。新增/编辑时服务端事务校验点位类型、所有者、范围和授权集合，买家不能提交或推断任何授权。
3. 买家按商品所属企业发现点位时，返回“该企业自有点 + 对该企业生效的平台中心仓”。中心仓在结算页标为“平台中心仓”，地址、营业时间和取货须知仍以服务端点位快照为准；点位停用或撤销企业授权会立即阻止新结算，但不覆盖已付款订单的历史快照和异常处理责任。
4. 多商家结算中，多个企业可以选择同一个平台中心仓。**每个商家订单仍独立创建 `PickupFulfillment`、独立短码/二维码、独立退款和收货副作用**；本期不把多个订单静默合并成一张凭证。买家界面可明确提示“同一中心仓，需分别出示各订单取货码”，避免一张码误领全部商品。
5. 企业员工仍可对自己企业的 `PICKUP` 订单执行“备货完成 / 核销”，无论买家选择的是企业点还是经授权的平台中心仓。平台管理员新增独立 `pickup_fulfillment:operate` 权限，可对任一自提订单执行同样的两步核销（先解析凭证、人工复核、再不可逆确认）；不得复用 `orders:ship` 或泛化的 `orders:read` 权限。
6. 平台操作必须与卖家操作走同一 Serializable/CAS 状态机、凭证验证、订单收货副作用和退款作废逻辑；事件使用 `ADMIN_READY` / `ADMIN_VERIFIED`、订单历史和管理审计记录真实管理员身份。平台管理员不能仅因点位管理权限而核销，企业员工不能因中心仓存在而读取其他企业订单。
7. 平台后台不新增独立中心仓页面：复用“自提点管理”的新增/编辑弹窗。新增时打开“平台中心仓”开关即隐藏所属企业字段，服务端只查询唯一正常经营的 `isPlatform=true` 公司并自动绑定；浏览器传入的 `companyId` 不参与中心仓归属裁决。关闭开关时仍必须选择企业，保持企业自有点的现有流程。
