...
- 复购目标 SKU 当前库存为 `0` 时，仍加入购物车，数量为 `1`，但默认不可选、不可结算，只能删除。
- 如果购物车里已经有同 SKU，复购遇到低库存或无库存时，直接覆盖为 `1`，不累加。
...
处理规则：
- `currentStock <= 0`：最终购物车数量设为 `1`，`isSelected = false`，返回原因 `OUT_OF_STOCK_UNSELECTABLE`，消息“商品暂无库存，已放入购物车，可删除但不可结算”。
...
同 SKU 已在购物车时：
- 普通库存充足时继续按原逻辑累加。
- 低库存或无库存时覆盖同 SKU 普通购物车行的数量为 `1`，并按库存状态设置 `isSelected`。
...
交互规则：
- `OUT_OF_STOCK`：展示“无库存”，复选框禁用且保持未选中，数量加减禁用，删除按钮可用。
...
- 结算页：商品行展示和提交前库存刷新。
- 复购成功后的 toast：低库存调整和无库存不可选要有明确文案。
...
- 库存为 `0`：最终购物车数量为 `1`，未选中，不可被结算。
- 库存为负数：按无库存处理，最终数量为 `1`，未选中。
...
- 库存 0、历史订单 3：再次购买后购物车显示无库存，未选中，不能结算，可以删除。
