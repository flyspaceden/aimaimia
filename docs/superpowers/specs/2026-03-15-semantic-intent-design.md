...
在现有 `AiVoiceIntentSlots` 基础上新增 7 个字段：
...
flavorPreference?: string;                                 // 口味偏好："甜的"、"脆的"、"鲜美"、"香辣"
```
...
| `usage` | **废弃**，统一为 `usageScenario`，旧字段标记 `@deprecated`。`buildDemandSlots()` 中原来写入 `usage` 的逻辑同步改为写入 `usageScenario` |
| `audience` / `budget` / `recommendThemes` | 保留不动 |

### 1.3 顶层 `fallbackReason` 扩展

`AiVoiceIntent.fallbackReason` 已存在为 `string` 类型，现有值包括 `'asr-failed'`、`'empty-transcript'`、`'low-confidence'`、`'need-clarification'`、`'missing-query'`。本次新增三个语义分流值：

```typescript
// 现有值保持不变，新增以下三个用于 LLM 输出阶段的分流
'out-of-domain'  // 问题和平台无关 → 引导式回复
'too-vague'      // 表达太模糊 → 澄清话术或 clarify candidates
'unsafe'         // 敏感内容 → 拒绝模板，不做商品引导
```

字段类型保持 `string`（不收窄为联合类型），保证现有管道其他阶段设置的值不受影响。新增值仅由 LLM 分类阶段产出，执行层按值分流。

### 1.4 前后端同步

- 后端 `voice-intent.types.ts` 和前端 `src/types/domain/Ai.ts` 同步扩展
- `AiVoiceResolved` 新增以下 resolved 字段：
  - `usageScenario?: string` — 透传，用于搜索评分和前端展示
  - `originPreference?: string` — 透传，用于产地匹配
  - `dietaryPreference?: string` — 透传，用于饮食标签匹配
  - `promotionIntent?` / `bundleIntent?` — 透传（B 阶段降级处理）
  - `freshness` 不入 resolved，已归入 `constraints`
...
用户表达落到 Level 3
  ...
  │      · 有 categoryHint 且同时有至少一个补充信号
  │        （constraints / usageScenario / originPreference / dietaryPreference /
  │         freshness / budget / audience，任一非空即可）
  │        注意：categoryHint 单独存在不算好结果，避免模型随手填
  │        "食品/特产/水果"等宽泛兜底类目就跳过升级
  │      · 有任一高价值槽位（usageScenario / promotionIntent / bundleIntent /
  │        originPreference / freshness / dietaryPreference / budget / audience）
  │        即使没有 categoryHint，只要有场景/偏好信号就算好结果
  │      · constraints.length ≥ 1
  │
  │    ├─ 好结果 → 直接用，不升级
  │    └─ 差结果（贫瘠） → 升级 Qwen-Plus 重新抽取 (~1-1.5s)
  │         Plus 从头重做分类+抽取（replacement 模式，非 additive）
  │         将原始 transcript 重新发给 Plus prompt，不携带 Flash 结果
  │
...
```
你是爱买买App的语义理解器。将用户语音转录解析为结构化意图。
...
flavorPreference?, promotionIntent?, bundleIntent?, audience?, budget?,
recommendThemes? } }
```
...
model Product {
  ...
  originRegion    String?                    // 产地文本："山东青岛"、"云南"
}
```

> **与现有 `origin Json?` 字段的关系**：Product 已有 `origin Json?` 字段（存储产地坐标/行政区划结构化数据）。新增的 `originRegion` 是用于语义搜索匹配的扁平文本字段。AI 填充时可从 `origin` JSON 中提取行政区文本写入 `originRegion`。两者共存，`origin` 用于地图展示，`originRegion` 用于搜索匹配。

### 3.2 字段与槽位的匹配关系

| 商品字段 | 对应的用户侧槽位 | 匹配方式 |
|---------|---------------|---------|
| `flavorTags` | `flavorPreference` 槽位 | 交集匹配 |
...
| `flavorPreference` → `flavorTags[]` | 交集匹配 | +8 |

**口味匹配说明**：新增轻量槽位 `flavorPreference?: string`（如"甜的""脆的""鲜美"），由 LLM 从用户表达中抽取。不依赖 `query` 字段（因为 `query` 可为空），而是 LLM 直接从 transcript 中识别口味倾向。评分时将 `flavorPreference` 与商品 `flavorTags` 做交集。
...
### 5.3 `AiVoiceIntent` 协议扩展

当前 `AiVoiceIntent` 只有 `feedback: string` 文本反馈，没有 `suggestedActions` 载体。为支持 out-of-domain 引导式回复，需要扩展协议：

```typescript
// AiVoiceIntent 新增可选字段
chatResponse?: {
  reply: string;                   // 自然语言回复
  suggestedActions: AiSuggestedAction[];  // 复用现有 AiSuggestedAction 类型
};
```

处理流程：
- `handleChatClassification()` 检测 `fallbackReason` 后，调用 LLM 生成 `chatResponse`（含 reply + suggestedActions）
- 前端收到 `AiVoiceIntent` 时，若 `chatResponse` 存在，跳转 chat 页面承载 `suggestedActions`；否则走现有 `feedback` 文本展示
- 单轮语音链路本身不新增 UI 组件，通过跳转到已有的 chat 页面展示富交互内容
...
改动范围：
- 前后端类型定义：扩展 slots（7 个新槽位）、`AiVoiceIntent.fallbackReason`、`usage` → `usageScenario` 迁移
- 后端 LLM 管道：Flash/Plus 双 prompt、质量检查 + 条件升级逻辑
- `fallbackReason` 分流逻辑：注入到 `handleChatClassification()`（单轮语音意图的 chat 处理函数），而非 `chatWithContext()`（多轮聊天流程）。`handleChatClassification()` 根据 `fallbackReason` 值选择不同的系统 prompt 片段
...
按字段独立记录来源。Product 已有 `attributes Json?` 字段（schema.prisma line 1007，用于自定义属性）。`semanticMeta` 存入该字段的 `semanticMeta` 键下，与卖家自定义属性共存但命名空间隔离：
