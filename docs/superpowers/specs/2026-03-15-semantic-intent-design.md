...
| `query` 中的口味词 → `flavorTags[]` | 交集匹配 | +8 |

**口味匹配说明**：`flavorTags` 没有对应的独立槽位。口味匹配通过从 `query` 文本中提取口味关键词（甜/脆/鲜/辣/酸等，复用现有 `extractSearchConstraints()` 的模式），与商品 `flavorTags` 做交集。这和现有的 `constraints` 提取逻辑类似，但写入评分而非槽位。
