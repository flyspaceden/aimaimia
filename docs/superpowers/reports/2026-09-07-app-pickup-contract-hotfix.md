...
- TypeScript 无错误；App 165 项单元测试、274 项脚本检查通过。脚本测试先执行网站既有法律静态页生成步骤。
...
原生调试包确认：旧代码真实响应下抛出 groups.find TypeError；修复后普通、团购、VIP 入口均正常。普通自提填写/选点/预览/微信渠道请求通过，错误结构显示提示并禁用提交，恢复后重试成功。测试接口在真实支付前拦截。

PR #20 已合入 main `101ada3d`；候选 `06d65a78` 两项必需 CI 通过且合并树一致。Android 1.0.6 production OTA `01a07f11-fd33-7209-a8ba-a992a7c261f9` 已发布，63 个唯一资源哈希校验通过。真实付款和商家核销仍需真机验收。
