...
- `seller/`：企业点位、备货和核销台。
- `website/`：推荐扫码页为无登录双入口；用户显式选择小程序或 App，推荐关系在目标客户端登录后核验。
...
- 自提订单不得创建面单、快递物流记录或进入商城自动确认收货；核销后必须通过微信交易发货接口按“用户自提”上报。一次微信支付拆出多个自提子订单时，等待全部有效子订单核销后统一上报，不能把第一笔核销误报为整单完成。
...
2026-08-24 用户已单独批准双入口 InviteChoice：`/invite/{code}` 不再展示手机号、验证码或网页微信登录，只记录落地事件并在用户点击时生成小程序 URL Link，或保存 App 推荐交接口令后进入下载流程。手机端保持两张入口按钮；桌面端直接展示 App 二维码，小程序二维码必须点击生成，不允许页面加载时自动请求微信链接配额。旧后端 H5 登录路由暂时保留为兼容接口，但新网页不得调用；是否物理删除必须另做路由使用审计。
...
- 平台统一自提点。
- 推荐码、小程序码，以及 H5 无登录双入口到小程序/App 登录绑定的兼容回归。
- 订阅消息及微信交易发货。
...
- 42 个核心商城/后台 Service 或 DTO 的 Git blob 与已验收 `staging@acc0e08c` 精确一致；20 个生产允许差异均由 Git 自动枚举并有 manifest 说明与 Delivery 排除断言；无服务端行为证据的任务奖励保持 staging fail-closed。
...
- H5 InviteChoice 已转入 2026-08-24 独立候选；App 自提只读兼容仍保持独立提交和 OTA 验收，不与 H5 静态发布混合。
