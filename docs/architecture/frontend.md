...
首页是 AI 入口，不展示商品列表。2026-07-24 起页面顺序固定为：两行品牌标语 → 用户身份卡 → 亮色 VIP 推荐提醒 → 搜索/购物车 → 单一 AI 买买入口 → VIP 礼包轮播。顶部恢复固定文案「消费者就是生产力 / 是社会价值的创造者」；AI 下方旧的两行使命文案、首页精选团购入口和首页抽奖按钮继续移除，团购和抽奖统一迁入“我的 > 活动中心”。
...
- **搜索框**：胶囊形，背景 `surface`，点击跳转独立搜索页；左侧放大镜替换为独立透明的河豚角色，并用细分隔线将角色与搜索文案分开。麦克风与右侧购物车仍保留原功能图标，避免装饰角色削弱语音和购物入口的可识别性。
- **品牌标语**：首页顶部固定两行展示「消费者就是生产力 / 是社会价值的创造者」，保留系统字体缩放并通过 `adjustsFontSizeToFit` 将窄屏限制在两行；购物车继续只保留在搜索行右侧，避免重复入口。
