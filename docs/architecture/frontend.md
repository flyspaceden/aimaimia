...
| 买家消息中心统一通知 | 买家 App `/inbox` 保持原入口，但消息来源切到 `NotificationMessage`；点击动作优先使用 `action.routeKey + params`，已支持订单详情、物流、待改收货信息、售后详情、优惠券、钱包、数字资产、客服会话和团购详情，旧裸路径只作为兼容兜底；未知 routeKey 只提示不可跳转，避免进入 expo unmatched route；2026-06-30 审查收口：消息列表支持分页加载，交易分类兼容新 `order/after_sale/wallet/group_buy` 分类，未读按钮使用服务端全局未读数并有本地兜底 | 2026-06-29 | `src/types/domain/Inbox.ts`, `src/repos/InboxRepo.ts`, `app/inbox/index.tsx`, `src/utils/notificationRoutes.ts`, `src/utils/__tests__/notificationRoutes.test.ts` |
...
| 买家 App 客服会话管理 | `/cs` 默认展示客服中心会话列表，支持“进行中 / 历史对话”、最后一条消息、状态、未读数和刷新；“我的”页联系客服入口进入列表，订单/售后上下文仍可直接打开会话；具体聊天页接入 Socket.IO `cs:join_session`，管理后台主动联系买家后，买家打开会话即可实时收到客服消息，并保留 5 秒 HTTP 轮询兜底 | 2026-07-06 | `app/cs/index.tsx`, `app/(tabs)/me.tsx`, `src/repos/CsRepo.ts`, `src/types/domain/CustomerService.ts`, `src/repos/http/config.ts`, `backend/src/modules/customer-service/cs.controller.ts`, `backend/src/modules/customer-service/cs.service.ts`, `backend/src/modules/customer-service/cs.gateway.ts`, `scripts/__tests__/buyer-cs-conversations.test.mjs` |
