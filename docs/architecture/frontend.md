...
首页是 AI 入口，不展示商品列表。核心是中央大光球。所有用户在搜索框下方均展示 VIP 礼包横滑推广位（读取公开接口 `GET /bonus/vip/gift-options`）：礼包跑马灯不再展示顶部标题行和档位数量，只保留卡片主体；VIP 用户另保留"推荐好友开通 VIP"单行提醒引导分享推荐码。VIP 点礼包卡片进入 `/vip/gifts` 浏览模式（不可购买，底部 CTA 为「分享给好友开通」）。
...
- **首页品牌标语**：固定展示「消费者就是生产力 / 是社会价值的创造者」，替代原时段问候和随机 AI 引导语；使用深绿色、加粗、固定两行展示，右侧保留购物车入口
- **搜索框**：胶囊形，背景 `surface`，点击后跳转独立搜索页；位置在两行品牌使命文案下方，已抽奖提示紧跟搜索框下方展示
- **VIP 礼包卡片**：首页只展示价格、礼包标题和简短副标题，不展示顶部标题行、档位数量、SKU、规格、数量等赠品明细；需要看明细时点击进入 `/vip/gifts`
- **大光球**：页面核心，占据视觉中心约 40% 的区域
  - 球内文字 **"AI"** 超大（36pt，fontWeight 800），**"买买"** 小字（16pt）紧贴其下
  - 整体上大下小排列，"AI" 占据球体视觉主体，强烈凸显 AI 身份
- **品牌使命文案**：AI 光球提示下方固定展示两行居中文案「让消费者创造一个属于自己的世界 / 为全世界创造一个共生的未来」，替代原快捷指令横滑气泡
...
- `/me` 身份卡头像右侧只保留昵称主标题，不再显示时段问候语；买家公开编号 `buyerNo` 以 `ID: AIMM00000000000001` 形式单独成行展示并提供复制按钮，复制成功提示「用户编号已复制」。若后端暂未返回编号，显示「ID: 用户编号生成中」，不得展示内部 `User.id`。
