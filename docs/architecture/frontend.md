...
| 首页顶部品牌标语 | 首页顶部移除时段问候和随机 AI 引导语，固定两行展示「消费者就是生产力 / 是社会价值的创造者」；AI 光球下方移除快捷指令气泡，改为「让消费者创造一个属于自己的世界 / 为全世界创造一个共生的未来」两行品牌使命文案；新增纯函数常量测试防止旧文案回归 | 2026-06-15 | `app/(tabs)/home.tsx`, `src/utils/homeHero.ts`, `src/utils/__tests__/homeHero.test.ts` |
