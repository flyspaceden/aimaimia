...
| 卖家/管理后台通知中心 | 卖家中心和管理后台顶栏新增通知铃铛，未读数每 60 秒刷新；新增 `/notifications` 列表页展示标题、内容、时间、未读状态和“查看”动作，点击前先标记已读；已支持卖家订单/售后/商品入口与管理端售后/发票/提现/客服入口，未知 routeKey 提示“该消息暂无可跳转页面” | 2026-06-29 | `seller/src/api/notifications.ts`, `seller/src/pages/notifications/index.tsx`, `seller/src/layouts/SellerLayout.tsx`, `admin/src/api/notifications.ts`, `admin/src/pages/notifications/index.tsx`, `admin/src/layouts/AdminLayout.tsx`, `seller/test/notificationsEntry.test.mjs`, `admin/test/notificationsEntry.test.mjs` |
| 买家消息中心统一通知 | 买家 App `/inbox` 保持原入口，但消息来源切到 `NotificationMessage`；点击动作优先使用 `action.routeKey + params`，已支持订单详情、物流、待改收货信息、售后详情、优惠券、钱包、数字资产、客服会话和团购详情，旧裸路径只作为兼容兜底；未知 routeKey 只提示不可跳转，避免进入 expo unmatched route | 2026-06-29 | `src/types/domain/Inbox.ts`, `src/repos/InboxRepo.ts`, `app/inbox/index.tsx`, `src/utils/inboxRouteAction.ts`, `scripts/__tests__/inbox-route-action.test.mjs` |
