...
- 未选中：`textTertiary` 色
- 首页、发现、我的三个 Tab 分别使用河豚、海星、小贝卡通角色；选中时轻微上移放大并保留底部光点，未选中时降低透明度
...
- `/me/referral` 是统一推荐中心：普通用户展示普通分享码、绑定推荐人、自动成为 VIP 进度、普通推荐收益说明和最近推荐用户；VIP 用户展示 VIP 推荐码、VIP 直邀 VIP 人数、绑定推荐人和最近推荐用户。普通和 VIP 的新二维码/分享链接统一指向官网 H5 `https://app.ai-maimai.com/invite/{code}`，被推荐人可先在 H5 用手机号验证码登录/自动注册并绑定关系；若用户在微信内完成登记后转系统浏览器下载，H5 以首次登记起固定 10 分钟的一次性下载凭证交接，候选值按 landing session 先写入微信页 `sessionStorage` 以支持响应丢失后的刷新重试。`?handoff=` 真实导航继续作为兼容路径；针对部分安卓微信菜单重新打开二维码初始 URL、丢弃当前 query/hash 的行为，系统浏览器会先调用短时恢复接口，仅在同邀请码、可信代理解析的同出口 IP、同屏幕/DPR/色深/时区/触控点、同语言、同安卓大版本与可由 `Build/` 确认的设备型号且 2 分钟内完整候选集合只有一个微信来源时原子消费并直达下载，多候选或无法确认时绝不猜测。下载凭证签发和唯一候选恢复共用事务级 advisory lock，关闭候选读取后的并发签发窗口。微信点击下载时同时把完整一次性下载 URL 写入剪贴板；若旧票据需在网络响应后续签，则不假设浏览器仍允许自动复制，而是提示用户点击遮罩按钮显式复制新链接。自动恢复失败时，外部浏览器首屏展示“继续下载”，允许用户显式读取或手动粘贴，不再直接要求重复填写手机号。凭证仍不包含手机号、登录 Token 或推荐码；读取剪贴板必须由用户点击触发，官网保留 `Referrer-Policy: no-referrer`。页面同时展示 H5 邀请漏斗统计（扫码打开次数、H5 已登录人数、H5 已绑定人数，不混入 App 内或历史绑定）。若推荐人摘要缺失但已有关系，前端仍按已绑定展示，名称兜底为"已绑定用户"。
