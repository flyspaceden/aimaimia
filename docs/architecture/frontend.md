...
| 卖家中心订单打印清单 | Seller 前端订单详情的“打印”改为“打印清单”，打印页只展示订单号、买家匿名信息、地区、电子面单号、商品明细和数量；不输出单价、小计、商品金额等卖家平台价格，不附电子面单 iframe，避免第二页空白；打印清单整体大字号，商品名称和数量加大加粗，普通商品不显示“普通”标签，便于卖家拣货核对 | 2026-06-18 | `seller/src/pages/orders/detail.tsx`, `seller/src/utils/waybillPrint.ts`, `seller/test/waybillPrint.test.ts` |
...
| 团购精选货架与详情页 | 新增 `/group-buy` 货架页和 `/group-buy/[activityId]` 详情页：无当前团购时展示商品货架；有当前团购时默认进入“我的团购”并可切换商品；当前面板展示推荐码/二维码/进度/结束入口，商品卡和进度不展示返还基数或返还百分比；购买新团购时用抽屉提示需先结束本次分享 | 2026-06-22 | `app/group-buy/index.tsx`, `app/group-buy/[activityId].tsx`, `src/components/group-buy/*` |
| 团购扫码落地与现金付款 | 新增 `/gb/[code]` 推荐码落地页和 `/group-buy/checkout` 团购付款页；落地页展示分享用户和同款商品，未登录先弹登录再回到付款；付款页复用地址选择和现有支付宝/微信支付确认链路，但不展示消费积分、平台红包、团购返还余额等抵扣入口，并在当前团购占用时返回当前团购处理 | 2026-06-22 | `app/gb/[code].tsx`, `app/group-buy/checkout.tsx` |
