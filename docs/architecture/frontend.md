...
| 团长经营扫码绑定与团长中心 | 新增独立 `Captain` domain types 与 `CaptainRepo`，调用 `/captain/landing/:code`、`/captain/bind`、`/captain/me`、`/captain/me/ledgers`、`/captain/me/orders`；新增 `/c/[code]` 团长码落地页和 `/me/captain` 团长经营中心；我的页仅对 ACTIVE 团长展示“团长经营”入口；`/c/*` 已加入 Android App Links、iOS Universal Links 和官网下载路由，且 App 根布局识别 `/c` 链接时不走 VIP `/r` 推荐绑定逻辑；官网 `/c/{团长码}` 下载页会复制团长链接，App 首启读取后保存 `pending_captain_code`，登录后调用 `CaptainRepo.bindByCode`，覆盖未安装 App 的团长码增长路径；页面文案使用“团长经营奖励 / 推广奖励 / 到账”，不展示三级及以下关系 | 2026-07-08 | `src/types/domain/Captain.ts`, `src/repos/CaptainRepo.ts`, `app/c/[code].tsx`, `app/me/captain.tsx`, `app/(tabs)/me.tsx`, `app/_layout.tsx`, `src/services/deferredLink.ts`, `src/store/useAuthStore.ts`, `app.json`, `website/src/App.tsx`, `website/src/pages/Download.tsx`, `website/src/lib/referralClipboard.ts`, `website/public/.well-known/apple-app-site-association`, `src/repos/__tests__/CaptainRepo.test.ts`, `src/utils/__tests__/captainShare.test.ts` |
