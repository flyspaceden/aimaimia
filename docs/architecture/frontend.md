...
| 团长适用范围联想与参数示例 | 团长配置的适用类目、商品、商户和排除商品接入真实候选数据：点击加载、输入中英文名称或部分 ID 联想、滚动分页，选中后保存实体 ID 但界面展示可读名称。页面所有可编辑参数的问号说明改为“什么意思 / 举个例子 / 还要一起看”，每项使用具体数字说明生效结果；浮层正文使用自适应宽度、中文自动换行和屏幕安全边界，长文字不会被右侧裁剪 | 2026-07-12 | `admin/src/pages/captain/ScopeEntitySelect.tsx`, `admin/src/pages/captain/settings.tsx`, `admin/src/api/captain.ts`, `backend/src/modules/admin/captain/*`, `admin/tests/captain-scope-suggestions.test.mjs` |
