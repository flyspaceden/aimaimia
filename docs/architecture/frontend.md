...
| 我的页数字资产排名 | `GET /me/digital-assets/summary` 新增 `assetRank`，按有数字资产账户的 VIP 用户 `cumulativeSpendAmount` 从高到低计算，资产更多者排名从 1 开始；没有数字资产账户或非 VIP 时返回 `null`。买家 App 我的页身份卡推荐码同一行右侧展示“数字资产排行榜：x”，无排名显示“数字资产排行榜：未上榜”，点击进入 `/me/digital-assets`。 | 2026-06-28 | `backend/src/modules/digital-asset/digital-asset.service.ts`, `src/types/domain/DigitalAsset.ts`, `app/(tabs)/me.tsx`, `scripts/__tests__/me-identity-card-layout.test.mjs` |
