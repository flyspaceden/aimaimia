...
| 卖家中心订单打印清单 | Seller 前端订单详情的“打印”改为“打印清单”，打印页先展示订单号、买家匿名信息、地区、电子面单号、商品明细和数量；不输出单价、小计、商品金额等卖家平台价格，再附电子面单 iframe 并自动唤起浏览器打印，便于卖家拣货核对 | 2026-06-18 | `seller/src/pages/orders/detail.tsx`, `seller/src/utils/waybillPrint.ts`, `seller/test/waybillPrint.test.ts` |
...
| 数字资产中心 | 我的页常用工具新增“数字资产”入口；`/me/digital-assets` 按会员态分流：普通用户只展示累计消费金额和“开通 VIP 激活资产”引导，VIP 用户展示数字资产总额、种子资产、消费资产、累计消费金额、消费资产倍率进度、VIP 种子资产规则、长期模块占位和最近 5 条消费记录；“查看全部消费记录”进入 `/me/consumption-records`，非 VIP 只可查看 `CUMULATIVE_SPEND` 流水，VIP 可查看累计消费/种子资产/消费资产全量流水；通过 `DigitalAssetRepo` 调用 `/me/digital-assets/summary` 与 `/me/digital-assets/ledgers`，使用 React Query 刷新，金额文本使用 `priceTextProps`，列表底部使用 `useBottomInset` 适配安全区 | 2026-06-17 | `app/(tabs)/me.tsx`, `app/me/digital-assets.tsx`, `app/me/consumption-records.tsx`, `src/types/domain/DigitalAsset.ts`, `src/types/domain/index.ts`, `src/repos/DigitalAssetRepo.ts`, `src/repos/index.ts` |
