...
| 普通成长中心与普通分享码 | 我的页常用工具新增“普通成长”入口；`/me/growth` 展示普通积分、成长值、等级进度、普通分享码二维码、复制、系统分享、绑定好友普通分享码、积分兑换和最近邀请记录；普通分享码使用独立 `/s/{code}` 链路，与 VIP `/r/{code}` 和团购 `/gb/{code}` 隔离；App 首启和登录后可识别普通分享剪贴板口令并调用 `GrowthRepo.bindNormalShareCode`，不复用 VIP 推荐码绑定逻辑；官网新增 `/s/{code}` 普通分享落地页，点击下载/复制时写入普通分享链接，继续复用现有下载分流能力 | 2026-07-04 | `app/(tabs)/me.tsx`, `app/me/growth.tsx`, `app/s/[code].tsx`, `app/_layout.tsx`, `src/types/domain/Growth.ts`, `src/repos/GrowthRepo.ts`, `src/services/deferredLink.ts`, `website/src/pages/NormalShareLanding.tsx`, `website/src/lib/referralClipboard.ts` |
