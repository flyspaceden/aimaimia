...
| 买家消息中心统一通知 | 买家 App `/inbox` 保持原入口，但消息来源切到 `NotificationMessage`；点击动作优先使用 `action.routeKey + params`，已支持订单详情、物流、待改收货信息、售后详情、优惠券、钱包、数字资产、客服会话和团购详情，旧裸路径只作为兼容兜底；未知 routeKey 只提示不可跳转，避免进入 expo unmatched route；消息列表支持分页加载，交易分类兼容新 `order/after_sale/wallet/group_buy` 分类，未读按钮使用服务端全局未读数并有本地兜底；重要平台公告保留 `WARNING/IMPORTANT` 元数据并显示“重要”标记 | 2026-06-29 / 2026-07-10 更新 | `src/types/domain/Inbox.ts`, `src/repos/InboxRepo.ts`, `app/inbox/index.tsx`, `src/utils/notificationRoutes.ts`, `src/utils/__tests__/notificationRoutes.test.ts`, `scripts/__tests__/inbox-announcement-types.test.mjs` |
