...
| 买家消息中心统一通知 | 买家 App `/inbox` 保持原入口，但消息来源切到 `NotificationMessage`；点击动作优先使用 `action.routeKey + params`，已支持订单详情、物流、待改收货信息、售后详情、优惠券、钱包、数字资产、客服会话、团购详情和商品详情，旧裸路径只作为兼容兜底；未知或缺少必要参数的 routeKey 只提示不可跳转，避免进入 expo unmatched route；消息列表支持分页加载，交易分类兼容新 `order/after_sale/wallet/group_buy` 分类，未读按钮使用服务端全局未读数并有本地兜底；重要平台公告保留 `WARNING/IMPORTANT` 元数据并显示“重要”标记；分类与仅未读筛选使用单一状态，清空筛选一次恢复全部消息并扩大 Android 真机触控区域；单条消息支持左滑软删除和 5 秒撤销，右上角菜单支持清空已读或二次确认后清空全部，删除仅按当前 `recipientKey` 生效；消息时间使用本地友好格式，不再直接显示 ISO 字符串 | 2026-06-29 / 2026-07-11 更新 | `src/types/domain/Inbox.ts`, `src/repos/InboxRepo.ts`, `app/inbox/index.tsx`, `src/utils/inboxDisplay.ts`, `src/utils/inboxFilters.ts`, `src/utils/notificationRoutes.ts`, `src/repos/__tests__/InboxRepo.test.ts`, `src/utils/__tests__/inboxDisplay.test.ts`, `src/utils/__tests__/inboxFilters.test.ts`, `src/utils/__tests__/notificationRoutes.test.ts`, `scripts/__tests__/inbox-announcement-types.test.mjs` |
