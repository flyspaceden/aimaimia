...
| 首页/我的页换位后普通成长入口修复 | 共享名片恢复普通用户 `普通推荐码` chip，首页按 `bonus-member` 状态传入普通分享入口；“我的”页常用工具恢复 `普通成长 / 会员成长 / 成长中心` 动态入口，避免 VIP 推荐体系与普通分享码混用 | 2026-07-05 | `app/(tabs)/home.tsx`, `app/(tabs)/me.tsx`, `src/components/cards/MeIdentityCard.tsx`, `scripts/__tests__/normal-growth-ui.test.mjs` |
...
| 普通成长中心与普通分享码 | 我的页常用工具新增“普通成长”入口；`/me/growth` 展示普通积分、成长值、等级进度、普通分享码二维码、复制、系统分享、绑定好友普通分享码、积分兑换和最近邀请记录；普通分享码卡片区分加载失败、可分享和已停用状态，加载失败或后台停用时不再展示可用二维码，也禁用复制/系统分享入口；成长页隐藏全局 AI 浮动入口，避免遮挡绑定、分享、兑换等推广动作；普通分享码使用独立 `/s/{code}` 链路，与 VIP `/r/{code}` 和团购 `/gb/{code}` 隔离；App 首启和登录后可识别普通分享剪贴板口令并调用 `GrowthRepo.bindNormalShareCode`，不复用 VIP 推荐码绑定逻辑；官网新增 `/s/{code}` 普通分享落地页，点击下载/复制时写入普通分享链接，继续复用现有下载分流能力 | 2026-07-04 | `app/(tabs)/me.tsx`, `app/me/growth.tsx`, `app/s/[code].tsx`, `app/_layout.tsx`, `src/types/domain/Growth.ts`, `src/repos/GrowthRepo.ts`, `src/services/deferredLink.ts`, `src/components/effects/AiFloatingCompanion.tsx`, `website/src/pages/NormalShareLanding.tsx`, `website/src/lib/referralClipboard.ts` |
| 普通成长规则可见化 | 我的页用户卡片内对普通用户显示“普通推荐码”按钮并跳转 `/me/growth`，VIP 用户不显示该普通入口；`/me/growth` 通过 `/growth/guide` 拉取后台启用规则，新增“推荐收益”“赚积分和成长值”“升级规则”，展示奖励数值、发放时机、次数限制、等级门槛，并明确普通积分兑换会消耗、成长值升级不消耗；奖励标签限制宽度，避免长奖励文案挤出小屏；官网 `/s/{code}` 安卓落地页优先尝试 `aimaimai://normal-share` 打开已安装 App，失败后再进入下载兜底 | 2026-07-04 | `app/(tabs)/me.tsx`, `app/me/growth.tsx`, `src/repos/GrowthRepo.ts`, `src/types/domain/Growth.ts`, `website/src/pages/NormalShareLanding.tsx`, `website/src/lib/referralClipboard.ts`, `scripts/__tests__/normal-growth-ui.test.mjs` |
| 普通/VIP 直推与自动 VIP 展示 | `/me/growth` 和 `/bonus/member` 展示普通/VIP 直推比例、当前直推关系、累计普通商品有效消费自动成为 VIP 的门槛与剩余金额；普通成长页说明普通邀请后可按后续普通商品订单利润获得直推佣金，好友成为 VIP 时按邀请人当时是否为 VIP 决定关系失效或进入 VIP 团队；VIP 成长页、推荐页、我的页推荐弹窗和 VIP 中心统一说明“好友成为 VIP 后进入 VIP 团队，后续普通商品订单按 VIP 直推比例结算，购买 VIP 礼包本身不单独发推荐奖” | 2026-07-05 | `app/(tabs)/me.tsx`, `app/me/growth.tsx`, `app/me/referral.tsx`, `app/me/vip.tsx`, `src/types/domain/Growth.ts`, `src/types/domain/Bonus.ts`, `src/repos/GrowthRepo.ts`, `src/repos/BonusRepo.ts`, `src/utils/referralRelation.ts`, `backend/src/modules/growth/growth.service.ts`, `backend/src/modules/bonus/bonus.service.ts` |
