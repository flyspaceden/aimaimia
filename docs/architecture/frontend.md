...
- 管理后台“订单队列奖励”配置页允许把队列利润比例设置为 1%–100%，并复用统一 `ProfitSafetyStatus` 和未保存参数预检接口；平均分配模式下未挂载的正态随机字段会以当前已保存值补齐，确保任意可见参数变更都能触发完整候选预检。调整开关或比例后实时展示队列占用、普通/VIP平台份额扣除前后值及最弱场景收入安全余量；预检未完成、失败、超过普通/VIP平台份额或结果不安全时禁用保存并显示具体原因，后端仍以 Serializable 利润安全事务作为最终强制裁决。
