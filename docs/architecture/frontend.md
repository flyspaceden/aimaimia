...
首页是 AI 入口，不展示商品列表。2026-07-24 起页面顺序固定为：用户身份卡 → 亮色 VIP 推荐提醒 → 搜索/购物车 → 单一 AI 买买入口 → VIP 礼包轮播。旧的顶部品牌标语、AI 下方两行使命文案、首页精选团购入口和首页抽奖按钮全部移除；团购和抽奖统一迁入“我的 > 活动中心”。
...
│  └─────────────────────────┘│
│  推荐好友开通 VIP      去分享 │ ← 亮色提醒，VIP 且有推荐码时展示
│                             │
│  ┌──────────────────┐  🛒  │
│  │ 🔍 搜索商品，或问我... │  │ ← 搜索占原团购入口位置
│  └──────────────────┘      │
...
- **VIP 推荐提醒**：使用薄荷白亮色背景、深绿文字和实色绿 CTA，不再使用深色整条背景。
- **搜索框**：胶囊形，背景 `surface`，点击跳转独立搜索页；右侧购物车为独立圆形按钮，避免删除原购物入口。
- **AI 主入口**：首页只保留一个 AI 买买按钮，不再与抽奖并排。左右仅使用已确认的龙虾和帝王蟹透明角色图，角色不压住主球；紧凑屏扩大舞台到页面全宽并将光球降为 136pt。
...
| 批次 | 内容 | 完成日期 | 涉及文件 |
|------|------|----------|----------|
| 首页/我的页海鲜视觉重排 | 首页移除品牌标语、使命文案、团购和抽奖，搜索前置，单一亮色 AI 球配龙虾/帝王蟹，VIP 轮播回到首页；我的页新增活动中心，财库/VIP 改全宽纵排，订单/工具/三 Tab 改为独立透明海鲜角色；补 320px 三列订单与 136pt 中号光球适配 | 2026-07-24 | `app/(tabs)/home.tsx`, `app/(tabs)/me.tsx`, `app/(tabs)/_layout.tsx`, `src/components/effects/AiOrb.tsx`, `src/components/ui/SeafoodIcon.tsx`, `assets/seafood/*` |
