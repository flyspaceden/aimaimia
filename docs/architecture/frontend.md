...
| 卖家中心组合商品创建体验 | Seller 创建/编辑商品页顺序改为「基本信息 → 价格与库存 → 商品图片」；组合商品在价格与库存内先选组合内容再填组合成本价；“搜索并添加单品规格”选后清空，已有组合复制改为按钮弹层；未审核通过组件在选择器中置灰，草稿接口支持保存计量单位 `unit` | 2026-06-25 | `seller/src/pages/products/edit.tsx`, `backend/src/modules/seller/products/seller-products.dto.ts`, `backend/src/modules/seller/products/seller-products.service.ts`, `backend/src/modules/product/product-bundle.service.ts` |
