...
> - 只有经过真机确认的单页例外，才允许传 `androidMinimumBottomPadding` 做页面级逃生；当前唯一例外是 `app/invoices/request.tsx`，用于避免发票申请页 CTA 在 low/zero bottom inset 时被系统手势区压到屏幕外。
...
- `BundleSummary` 及购物车 / 结算 / 订单 / 售后里的组合摘要必须复用同一横向约束：左侧缩略图固定尺寸，中间文本列 `flex: 1` + `minWidth: 0`，右侧数量列固定宽度并右对齐。
- 父商品主信息区、价格区、售后按钮区不能被组件文案挤压走位；允许父行自然增高，不允许把数量列或售后按钮做成覆盖文本的绝对定位浮层。
...
> **配套文件**：
> - `src/theme/responsive.ts`（工具实现；`useBottomInset()` 默认只封装系统 safe-area + caller extra，支持单页 `androidMinimumBottomPadding` 例外）
