...
- **功能**:
  ...
  - 面单打印（订单详情“打印清单”生成单页卖家拣货清单，并自动唤起浏览器打印；拣货清单只含父订单商品、商品名旁 `x数量` 和右侧数量，不输出卖家平台价格，不附电子面单 iframe；整体大字号，普通商品不显示“普通”标签；2026-06-23 起不再输出详情清单、组合明细或第二页拣货汇总）
  - 取消面单（未发货时）
  - 确认发货
  - 批量发货
...
#### 问题 8: 面单真实打印方案缺失
- **类型**: 产品/硬件方案
- **当前**: 订单详情已提供浏览器打印页（单页商品拣货清单 + 自动 `window.print()`）；批量打印仍为面单 iframe 批打
- **问题**: 没有对接热敏打印机 / 云打印机，卖家仍需要通过浏览器系统打印对话框确认
- **方案选项**:
  1. **浏览器打印**（当前）- 免费，已具备商品清单和自动唤起打印
  2. **顺丰/云打印机方案** - 需硬件与云打印权限
  3. **网页调用系统打印机** - 当前已实现自动唤起，但浏览器仍会要求用户确认
- **推荐**: 先使用当前浏览器打印闭环，后期订单量上来后再评估云打印硬件
- **工作量**: 0.5-2 天（取决于方案）
...
```env
# 顺丰丰桥（SF Fengqiao）
SF_API_URL="https://sfapi.sf-express.com/std/service"  # 生产环境（⚠️ 不是 bsp-oisp，那个已废弃返回 404，2026-06-05 修正）
SF_API_URL_UAT="https://sfapi-sbox.sf-express.com/std/service"  # UAT 沙箱
...
- UAT: `https://sfapi-sbox.sf-express.com/std/service`
- PROD: `https://sfapi.sf-express.com/std/service`

（⚠️ 历史上曾用 `bsp-oisp.sf-express.com` / `bsp-oisp-uat.sf-express.com`，2026-06-05 实测这两个域名的 `/std/service` 均已废弃返回 404，已统一切到 `sfapi(-sbox).sf-express.com`，详见密码本 §7。）
