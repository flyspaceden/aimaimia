...
| 手动发放 | `MANUAL` | 管理员按指定买家、普通买家、VIP 买家或全部有效买家发放；也可标记为“积分兑换专用”作为积分成长兑换红包池 | 手动 / 积分兑换 |
...
| 有效期 | 从领取时起算的有效天数 | `validDays`（0 表示跟随活动结束时间；长期活动必须大于 0） |

固定金额红包必须满足 `minOrderAmount >= discountValue`。管理后台会要求先填写抵扣金额，再填写最低消费门槛；后端创建/编辑活动时也会拒绝门槛低于抵扣金额的配置。百分比折扣没有固定抵扣金额，因此不套用这条比较规则。
...
enum CouponTriggerType {
  ...
  FIRST_ORDER       // 首次下单
  BIRTHDAY          // 生日
  CHECK_IN          // 签到（当前管理后台不开放新建）
  INVITE            // 邀请新用户
  REVIEW            // 好评（当前管理后台不开放新建）
  SHARE             // 分享
...
model CouponCampaign {
  ...
  // 有效期
  validDays            Int                   @default(7)  // 领取后有效天数（0=跟随活动结束时间；长期活动必须大于 0）
...
| 活动时间 | `startAt ~ endAt` | 日期范围；长期活动显示“长期有效” |
