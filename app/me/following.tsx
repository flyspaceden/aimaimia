        ...
        {isLoading ? (
          ...
          <View style={{ marginTop: spacing.md }}>
            <EmptyState
              ...
              actionLabel={hasKeyword ? '清空搜索' : '去首页'}
              onAction={() => {
                if (hasKeyword) {
                  setKeyword('');
                  return;
          ...
          >
            {filteredItems.map((item: FollowListItem, index: number) => (
              <Animated.View key={item.author.id} entering={FadeInDown.duration(300).delay(50 + index * 30)}>
                <Pressable
