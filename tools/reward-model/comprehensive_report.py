...
def generate_report():
    ...
    add('| 参数 | 普通用户 | VIP用户 |')
    add('| :--- | :--- | :--- |')
    add(f'| 树结构 | 2叉树, maxLayers=8 | 3叉树, 13层, 50%填充率, 10个根 |')
    add(f'| 用户比例 | 50% | 50% |')
    ...
    add('## 四、逐层奖励分析（加价率=1.30）')
    add()
    add('展示每一层用户的数量和月均可获得的奖励金额。')
    add()
    ...
    for worst in [True, False]:
        ...
        # VIP树（3叉×50%填充, maxLayers=13, 10个根）
        add(f'#### VIP树（3叉×50%填充, maxLayers={MAX_LAYERS_VIP}, 10个根节点）')
        add()
