...
export default function MerchantApply() {
  ...
  function handleFileSelect(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!ALLOWED_FILE_TYPES.includes(selected.type)) {
      setFieldErrors(prev => ({ ...prev, file: '仅支持 JPG、PNG、PDF 格式' }))
      e.target.value = ''
      return
    }
      ...
      <div className="max-w-2xl mx-auto px-6">
        <ScrollReveal>
          <div className="text-center mb-10">
            <h1 className="text-h1-mobile md:text-h1 text-text-primary mb-3">商户入驻申请</h1>
            <p className="text-text-secondary">填写以下信息，开启您的农业直销之旅</p>
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal delay={0.1}>
          <form onSubmit={handleSubmit} className="bg-white rounded-card-lg p-8 shadow-card space-y-6">
            {/* 企业/店铺名称 */}
            <div>
              <label htmlFor="companyName" className={labelClass}>企业/店铺名称 *</label>
              <input
                id="companyName"
                type="text"
                value={companyName}
            ...
            {/* 经营品类 */}
            <div>
              <label htmlFor="category" className={labelClass}>经营品类 *</label>
              <select
                id="category"
                value={category}
            ...
            {/* 联系人姓名 */}
            <div>
              <label htmlFor="contactName" className={labelClass}>联系人姓名 *</label>
              <input
                id="contactName"
                type="text"
                value={contactName}
            ...
            {/* 手机号 */}
            <div>
              <label htmlFor="phone" className={labelClass}>手机号 *</label>
              <input
                id="phone"
                type="tel"
                value={phone}
            ...
            {/* 邮箱（选填） */}
            <div>
              <label htmlFor="email" className={labelClass}>邮箱 <span className="text-text-tertiary font-normal">(选填)</span></label>
              <input
                id="email"
                type="email"
                value={email}
            ...
            {/* 验证码 */}
            <div>
              <label htmlFor="captcha" className={labelClass}>验证码 *</label>
